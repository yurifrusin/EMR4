#!/usr/bin/env python3
"""Credential-free fixture cell for one receptionist-first v6.2 turn."""
...
def run() -> dict[str, Any]:
    ...
    turn_code = turn.get("turn_code")
    ticket = turn.get("correction_ticket")
    task = turn.get("task")
    desk_context = (
        task.get("desk_context") if isinstance(task, dict) else None
    )
    if (
        turn.get("contract_version")
        != "reception.one.bureau.receptionist-first.v6.2"
        ...
        or not isinstance(desk_context, dict)
        or turn.get("task_sha256") != _hash(task)
        or turn.get("desk_context_sha256") != _hash(desk_context)
        or desk_context.get("authority")
        != "context_only_no_command_authority"
        or desk_context.get("effect_ceiling") != "proposal_only"
        or desk_context.get("excluded_context")
        != [
    ...
    return {
        "schema_version": (
            "reception.one.receptionist_first_v62.fixture_cell_result.v1"
        ),
        "status": "completed",
        "turn_code": turn_code,
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": (
                        "reception.one.receptionist_first_v62."
