#!/usr/bin/env python3
"""Provider-free real-isolation rehearsal for the v6.3 form toolkit."""
...
from scripts import reception_one_receptionist_first_v63 as receptionist


ARTIFACT_PATH = receptionist.ARTIFACT_DIR / "real-isolation-evidence.json"
IMAGES = (
    "reception-one-receptionist-first-v63-turn-1:v1",
    "reception-one-receptionist-first-v63-turn-2:v1",
)
CONTAINERS = (
    "reception-one-receptionist-first-v63-turn-1",
    "reception-one-receptionist-first-v63-turn-2",
)
CELL_SCRIPT = (
    ROOT / "scripts" / "reception_one_receptionist_first_v63_cell.py"
)


class IsolationError(RuntimeError):
    """A bounded v6.3 isolation failure."""
...
def run_isolation() -> dict[str, Any]:
    ...
    if (
        ...
        raise IsolationError("v63_isolation_boundary_invalid")
    return {
        **evidence,
        "schema_version": (
            "reception.one.receptionist_first_v63.isolation_evidence.v1"
        ),
        "result": (
            "reception_one_receptionist_first_v63_real_isolation_pass"
        ),
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "result": (
                        "reception_one_receptionist_first_v63_"
