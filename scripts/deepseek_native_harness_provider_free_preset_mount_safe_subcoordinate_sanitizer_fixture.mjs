...
function wrapperTerminal(code) {
  return { stage: "fixture_boot", code, detail: null };
}
...
let sanitizePresetMountError;
try {
  ({ sanitizePresetMountError } = await import("./deepseek_native_harness_provider_free_preset_mount_safe_subcoordinate_sanitizer.mjs"));
  if (typeof sanitizePresetMountError !== "function") {
    sanitizePresetMountError = undefined;
  }
} catch {
  sanitizePresetMountError = undefined;
}

if (sanitizePresetMountError === undefined) {
  publish(wrapperTerminal("SANITIZER_MODULE_IMPORT_REJECTED"));
} else {
  try {
    const results = [
    ...
    publish(wrapperTerminal("SANITIZER_MATRIX_EVALUATION_REJECTED"));
  }
}
