#!/usr/bin/env python3
"""Provider-free real-isolation rehearsal for the v6.4 proofreader."""
...
from scripts import reception_one_receptionist_first_v64 as receptionist


ARTIFACT_PATH = receptionist.ARTIFACT_DIR / "real-isolation-evidence.json"
IMAGES = (
    "reception-one-receptionist-first-v64-turn-1:v1",
    "reception-one-receptionist-first-v64-turn-2:v1",
)
CONTAINERS = (
    "reception-one-receptionist-first-v64-turn-1",
    "reception-one-receptionist-first-v64-turn-2",
)
CELL_SCRIPT = (
    ROOT / "scripts" / "reception_one_receptionist_first_v64_cell.py"
)


class IsolationError(RuntimeError):
    """A bounded v6.4 isolation failure."""
...
def run_isolation() -> dict[str, Any]:
    ...
    if (
        ...
        raise IsolationError("v64_isolation_boundary_invalid")
    return {
        **evidence,
        "schema_version": (
            "reception.one.receptionist_first_v64.isolation_evidence.v1"
        ),
        "result": (
            "reception_one_receptionist_first_v64_real_isolation_pass"
        ),
        "desk_context_sha256": receptionist.build_turn_input(
            base._fixture()[0]
        )["desk_context_sha256"],
        "same_context_packet_proofread": True,
        "recognized_constraint_gate": True,
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "result": (
                        "reception_one_receptionist_first_v64_"
