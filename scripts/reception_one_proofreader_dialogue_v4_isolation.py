#!/usr/bin/env python3
"""Provider-free real-isolation rehearsal for both v4 dialogue turns."""
...
IMAGES = (
    "reception-one-proofreader-dialogue-v4-turn-1:v1",
    "reception-one-proofreader-dialogue-v4-turn-2:v1",
)
CONTAINERS = (
    "reception-one-proofreader-dialogue-v4-turn-1",
    "reception-one-proofreader-dialogue-v4-turn-2",
)
DOCKERFILE = legacy_lane.ARTIFACT_DIR / "Dockerfile"
CELL_SCRIPT = ROOT / "scripts" / "reception_one_proofreader_dialogue_v4_cell.py"
FRAME = (
    ROOT
    / "orchestration"
    / "continuity"
    ...
    *,
    ...
    run_command(
        ...
        [
            ...
            container,
            "--hostname",
            f"reception-one-proofreader-dialogue-v4-turn-{index}",
            "--network",
            "none",
            "--read-only",
            "--user",
...
def run_isolation() -> dict[str, Any]:
    ...
    corrected = structured.load_object(FIXTURE)
    typed_plan.validate_schema(frame, "input")
    structured.validate_exact(corrected, structured.PLAN_PROGRAM_SCHEMA_PATH)
        ...
        for source in step["source_refs"]:
            ...
            exposed = {
                ...
                for output in structured.operator_table()[
                    first["steps"][source["prior_step_index"]]["operator_code"]
                ]["output_slots"]
            }
    ...
    first_input = dialogue.build_turn_input(frame)
    packets: list[dict[str, Any]] = []
    checks: list[dict[str, bool]] = []
    manifests: list[dict[str, str]] = []
    cleanup_errors: list[str] = []
    with tempfile.TemporaryDirectory(
        prefix="reception-one-proofreader-dialogue-v4-"
    ) as raw:
        root = Path(raw)
        ...
        try:
            ...
            second_input = dialogue.build_turn_input(
                frame,
    ...
    residue = {
        ...
        "images_present": any(
            exists(docker, "image", item) for item in IMAGES
        ),
        "temporary_context_present": False,
    }
    if cleanup_errors or any(residue.values()):
        raise IsolationError("cleanup_or_residue_failed")
    first_evaluation = dialogue.evaluate_program(
        frame,
        packets[0]["program"],
        turn_code=1,
    )
    second_evaluation = dialogue.evaluate_program(
        frame,
        packets[1]["program"],
        turn_code=2,
    )
    if (
        ...
        != dialogue.canonical_hash(ticket)
    ...
    return {
        "schema_version": (
            "reception.one.proofreader_dialogue_v4.isolation_evidence.v1"
        ),
        "result": (
            "reception_one_proofreader_dialogue_v4_real_isolation_pass"
        ),
        "evidence_mode": "authored_synthetic_provider_free_two_turn_fixture",
        "source_manifest_sha256": dialogue.canonical_hash(manifests),
        "task_sha256": first_input["task_sha256"],
        "first_program_sha256": dialogue.canonical_hash(
            packets[0]["program"]
        ),
        "first_disposition": first_evaluation["disposition"],
        "correction_ticket_sha256": dialogue.canonical_hash(ticket),
        "second_program_sha256": dialogue.canonical_hash(
            packets[1]["program"]
        ),
        "second_disposition": second_evaluation["disposition"],
        "normalized_plan_sha256": dialogue.canonical_hash(
            second_evaluation["normalized_plan"]
        ),
