#!/usr/bin/env python3
"""Run the bounded one-or-two-call Reception One v5 form dialogue."""
...
class PreprintedLiveError(RuntimeError):
    """A bounded parent-form lifecycle rejection."""
...
def _append_parent_event(
    ...
    event = {
        "schema_version": getattr(
            ...
            "reception.one.preprinted_form_v5.parent_audit_event.v1",
        ),
...
def decide_sequence(
    ...
    *,
    attempt_ids: tuple[str, str] = ATTEMPT_IDS,
    ledger_ids: tuple[str, str] = LEDGER_IDS,
    maximum_provider_calls: int | None = None,
) -> dict[str, Any]:
    """Pure two-call state machine for tests and occupied execution."""

    call_ceiling = (
        preprinted.MAX_PROVIDER_CALLS
        if maximum_provider_calls is None
        else maximum_provider_calls
    )
    if (
        isinstance(call_ceiling, bool)
        or not isinstance(call_ceiling, int)
        or not 1 <= call_ceiling <= preprinted.MAX_PROVIDER_CALLS
    ):
        raise PreprintedLiveError("provider_call_ceiling_invalid")
    if len(turns) > call_ceiling:
        raise PreprintedLiveError("absolute_call_ceiling_exceeded")
    for index, turn in enumerate(turns, start=1):
        if turn.get("provider_call_count") != 1:
            raise PreprintedLiveError("turn_call_count_invalid")
        if turn.get("attempt_id") != attempt_ids[index - 1]:
            raise PreprintedLiveError("turn_attempt_order_invalid")
        if turn.get("ledger_id") != ledger_ids[index - 1]:
            raise PreprintedLiveError("turn_ledger_order_invalid")
        if turn.get("ledger", {}).get("status") != "consumed":
            raise PreprintedLiveError("turn_ledger_not_consumed")
    actual_calls = sum(turn["provider_call_count"] for turn in turns)
    if actual_calls > call_ceiling:
        raise PreprintedLiveError("absolute_call_ceiling_exceeded")
    if not turns:
        return {
            "status": "ready",
    ...
    if first_release is not None and first_ticket is not None:
        raise PreprintedLiveError("release_and_correction_ticket_conflict")
    if first_release is not None:
        if len(turns) != 1:
            raise PreprintedLiveError("call_after_admission_forbidden")
        return {
            "status": "admitted",
            "next_turn_code": None,
    ...
    if len(turns) == 1:
        ...
        if call_ceiling == 1:
            return {
                ...
                "actual_provider_calls": 1,
                "terminal": True,
                "correction_ticket_hash": preprinted.canonical_hash(
                    first_ticket
                ),
            }
        return {
            ...
            "actual_provider_calls": 1,
            "terminal": False,
            "correction_ticket_hash": preprinted.canonical_hash(first_ticket),
        }
    second = turns[1]
    if second["exchange"].get("correction_ticket") is not None:
        raise PreprintedLiveError("second_correction_ticket_forbidden")
...
def build_parent_evidence(
    *,
    ...
    if len(turns) != len(external_audits):
        raise PreprintedLiveError("turn_audit_count_mismatch")
    for turn, audit in zip(turns, external_audits, strict=True):
        if (
            ...
            raise PreprintedLiveError("turn_external_audit_binding_invalid")
    release = next(
        (
    ...
    return {
        "schema_version": (
            getattr(
                ...
                "reception.one.preprinted_form_v5.parent_evidence.v1",
            )
        ),
        "result": (
            getattr(
                preprinted,
                "PARENT_RESULT_PASS",
                "reception_one_preprinted_form_v5_occupied_pass",
            )
            if release is not None
            else getattr(
                preprinted,
                "PARENT_RESULT_NO_RELEASE",
                "reception_one_preprinted_form_v5_occupied_no_release",
            )
        ),
        "dialogue_protocol": preprinted.DIALOGUE_PROTOCOL,
        "model_response_contract": (
            getattr(
                preprinted,
                "MODEL_RESPONSE_CONTRACT",
                "reception.one.bureau.preprinted-model-form-body.v5",
            )
        ),
        "assembled_response_contract": "reception.one.bureau.plan-program.v3",
        ...
        "absolute_provider_call_ceiling": call_ceiling,
        ...
        "exact_binding": turns[0]["exact_binding"] if turns else None,
        "explicit_exclusions": {
        ...
        "candid_limit": (
            ...
            "fitness, training need or safety for real, product, patient, "
            "health, clinical or historical data."
        ),
    }
...
def run_dialogue(
    *,
    ...
    if len(attempt_ids) != 2 or len(ledger_ids) != 2:
        raise PreprintedLiveError("dialogue_identifier_budget_invalid")
    call_ceiling = (
        preprinted.MAX_PROVIDER_CALLS
        if maximum_provider_calls is None
        else maximum_provider_calls
    ...
    if (
        ...
        raise PreprintedLiveError("provider_call_ceiling_invalid")
    for attempt_id, ledger_id in zip(attempt_ids, ledger_ids, strict=True):
        try:
            live.broker.validate_attempt_ledger_pair(attempt_id, ledger_id)
        except live.broker.BrokerError as error:
            raise PreprintedLiveError(str(error)) from error
    ...
    if any(path.exists() for path in all_paths):
        raise PreprintedLiveError("dialogue_output_already_exists")
    turns: list[dict[str, Any]] = []
    external_audits: list[dict[str, Any]] = []
    parent_events: list[dict[str, Any]] = []
    _append_parent_event(
        ...
        {
            "dialogue_protocol": preprinted.DIALOGUE_PROTOCOL,
            "maximum_actual_provider_calls": call_ceiling,
            "incremental_cost_ceiling_usd": 1,
            "turn_two_terminal": True,
            "broker_owned_fields": ["version_code"],
            "model_authored_fields": list(preprinted.MODEL_AUTHORED_FIELDS),
        },
    )
    for index in range(1, call_ceiling + 1):
        decision = decide_sequence(
            turns,
            attempt_ids=attempt_ids,
            ledger_ids=ledger_ids,
        ...
        if decision["next_turn_code"] != index:
            raise PreprintedLiveError("dialogue_state_machine_invalid")
        ...
        if decision["next_turn_code"] == 2:
            ...
            if preprinted.canonical_hash(ticket) != decision[
                "correction_ticket_hash"
            ]:
                raise PreprintedLiveError("correction_ticket_hash_mismatch")
            _write_json(ticket_path, ticket)
    final_decision = decide_sequence(
        turns,
        attempt_ids=attempt_ids,
        ledger_ids=ledger_ids,
        maximum_provider_calls=call_ceiling,
    ...
    if not final_decision["terminal"]:
        raise PreprintedLiveError("dialogue_not_terminal")
    _append_parent_event(
        parent_audit_path,
        parent_events,
        "dialogue_closed",
...
def main() -> int:
    ...
    except (PreprintedLiveError, live.LiveError, ValueError) as error:
        print(
            json.dumps(
                {
                    "result": (
                        "reception_one_preprinted_form_v5_occupied_blocked"
                    ),
