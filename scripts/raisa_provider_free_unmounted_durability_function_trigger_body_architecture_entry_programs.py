...
from scripts import (
    raisa_provider_free_unmounted_durability_function_trigger_body_architecture_builder as dsl,
)
...
TYPES: dict[str, dict[str, str]] = {
    relation: {
        column: type_name for column, type_name in zip(columns, type_names, strict=True)
    }
    for relation, columns, type_names in [
        (
            ...
            [
                PG + "name",
                F + "logical_capability",
                PG + "uuid",
                F + "source_contract_code",
                PG + "bigint",
                PG + "bigint",
                PG + "timestamptz",
                PG + "timestamptz",
        ...
        (
            ...
            [
                ...
                PG + "bigint",
                PG + "bigint",
                F + "admission_entry_kind",
                PG + "bigint",
                F + "key_id",
                F + "digest_sha256",
                F + "digest_sha256",
                F + "digest_sha256",
                F + "observation_decision",
        ...
        (
            ...
            [
                ...
                PG + "bigint",
                PG + "bigint",
                F + "recovery_pin_reason",
                F + "recovery_pin_state",
                PG + "timestamptz",
                PG + "timestamptz",
            ],
        ),
...
def _loc(value: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        ...
        "source_contract_id": _field(
            value, "source_contract_id", F + "source_contract_code"
        ),
...
def _predicate(relation: str, values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return dsl.all_of(
        *(dsl.eq(_src(relation, key), value) for key, value in values.items())
    )


def _row_predicate(
    symbol: str, relation: str, values: dict[str, dict[str, Any]]
) -> dict[str, Any]:
    return dsl.all_of(
        *(dsl.eq(_col(symbol, relation, key), value) for key, value in values.items())
    )
...
def _composite(
    type_name: str, fields: list[tuple[str, dict[str, Any]]]
) -> dict[str, Any]:
    return {
        "op": "COMPOSITE_CONSTRUCT",
        "type": type_name,
        "fields": [{"field": name, "value": value} for name, value in fields],
    }


def _retry_or_return(
    body_id: str,
    suffix: str,
    source_symbol: str,
    type_name: str,
...
def _winner(
    relation: str, bindings: list[tuple[str, dict[str, Any]]]
) -> dict[str, Any]:
    predicates = []
    for name, value in bindings:
        source = _src(relation, name)
        if value.get("op") == "CONST" and value.get("value") is None:
            predicates.append(dsl.unary("IS_NULL", source))
        else:
            predicates.append(dsl.eq(source, value))
    return dsl.all_of(*predicates)
...
def _symbols(
    inputs: list[tuple[str, str]], locals_: list[tuple[str, str]]
) -> list[dict[str, Any]]:
    return [
...
def _binding(
    body_id: str, capability: str, values: dict[str, dict[str, Any]]
...
def _admission_bindings(
    ...
    *,
    ...
    if kind not in {"PRIMARY", "CONFLICT"}:
        raise ValueError(f"unsupported admission kind: {kind}")
    if (kind == "PRIMARY" and conflict is not None) or (
        kind == "CONFLICT" and conflict is None
    ):
        raise ValueError("admission kind and conflict reason disagree")

    primary = kind == "PRIMARY"
    observation_digest = (
        _field(packet, "observation_digest", F + "digest_sha256")
        if primary
        else dsl.const(F + "digest_sha256", None)
    )
    decision = (
        _field(packet, "decision", F + "observation_decision")
        if primary
        else dsl.const(F + "observation_decision", None)
    )
    reason_code = (
        _field(packet, "reason_code", F + "observation_reason")
        if primary
        else dsl.const(F + "observation_reason", None)
    )
    affected_frame_mask = (
        _field(packet, "affected_frame_mask", F + "frame_mask")
        if primary
        else dsl.const(F + "frame_mask", None)
    )
    checkpoint_disposition = (
        _field(packet, "checkpoint_disposition", F + "checkpoint_disposition")
        if primary
        else dsl.const(F + "checkpoint_disposition", None)
    )
    attempted_admission_digest = (
        dsl.const(F + "digest_sha256", None) if primary else attempted
    )
    ...
    values.extend(
        [
            ...
            (
                "source_membership_digest",
                _field(packet, "source_membership_digest", F + "digest_sha256"),
            ),
            ("admission_digest", attempted),
            ("observation_digest", observation_digest),
            ("decision", decision),
            ("reason_code", reason_code),
            ("affected_frame_mask", affected_frame_mask),
            ("checkpoint_disposition", checkpoint_disposition),
            ("attempted_admission_digest", attempted_admission_digest),
...
def build_admission_body() -> dict[str, Any]:
    ...
    attempted = dsl.digest(
        ...
        [
            *loc.values(),
            position,
            *(
                _field(packet, name, type_name)
                for name, type_name in [
                    ("observation_digest", F + "digest_sha256"),
                    ("decision", F + "observation_decision"),
                    ("reason_code", F + "observation_reason"),
                    ("affected_frame_mask", F + "frame_mask"),
                    ("checkpoint_disposition", F + "checkpoint_disposition"),
                    ("key_id", F + "key_id"),
    ...
    primary_bindings = _admission_bindings(
        loc,
        position,
        packet,
        binding_revision,
    ...
    mismatch_bindings = _admission_bindings(
        loc,
        position,
        packet,
        binding_revision,
        kind="CONFLICT",
        attempted=attempted,
        conflict="POSITION_DIGEST_MISMATCH",
    )
    reuse_bindings = _admission_bindings(
        loc,
        position,
        packet,
        binding_revision,
        kind="CONFLICT",
        attempted=attempted,
        conflict="OBSERVATION_DIGEST_REUSE",
    )

    retained_primary = [
        dsl.select_node(
            f"{body_id}.retained_primary",
            relation=ADMISSION,
            columns=COLUMNS[ADMISSION],
            predicate=primary_pred,
            cardinality="EXACTLY_ONE",
            output_symbol="retained_primary",
        ...
        _if(
            ...
            [
                _retry_or_return(
                    body_id, "primary_replay", "retained_primary", ADMISSION
                )
            ],
            [
                _insert_reload(
                    f"{body_id}.insert_mismatch",
                    ADMISSION,
                    mismatch_bindings,
                    "mismatch_conflict",
                    COORDS + ["source_position", "entry_kind"],
                ),
                _retry_or_return(
                    body_id, "mismatch_conflict", "mismatch_conflict", ADMISSION
                ),
            ],
        ),
    ]
    retained_conflict = [
        dsl.select_node(
            f"{body_id}.retained_conflict",
            relation=ADMISSION,
            columns=COLUMNS[ADMISSION],
            predicate=conflict_pred,
            cardinality="EXACTLY_ONE",
            output_symbol="retained_conflict",
            order_by=COORDS + ["source_position", "entry_kind"],
        ),
        _retry_or_return(body_id, "conflict_replay", "retained_conflict", ADMISSION),
    ]
    source_pred = _predicate(
        SOURCE,
        {
            "practice_id": loc["practice_id"],
            "source_contract_id": loc["source_contract_id"],
    ...
    reuse_pred = dsl.all_of(
        ...
        dsl.eq(
            _src(ADMISSION, "observation_digest"),
            _field(packet, "observation_digest", F + "digest_sha256"),
        ),
    ...
    first_membership = [
        ...
        dsl.assert_node(
            f"{body_id}.source_digest",
            dsl.eq(
                _field(packet, "source_membership_digest", F + "digest_sha256"),
                dsl.digest(
                    F + "source_membership_digest_v1",
                    [_col("source", SOURCE, name) for name in COLUMNS[SOURCE]],
                ),
            ),
            "F_ADMISSION_SOURCE",
        ),
        ...
        dsl.assert_node(
            ...
            dsl.all_of(
                dsl.eq(
                    _col("key", KEY, "key_id"), _field(packet, "key_id", F + "key_id")
                ),
        ...
        dsl.select_node(
            ...
            relation=ADMISSION,
            columns=COLUMNS[ADMISSION],
            predicate=reuse_pred,
            cardinality="COMPLETE_SET",
            output_symbol="reuse_set",
            order_by=COORDS + ["source_position", "entry_kind"],
            set_read=True,
        ),
        _if(
            ...
            dsl.binary(
                "GT", _count("reuse_set", ADMISSION), dsl.const(PG + "bigint", 0)
            ),
            [
                _insert_reload(
                    f"{body_id}.insert_reuse",
                    ADMISSION,
                    reuse_bindings,
                    "reuse_conflict",
                    COORDS + ["source_position", "entry_kind"],
                ...
                _retry_or_return(
                    body_id, "reuse_conflict", "reuse_conflict", ADMISSION
                ),
            ],
            [
                _insert_reload(
                    f"{body_id}.insert_primary",
                    ADMISSION,
                    primary_bindings,
                    "inserted_primary",
                    COORDS + ["source_position", "entry_kind"],
                ...
                _retry_or_return(
                    body_id, "primary_insert", "inserted_primary", ADMISSION
                ),
            ],
        ),
    ]
    nodes = [
        ...
        *binding_nodes,
        dsl.let_node(
            f"{body_id}.attempted_digest",
            "attempted_digest",
            F + "digest_sha256",
            attempted,
        ),
        dsl.select_node(
            f"{body_id}.primary_set",
            relation=ADMISSION,
            columns=COLUMNS[ADMISSION],
            predicate=primary_pred,
            cardinality="COMPLETE_SET",
            output_symbol="primary_set",
            order_by=COORDS + ["source_position", "entry_kind"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.conflict_set",
            relation=ADMISSION,
            columns=COLUMNS[ADMISSION],
            predicate=conflict_pred,
            cardinality="COMPLETE_SET",
            output_symbol="conflict_set",
            order_by=COORDS + ["source_position", "entry_kind"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.receipt_set",
            relation=RECEIPT,
            columns=COLUMNS[RECEIPT],
            predicate=_predicate(RECEIPT, coordinate),
            cardinality="COMPLETE_SET",
            output_symbol="receipt_set",
            order_by=COORDS + ["source_position"],
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.retained_cardinality",
            dsl.all_of(
                dsl.binary(
                    "LTE", _count("primary_set", ADMISSION), dsl.const(PG + "bigint", 1)
                ),
                dsl.binary(
                    "LTE",
                ...
                dsl.binary(
                    "LTE", _count("receipt_set", RECEIPT), dsl.const(PG + "bigint", 1)
                ),
            ),
            "F_STATE",
        ),
        dsl.assert_node(
            f"{body_id}.receipt_has_primary",
            dsl.any_of(
        ...
        _if(
            ...
            [
                _if(
                    ...
                    dsl.eq(
                        _count("conflict_set", ADMISSION), dsl.const(PG + "bigint", 1)
                    ),
                    retained_conflict,
                    first_membership,
                )
    ...
    locals_.extend(
        [
            ("attempted_digest", F + "digest_sha256"),
            ("primary_set", ADMISSION + "[]"),
            ("conflict_set", ADMISSION + "[]"),
            ("receipt_set", RECEIPT + "[]"),
            ("retained_primary", ADMISSION),
            ("retained_conflict", ADMISSION),
            ("source", SOURCE),
            ("key", KEY),
            ("reuse_set", ADMISSION + "[]"),
            ("mismatch_conflict", ADMISSION),
            ("reuse_conflict", ADMISSION),
            ("inserted_primary", ADMISSION),
        ]
    )
    return dsl.body(
        body_id,
        ...
        _symbols(
            [
                ("generation_locator", GEN_LOC),
                ("source_position", PG + "bigint"),
                ("proofread_packet", PACKET),
            ],
            locals_,
        ),
        nodes,
    ...
    *,
    ...
    return _composite(
        ...
        [
            ...
            (
                "checkpoint_state",
                _col(checkpoint_symbol, CHECKPOINT, "checkpoint_state"),
            ),
...
def build_coordinator_body() -> dict[str, Any]:
    ...
    scope = {
        key: loc[key] for key in ("practice_id", "source_contract_id", "stream_id")
    }
    coordinate = {**loc, "source_position": position}
    primary_pred = dsl.all_of(
        _predicate(ADMISSION, coordinate),
    ...
    current_anchor_pred = _predicate(
        ...
        {
            **loc,
            "lifecycle_revision": _col("checkpoint", CHECKPOINT, "lifecycle_revision"),
        },
    )
    next_revision = dsl.add(
        _col("checkpoint", CHECKPOINT, "lifecycle_revision"),
        dsl.const(PG + "bigint", 1),
        PG + "bigint",
    )
    rebase_integrity = dsl.digest(
        F + "checkpoint_rebase_digest_v1", [*loc.values(), position, next_revision]
    )
    applied_integrity = dsl.digest(
        F + "checkpoint_apply_digest_v1",
        [
            *loc.values(),
            position,
            _col("primary", ADMISSION, "admission_digest"),
            next_revision,
        ],
    )
        ...
        [
            {
                ...
                "then": dsl.const(F + "checkpoint_disposition", "HOLD_REBASE"),
            }
        ],
    ...
    terminal_branch = [
        ...
        dsl.let_node(
            f"{body_id}.terminal_result",
            "terminal_result",
            result_type,
            terminal_result,
        ),
        _retry_or_return(
            body_id, "terminal", "terminal_result", result_type, composite=True
        ),
    ]

    replay_result = _composite(
        result_type,
        [
            (
                "result_kind",
                dsl.const(F + "durability_transition_result_kind", "RECEIPT_REPLAYED"),
            ),
    ...
    receipt_replay = [
        dsl.lock_node(
            f"{body_id}.lock_receipt",
            relation=RECEIPT,
            predicate=receipt_pred,
            key_columns=COORDS + ["source_position"],
            mode="FOR_UPDATE",
            order=4,
            output_symbol="stored_receipt",
            columns=COLUMNS[RECEIPT],
        ),
        dsl.lock_node(
            f"{body_id}.lock_replay_primary",
            relation=ADMISSION,
            predicate=primary_pred,
            key_columns=COORDS + ["source_position", "entry_kind"],
            mode="FOR_SHARE",
            order=5,
            output_symbol="replay_primary",
            columns=COLUMNS[ADMISSION],
        ),
        dsl.let_node(
        ...
        dsl.let_node(
            f"{body_id}.replay_result", "replay_result", result_type, replay_result
        ),
        _retry_or_return(
            body_id, "receipt_replay", "replay_result", result_type, composite=True
        ),
    ]
            ...
            *anchor_lock,
            ...
            dsl.let_node(
                ...
                _transition_result(
                    ...
                    source_position=position,
                    decision=dsl.const(F + "observation_decision", "REBASE_REQUIRED"),
                    reason=reason,
                    disposition=dsl.const(F + "checkpoint_disposition", "HOLD_REBASE"),
                    revision=_col(checkpoint_symbol, CHECKPOINT, "lifecycle_revision"),
                    evidence=_col(
                        checkpoint_symbol,
                        CHECKPOINT,
                        "checkpoint_integrity_digest",
    ...
    receipt_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        (
            "checkpoint_disposition",
            _col("primary", ADMISSION, "checkpoint_disposition"),
        ),
        ("lifecycle_revision", next_revision),
    ...
    applied_lifecycle_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        ("key_interval_start", dsl.const(PG + "bigint", None)),
        ("key_interval_end", dsl.const(PG + "bigint", None)),
        ("prior_lifecycle_digest", _col("anchor", ANCHOR, "anchor_digest")),
        ("lifecycle_digest", applied_integrity),
        ("created_at", dsl.transaction_timestamp()),
    ]
    applied_audit_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        ("reason_code", _col("primary", ADMISSION, "reason_code")),
        ("affected_frame_mask", _col("primary", ADMISSION, "affected_frame_mask")),
        ("prior_audit_digest", _col("checkpoint", CHECKPOINT, "audit_head_digest")),
        ("audit_head_digest", applied_integrity),
        ("created_at", dsl.transaction_timestamp()),
    ]
                ...
                bindings=[
                    ...
                    (
                        ...
                        dsl.case(
                            ...
                            dsl.const(F + "obligation_count_bucket", "FIVE_PLUS"),
                        ),
                    ),
                    ("updated_at", dsl.transaction_timestamp()),
                ],
                ...
                predicate=_predicate(
                    FRAME,
                    {
                        **loc,
                        ...
                        "lifecycle_state": dsl.const(F + "frame_lifecycle", "CURRENT"),
                    },
                ),
                key_columns=COORDS + ["frame_generation_id"],
                bindings=[
                    ...
                    [
                        _if_rejoin(
                            ...
                            [
                                dsl.assert_node(
                                    f"{body_id}.mask_none",
                                    dsl.eq(mask, dsl.const(F + "frame_mask", 0)),
                                    "F_STATE",
                                )
                            ],
                        )
    ...
    apply_branch = [
        dsl.lock_node(
            f"{body_id}.lock_diary_frame",
            relation=FRAME,
            predicate=dsl.all_of(
                current_frame_pred,
                dsl.eq(
                    _src(FRAME, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_DIARY_PROJECTION"),
                ),
            ),
            key_columns=COORDS + ["frame_generation_id"],
            mode="FOR_UPDATE",
            order=6,
            output_symbol="diary_frame",
            columns=COLUMNS[FRAME],
        ),
        dsl.lock_node(
            f"{body_id}.lock_waiting_frame",
            relation=FRAME,
            predicate=dsl.all_of(
                current_frame_pred,
                dsl.eq(
                    _src(FRAME, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_WAITING_ROOM_PROJECTION"),
                ),
            ),
            key_columns=COORDS + ["frame_generation_id"],
            mode="FOR_UPDATE",
            order=7,
            output_symbol="waiting_frame",
            columns=COLUMNS[FRAME],
        ),
        dsl.lock_node(
            f"{body_id}.lock_diary_watermark",
            relation=WATERMARK,
            predicate=dsl.all_of(
                watermark_pred,
                dsl.eq(
                    _src(WATERMARK, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_DIARY_PROJECTION"),
                ),
            ),
            key_columns=COORDS + ["frame_type"],
            mode="FOR_UPDATE",
            order=8,
            output_symbol="diary_watermark",
            columns=COLUMNS[WATERMARK],
        ),
        dsl.lock_node(
            f"{body_id}.lock_waiting_watermark",
            relation=WATERMARK,
            predicate=dsl.all_of(
                watermark_pred,
                dsl.eq(
            ...
            mode="FOR_UPDATE",
            order=9,
            output_symbol="waiting_watermark",
            columns=COLUMNS[WATERMARK],
        ),
        dsl.insert_node(
            f"{body_id}.lifecycle",
            relation=LIFECYCLE,
            bindings=applied_lifecycle_bindings,
            output_symbol="applied_lifecycle",
            returning_columns=COLUMNS[LIFECYCLE],
        ),
        dsl.insert_node(
            f"{body_id}.audit",
            relation=AUDIT,
            bindings=applied_audit_bindings,
            output_symbol="applied_audit",
            returning_columns=COLUMNS[AUDIT],
        ),
        _insert_reload(
            f"{body_id}.receipt_insert",
            RECEIPT,
            receipt_bindings,
            "inserted_receipt",
            COORDS + ["source_position"],
        ),
        *dependent_branch,
        dsl.update_node(
            f"{body_id}.checkpoint_update",
            relation=CHECKPOINT,
            predicate=_predicate(CHECKPOINT, loc),
            key_columns=COORDS,
            bindings=[
                ("checkpoint_state", dsl.const(F + "checkpoint_state", "ACTIVE")),
                ("last_contiguous_position", position),
                (
                    "last_observation_digest",
                    _col("primary", ADMISSION, "observation_digest"),
                ),
                ("lifecycle_revision", next_revision),
                ("audit_head_digest", applied_integrity),
                ("checkpoint_integrity_digest", applied_integrity),
                ("updated_at", dsl.transaction_timestamp()),
            ],
            output_symbol="applied_checkpoint",
            returning_columns=COLUMNS[CHECKPOINT],
        ),
        dsl.let_node(
            f"{body_id}.applied_result",
            "applied_result",
            result_type,
            _transition_result(
                kind="RECEIPT_APPLIED",
                checkpoint_symbol="applied_checkpoint",
                source_position=position,
                decision=_col("inserted_receipt", RECEIPT, "decision"),
                reason=_col("inserted_receipt", RECEIPT, "reason_code"),
                disposition=_col("inserted_receipt", RECEIPT, "checkpoint_disposition"),
                revision=_col("inserted_receipt", RECEIPT, "lifecycle_revision"),
                evidence=_col("inserted_receipt", RECEIPT, "receipt_digest"),
            ),
        ),
        _retry_or_return(
            body_id, "applied", "applied_result", result_type, composite=True
        ),
    ]
    ...
    dependent_state_exact = dsl.all_of(
        ...
        dsl.eq(_count("watermark_set", WATERMARK), dsl.const(PG + "bigint", 2)),
        dsl.binary(
            "LTE",
                                ...
                                _if(
                                    ...
                                    [
                                        _if(
                                            ...
                                            dsl.eq(
                                                _count("current_anchor_set", ANCHOR),
                                                dsl.const(PG + "bigint", 1),
                                            ),
                                            [
                                                _if(
    ...
    primary_branch = [
        ...
        dsl.select_node(
            f"{body_id}.current_frame_set",
            relation=FRAME,
            columns=COLUMNS[FRAME],
            predicate=current_frame_pred,
            cardinality="COMPLETE_SET",
            output_symbol="current_frame_set",
            order_by=COORDS + ["frame_type", "frame_generation_id"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.watermark_set",
            relation=WATERMARK,
            columns=COLUMNS[WATERMARK],
            predicate=watermark_pred,
            cardinality="COMPLETE_SET",
            output_symbol="watermark_set",
            order_by=COORDS + ["frame_type"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.pending_obligation_set",
            relation=OBLIGATION,
            columns=COLUMNS[OBLIGATION],
            predicate=pending_obligation_pred,
            cardinality="COMPLETE_SET",
            output_symbol="pending_obligation_set",
            order_by=COORDS + ["frame_generation_id"],
            set_read=True,
        ),
        *apply_state_branch,
    ...
    primary_route = [
        dsl.select_node(
            f"{body_id}.current_anchor_set",
            relation=ANCHOR,
            columns=COLUMNS[ANCHOR],
            predicate=current_anchor_pred,
            cardinality="COMPLETE_SET",
            output_symbol="current_anchor_set",
            order_by=COORDS + ["lifecycle_revision"],
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.primary_anchor_cardinality",
            dsl.eq(_count("current_anchor_set", ANCHOR), dsl.const(PG + "bigint", 1)),
            "F_STATE",
        ),
        dsl.lock_node(
        ...
        dsl.lock_node(
            f"{body_id}.lock_primary",
            relation=ADMISSION,
            predicate=primary_pred,
            key_columns=COORDS + ["source_position", "entry_kind"],
            mode="FOR_UPDATE",
            order=5,
            output_symbol="primary",
            columns=COLUMNS[ADMISSION],
        ),
        dsl.select_node(
            f"{body_id}.source_position_set",
            relation=SOURCE,
            columns=COLUMNS[SOURCE],
            predicate=source_position_pred,
            cardinality="COMPLETE_SET",
            output_symbol="source_position_set",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        [
            {
                ...
                "then": dsl.const(F + "observation_reason", "SAME_POSITION_MISMATCH"),
            }
        ],
    ...
    conflict_branch = [
        ...
        *rebase_branch("conflict", conflict_reason, anchor_symbol="conflict_anchor"),
    ]
    admission_state = _if(
        ...
        [
            _if(
                ...
                [
                    *rebase_branch(
                        "missing_admission",
                        dsl.const(F + "observation_reason", "MISSING_ADMISSION"),
                    )
                ],
            )
        ],
    ...
    nodes = [
        _isolation(body_id, "SERIALIZABLE"),
        *binding_nodes,
        dsl.lock_node(
            f"{body_id}.lock_barrier",
            relation=BARRIER,
            predicate=_predicate(BARRIER, scope),
            key_columns=["practice_id", "source_contract_id", "stream_id"],
            mode="FOR_UPDATE",
            order=1,
            output_symbol="barrier",
            columns=COLUMNS[BARRIER],
        ),
        dsl.lock_node(
            f"{body_id}.lock_generation",
            relation=GENERATION,
            predicate=_predicate(GENERATION, loc),
            key_columns=COORDS,
            mode="FOR_UPDATE",
            order=2,
            output_symbol="generation",
            columns=COLUMNS[GENERATION],
        ),
        dsl.lock_node(
            f"{body_id}.lock_checkpoint",
            relation=CHECKPOINT,
            predicate=_predicate(CHECKPOINT, loc),
            key_columns=COORDS,
            mode="FOR_UPDATE",
            order=3,
            output_symbol="checkpoint",
    ...
    locals_ = [(row["id"], row["type"]) for row in binding_symbols] + [
        ...
        ("terminal_result", result_type),
        ("receipt_set", RECEIPT + "[]"),
        ("stored_receipt", RECEIPT),
        ("replay_primary", ADMISSION),
        ("rederived_receipt_digest", F + "digest_sha256"),
        ("replay_result", result_type),
        ("primary_set", ADMISSION + "[]"),
        ("conflict_set", ADMISSION + "[]"),
        ("conflict_anchor", ANCHOR),
        ("conflict", ADMISSION),
        ("source_position_set", SOURCE + "[]"),
        ("source_at_position", SOURCE),
        ("key_membership_set", KEY + "[]"),
        ("current_anchor_set", ANCHOR + "[]"),
        ("current_frame_set", FRAME + "[]"),
        ("watermark_set", WATERMARK + "[]"),
        ("pending_obligation_set", OBLIGATION + "[]"),
        ("primary", ADMISSION),
        ("diary_frame", FRAME),
        ("waiting_frame", FRAME),
        ("diary_watermark", WATERMARK),
        ("waiting_watermark", WATERMARK),
        *rebase_symbols,
        *dependent_symbols,
        ("applied_lifecycle", LIFECYCLE),
        ("applied_audit", AUDIT),
        ("inserted_receipt", RECEIPT),
        ("applied_checkpoint", CHECKPOINT),
        ("applied_result", result_type),
    ]
    return dsl.body(
        body_id,
        "ENTRY_POINT",
        body_id,
        _symbols([("admission_locator", ADMISSION_LOC)], locals_),
        nodes,
    )
...
def build_registration_body() -> dict[str, Any]:
    ...
    initial_attestation = _field(
        interval, "availability_attestation_digest", F + "digest_sha256"
    )
    ...
    next_position = dsl.add(
        _col("head", HEAD, "last_position"), dsl.const(PG + "bigint", 1), PG + "bigint"
    )
    baseline_digest = dsl.digest(
        F + "registration_baseline_digest_v1",
        [
            *loc.values(),
            *controlling.values(),
    ...
    replay_branch = [
        dsl.select_node(
            ...
            cardinality="EXACTLY_ONE",
            output_symbol="existing_generation",
            order_by=COORDS,
        ),
        dsl.select_node(
            f"{body_id}.replay_checkpoint",
            relation=CHECKPOINT,
            columns=COLUMNS[CHECKPOINT],
            predicate=_predicate(CHECKPOINT, loc),
            cardinality="EXACTLY_ONE",
            output_symbol="replay_checkpoint",
            order_by=COORDS,
        ),
        dsl.select_node(
            f"{body_id}.replay_frame_set",
            relation=FRAME,
            columns=COLUMNS[FRAME],
            predicate=replay_frame_pred,
            cardinality="COMPLETE_SET",
            output_symbol="replay_frame_set",
            order_by=COORDS + ["frame_type", "frame_generation_id"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.replay_diary_frame",
            relation=FRAME,
            columns=COLUMNS[FRAME],
            predicate=dsl.all_of(
                replay_frame_pred,
            ...
            cardinality="EXACTLY_ONE",
            output_symbol="replay_diary_frame",
            order_by=COORDS + ["frame_type", "frame_generation_id"],
        ),
        dsl.select_node(
            f"{body_id}.replay_waiting_frame",
            relation=FRAME,
            columns=COLUMNS[FRAME],
            predicate=dsl.all_of(
                replay_frame_pred,
            ...
            cardinality="EXACTLY_ONE",
            output_symbol="replay_waiting_frame",
            order_by=COORDS + ["frame_type", "frame_generation_id"],
        ),
        dsl.select_node(
            f"{body_id}.replay_watermark_set",
            relation=WATERMARK,
            columns=COLUMNS[WATERMARK],
            predicate=replay_watermark_pred,
            cardinality="COMPLETE_SET",
            output_symbol="replay_watermark_set",
            order_by=COORDS + ["frame_type"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.replay_diary_watermark",
            relation=WATERMARK,
            columns=COLUMNS[WATERMARK],
            predicate=dsl.all_of(
                replay_watermark_pred,
                dsl.eq(
                    _src(WATERMARK, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_DIARY_PROJECTION"),
                ),
            ...
            cardinality="EXACTLY_ONE",
            output_symbol="replay_diary_watermark",
            order_by=COORDS + ["frame_type"],
        ),
        dsl.select_node(
            f"{body_id}.replay_waiting_watermark",
            relation=WATERMARK,
            columns=COLUMNS[WATERMARK],
            predicate=dsl.all_of(
                replay_watermark_pred,
            ...
            cardinality="EXACTLY_ONE",
            output_symbol="replay_waiting_watermark",
            order_by=COORDS + ["frame_type"],
        ),
        dsl.select_node(
            f"{body_id}.replay_initial_key",
            relation=KEY,
            columns=COLUMNS[KEY],
            predicate=replay_key_pred,
            cardinality="EXACTLY_ONE",
            output_symbol="replay_initial_key",
            order_by=COORDS + ["interval_start"],
        ),
        dsl.select_node(
            f"{body_id}.replay_baseline_anchor",
            relation=ANCHOR,
            columns=COLUMNS[ANCHOR],
            predicate=replay_anchor_pred,
            cardinality="EXACTLY_ONE",
            output_symbol="replay_baseline_anchor",
            order_by=COORDS + ["lifecycle_revision"],
        ),
        dsl.assert_node(
            ...
            dsl.all_of(
                dsl.eq(
                    _col("existing_generation", GENERATION, "lifecycle_state"),
                    dsl.const(F + "generation_state", "ACTIVE"),
                ),
                dsl.unary(
                    "IS_NULL", _col("existing_generation", GENERATION, "consumed_at")
                ),
                dsl.unary(
                    "IS_NULL",
                    _col("existing_generation", GENERATION, "terminal_reason"),
                ),
                *(
                ...
                dsl.eq(
                    _col("replay_checkpoint", CHECKPOINT, "checkpoint_state"),
                    dsl.const(F + "checkpoint_state", "ACTIVE"),
                ),
                dsl.eq(
                    _col("replay_checkpoint", CHECKPOINT, "last_contiguous_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _col("replay_checkpoint", CHECKPOINT, "last_observation_digest"),
                    expected_last_observation,
                ),
                dsl.eq(
                    _col("replay_checkpoint", CHECKPOINT, "lifecycle_revision"),
                    dsl.const(PG + "bigint", 0),
                ),
                dsl.eq(
                    _col("replay_checkpoint", CHECKPOINT, "audit_head_digest"),
                    baseline_digest,
                ),
                dsl.eq(
                    _col(
                        "replay_checkpoint", CHECKPOINT, "checkpoint_integrity_digest"
                    ),
                    baseline_digest,
                ),
                ...
                dsl.binary(
                    "NE",
                    _col("replay_diary_frame", FRAME, "frame_generation_id"),
                    _col("replay_waiting_frame", FRAME, "frame_generation_id"),
                ),
                dsl.eq(
                    _col("replay_diary_frame", FRAME, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_DIARY_PROJECTION"),
                ),
                dsl.eq(
                    _col("replay_waiting_frame", FRAME, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_WAITING_ROOM_PROJECTION"),
                ),
                dsl.eq(
                    _col("replay_diary_frame", FRAME, "assembled_through_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _col("replay_waiting_frame", FRAME, "assembled_through_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _count("replay_watermark_set", WATERMARK),
                    dsl.const(PG + "bigint", 2),
                ),
                dsl.eq(
                    _col("replay_diary_watermark", WATERMARK, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_DIARY_PROJECTION"),
                ),
                dsl.eq(
                    _col("replay_waiting_watermark", WATERMARK, "frame_type"),
                    dsl.const(F + "frame_type", "CURRENT_WAITING_ROOM_PROJECTION"),
                ),
                dsl.eq(
                    _col("replay_diary_watermark", WATERMARK, "watermark_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _col("replay_waiting_watermark", WATERMARK, "watermark_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _col("replay_initial_key", KEY, "interval_start"), initial_start
                ),
                dsl.eq(_col("replay_initial_key", KEY, "interval_end"), initial_end),
                dsl.eq(_col("replay_initial_key", KEY, "key_id"), initial_key_id),
                dsl.eq(
                    _col("replay_initial_key", KEY, "availability_attestation_digest"),
                    initial_attestation,
                ),
                dsl.eq(
                    _col("replay_baseline_anchor", ANCHOR, "lifecycle_revision"),
                    dsl.const(PG + "bigint", 0),
                ),
                dsl.eq(
                    _col("replay_baseline_anchor", ANCHOR, "checkpoint_state"),
                    dsl.const(F + "checkpoint_state", "ACTIVE"),
                ),
                dsl.eq(
                    _col("replay_baseline_anchor", ANCHOR, "last_contiguous_position"),
                    _col("head", HEAD, "last_position"),
                ),
                dsl.eq(
                    _col("replay_baseline_anchor", ANCHOR, "last_observation_digest"),
                    expected_last_observation,
                ),
                *(
                    dsl.eq(_col("replay_baseline_anchor", ANCHOR, name), value)
                    for name, value in controlling.items()
                ),
                dsl.eq(
                    _col(
                        "replay_baseline_anchor", ANCHOR, "checkpoint_integrity_digest"
                    ),
                    baseline_digest,
                ),
                dsl.eq(
                    _col("replay_baseline_anchor", ANCHOR, "anchor_digest"),
                    baseline_digest,
                ),
        ...
        _retry_or_return(
            body_id, "registration_replay", "existing_generation", GENERATION
        ),
    ]
    ...
    checkpoint_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        ("audit_head_digest", baseline_digest),
        ("checkpoint_integrity_digest", baseline_digest),
        ("updated_at", dsl.transaction_timestamp()),
    ]

    def frame_bindings(
        frame_type: str, frame_id: dict[str, Any]
    ) -> list[tuple[str, dict[str, Any]]]:
        return [(name, loc[name]) for name in COORDS] + [
            ("frame_generation_id", frame_id),
            ("frame_type", dsl.const(F + "frame_type", frame_type)),
            ("assembled_through_position", _col("head", HEAD, "last_position")),
            ("lifecycle_state", dsl.const(F + "frame_lifecycle", "CURRENT")),
            ("created_at", dsl.transaction_timestamp()),
            ("retired_at", dsl.const(PG + "timestamptz", None)),
        ]

    def watermark_bindings(frame_type: str) -> list[tuple[str, dict[str, Any]]]:
        return [(name, loc[name]) for name in COORDS] + [
            ("frame_type", dsl.const(F + "frame_type", frame_type)),
            ("watermark_position", _col("head", HEAD, "last_position")),
            ("updated_at", dsl.transaction_timestamp()),
        ]

    key_bindings = [(name, loc[name]) for name in COORDS] + [
        ("interval_start", initial_start),
        ("interval_end", initial_end),
        ("key_id", initial_key_id),
        ("availability_attestation_digest", initial_attestation),
        ("created_at", dsl.transaction_timestamp()),
    ]
    anchor_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        *controlling.items(),
        ("checkpoint_integrity_digest", baseline_digest),
        ("anchor_digest", baseline_digest),
        ("created_at", dsl.transaction_timestamp()),
    ]
    new_branch = [
        dsl.assert_node(
            f"{body_id}.initial_key_start",
            dsl.eq(initial_start, next_position),
            "F_KEY_PARTITION",
        ),
        dsl.assert_node(
            f"{body_id}.initial_key_order",
            dsl.binary("GTE", initial_end, initial_start),
            "F_KEY_PARTITION",
        ),
        _insert_reload(
            f"{body_id}.generation_insert",
            GENERATION,
            generation_bindings,
            "inserted_generation",
            COORDS,
        ...
        dsl.let_node(
            f"{body_id}.diary_frame_id", "diary_frame_id", PG + "uuid", dsl.uuid_v4()
        ),
        dsl.let_node(
            f"{body_id}.waiting_frame_id",
            "waiting_frame_id",
            PG + "uuid",
            dsl.uuid_v4(),
        ),
        _insert_reload(
            f"{body_id}.diary_frame",
            FRAME,
            frame_bindings(
                "CURRENT_DIARY_PROJECTION", dsl.local_ref("diary_frame_id", PG + "uuid")
            ),
            "diary_frame",
            COORDS + ["frame_generation_id"],
        ),
        _insert_reload(
            f"{body_id}.waiting_frame",
            FRAME,
            frame_bindings(
                "CURRENT_WAITING_ROOM_PROJECTION",
                dsl.local_ref("waiting_frame_id", PG + "uuid"),
            ),
            "waiting_frame",
            COORDS + ["frame_generation_id"],
        ),
        _insert_reload(
            f"{body_id}.diary_watermark",
            WATERMARK,
            watermark_bindings("CURRENT_DIARY_PROJECTION"),
            "diary_watermark",
            COORDS + ["frame_type"],
        ),
        _insert_reload(
            f"{body_id}.waiting_watermark",
            WATERMARK,
            watermark_bindings("CURRENT_WAITING_ROOM_PROJECTION"),
            "waiting_watermark",
            COORDS + ["frame_type"],
        ),
        _insert_reload(
            f"{body_id}.initial_key",
            KEY,
            key_bindings,
            "initial_key",
            COORDS + ["interval_start"],
        ),
        _insert_reload(
            f"{body_id}.baseline_anchor",
            ANCHOR,
            anchor_bindings,
            "baseline_anchor",
            COORDS + ["lifecycle_revision"],
        ),
        dsl.update_node(
            f"{body_id}.barrier_update",
            relation=BARRIER,
            predicate=_predicate(BARRIER, scope),
            key_columns=["practice_id", "source_contract_id", "stream_id"],
            bindings=[
                (
                    "barrier_revision",
                    dsl.add(
                        _col("barrier", BARRIER, "barrier_revision"),
                        dsl.const(PG + "bigint", 1),
                        PG + "bigint",
                    ),
                ),
            ...
            output_symbol="updated_barrier",
            returning_columns=COLUMNS[BARRIER],
        ),
        _retry_or_return(
            body_id, "registration_insert", "inserted_generation", GENERATION
        ),
    ]
    nodes = [
        _isolation(body_id, "SERIALIZABLE"),
        *binding_nodes,
        ...
        dsl.select_node(
            ...
            predicate=_predicate(HEAD, head_coordinate),
            cardinality="COMPLETE_SET",
            output_symbol="head_set",
            order_by=["practice_id", "source_contract_id", "stream_id"],
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.head_unambiguous",
            dsl.binary("LTE", _count("head_set", HEAD), dsl.const(PG + "bigint", 1)),
            "F_REGISTRATION",
        ),
        _if_rejoin(
        ...
        dsl.select_node(
            f"{body_id}.existing_set",
            relation=GENERATION,
            columns=COLUMNS[GENERATION],
            predicate=existing_pred,
            cardinality="COMPLETE_SET",
            output_symbol="existing_set",
            order_by=COORDS,
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.registration_unambiguous",
            dsl.binary(
                "LTE", _count("existing_set", GENERATION), dsl.const(PG + "bigint", 1)
            ),
            "F_REGISTRATION",
        ),
        _if(
            f"{body_id}.registered",
            dsl.eq(_count("existing_set", GENERATION), dsl.const(PG + "bigint", 1)),
            replay_branch,
            new_branch,
        ),
    ]
    locals_ = [(row["id"], row["type"]) for row in binding_symbols] + [
        ...
        ("head_set", HEAD + "[]"),
        ("head", HEAD),
        ("existing_set", GENERATION + "[]"),
        ("existing_generation", GENERATION),
        ("replay_checkpoint", CHECKPOINT),
        ("replay_frame_set", FRAME + "[]"),
        ("replay_diary_frame", FRAME),
        ("replay_waiting_frame", FRAME),
        ("replay_watermark_set", WATERMARK + "[]"),
        ("replay_diary_watermark", WATERMARK),
        ("replay_waiting_watermark", WATERMARK),
        ("replay_initial_key", KEY),
        ("replay_baseline_anchor", ANCHOR),
        ("inserted_generation", GENERATION),
        ("inserted_checkpoint", CHECKPOINT),
        ("diary_frame_id", PG + "uuid"),
        ("waiting_frame_id", PG + "uuid"),
        ("diary_frame", FRAME),
        ("waiting_frame", FRAME),
        ("diary_watermark", WATERMARK),
        ("waiting_watermark", WATERMARK),
        ...
        ("updated_barrier", BARRIER),
    ]
    return dsl.body(
        body_id,
        "ENTRY_POINT",
        body_id,
        _symbols([("registration", REGISTRATION)], locals_),
        nodes,
    )
...
def build_anchor_body() -> dict[str, Any]:
    ...
    decision_branch = [
        dsl.assert_node(
            ...
            dsl.all_of(
                ...
                dsl.binary("GT", lifecycle_position, dsl.const(PG + "bigint", 0)),
                dsl.unary(
                    "IS_NULL",
                    _col("anchor_lifecycle", LIFECYCLE, "key_interval_start"),
                ),
    ...
    lifecycle_key_predicate = _predicate(
        ...
        {
            **loc,
            "interval_start": _col("anchor_lifecycle", LIFECYCLE, "key_interval_start"),
        },
    )
        ...
        dsl.assert_node(
            ...
            dsl.all_of(
                *key_controlling_exact,
                ...
                dsl.eq(
                    _col("checkpoint", CHECKPOINT, "last_contiguous_position"),
                    _col("key_previous_anchor", ANCHOR, "last_contiguous_position"),
                ),
                dsl.eq(
                    _col("checkpoint", CHECKPOINT, "last_observation_digest"),
                    _col("key_previous_anchor", ANCHOR, "last_observation_digest"),
                ),
                dsl.eq(
                    _col("generation", GENERATION, "lifecycle_state"),
                    dsl.const(F + "generation_state", "ACTIVE"),
    ...
    trusted_integrity = dsl.local_ref("trusted_integrity_digest", F + "digest_sha256")
    anchor_digest_expression = dsl.digest(
        F + "recovery_anchor_digest_v1",
        [
            *loc.values(),
            requested_revision,
            _col("checkpoint", CHECKPOINT, "checkpoint_state"),
    ...
    replay_branch = [
        dsl.select_node(
            ...
            relation=ANCHOR,
            columns=COLUMNS[ANCHOR],
            predicate=_predicate(ANCHOR, anchor_coord),
            cardinality="EXACTLY_ONE",
            output_symbol="stored_anchor",
            order_by=COORDS + ["lifecycle_revision"],
        ),
        dsl.assert_node(
    ...
    nodes = [
        ...
        *binding_nodes,
        ...
        dsl.assert_node(
            f"{body_id}.revision_nonzero",
            dsl.binary("GT", requested_revision, dsl.const(PG + "bigint", 0)),
            "F_ANCHOR",
        ),
        dsl.assert_node(
            f"{body_id}.revision_current",
            dsl.eq(
                requested_revision, _col("checkpoint", CHECKPOINT, "lifecycle_revision")
            ),
            "F_ANCHOR",
        ),
        dsl.assert_node(
            f"{body_id}.common_state_exact", common_state_exact, "F_ANCHOR"
        ),
        dsl.select_node(
            f"{body_id}.baseline_anchor_set",
            relation=ANCHOR,
            columns=COLUMNS[ANCHOR],
            predicate=baseline_anchor_predicate,
            cardinality="COMPLETE_SET",
            output_symbol="baseline_anchor_set",
            order_by=COORDS + ["lifecycle_revision"],
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.baseline_anchor_cardinality",
            dsl.eq(_count("baseline_anchor_set", ANCHOR), dsl.const(PG + "bigint", 1)),
            "F_ANCHOR",
        ),
        dsl.select_node(
            f"{body_id}.baseline_anchor",
        ...
        dsl.select_node(
            ...
            relation=LIFECYCLE,
            columns=COLUMNS[LIFECYCLE],
            predicate=_predicate(LIFECYCLE, anchor_coord),
            cardinality="EXACTLY_ONE",
            output_symbol="anchor_lifecycle",
            order_by=COORDS + ["lifecycle_revision"],
        ),
        dsl.assert_node(
            f"{body_id}.checkpoint_lifecycle_timestamp",
            dsl.eq(
                _col("checkpoint", CHECKPOINT, "updated_at"),
                _col("anchor_lifecycle", LIFECYCLE, "created_at"),
            ),
            "F_ANCHOR",
        ),
        proof_branch,
        dsl.let_node(
            f"{body_id}.derive_anchor_digest",
            "derived_anchor_digest",
            F + "digest_sha256",
            anchor_digest_expression,
        ),
        dsl.select_node(
            f"{body_id}.anchor_set",
            relation=ANCHOR,
            columns=COLUMNS[ANCHOR],
            predicate=_predicate(ANCHOR, anchor_coord),
            cardinality="COMPLETE_SET",
            output_symbol="anchor_set",
            order_by=COORDS + ["lifecycle_revision"],
            set_read=True,
        ),
        dsl.assert_node(
            f"{body_id}.anchor_cardinality",
            dsl.binary(
                "LTE", _count("anchor_set", ANCHOR), dsl.const(PG + "bigint", 1)
            ),
            "F_ANCHOR",
        ),
        _if(
            f"{body_id}.exists",
            dsl.eq(_count("anchor_set", ANCHOR), dsl.const(PG + "bigint", 1)),
            replay_branch,
            insert_branch,
        ),
    ]
    locals_ = [(row["id"], row["type"]) for row in binding_symbols] + [
        ...
        ("generation", GENERATION),
        ("checkpoint", CHECKPOINT),
        ("baseline_anchor_set", ANCHOR + "[]"),
        ("baseline_anchor", ANCHOR),
        ("anchor_lifecycle", LIFECYCLE),
        ("decision_audit", AUDIT),
        ("decision_receipt_set", RECEIPT + "[]"),
        ("decision_receipt", RECEIPT),
        ("anchor_primary_set", ADMISSION + "[]"),
        ("anchor_conflict_set", ADMISSION + "[]"),
        ("anchor_primary", ADMISSION),
        ("rederived_anchor_receipt_digest", F + "digest_sha256"),
        ("rederived_rebase_integrity", F + "digest_sha256"),
        ("rederived_apply_integrity", F + "digest_sha256"),
        ("key_audit_set", AUDIT + "[]"),
        ("key_receipt_set", RECEIPT + "[]"),
        ("decision_previous_anchor", ANCHOR),
        ("anchor_key", KEY),
        ("key_previous_anchor", ANCHOR),
        ("rederived_rotation_integrity", F + "digest_sha256"),
        ("trusted_integrity_digest", F + "digest_sha256"),
        ("derived_anchor_digest", F + "digest_sha256"),
        ("anchor_set", ANCHOR + "[]"),
        ("stored_anchor", ANCHOR),
        ("inserted_anchor", ANCHOR),
        *audit_chain_symbols,
    ]
    return dsl.body(
        body_id,
        ...
        body_id,
        _symbols(
            [("generation_locator", GEN_LOC), ("lifecycle_revision", PG + "bigint")],
            locals_,
        ),
        nodes,
    )
...
def build_rotation_body() -> dict[str, Any]:
    ...
    replay_branch = [
        dsl.select_node(
            f"{body_id}.stored_key",
            relation=KEY,
            columns=COLUMNS[KEY],
            predicate=existing_pred,
            cardinality="EXACTLY_ONE",
            output_symbol="stored_key",
            order_by=COORDS + ["interval_start"],
        ),
        dsl.assert_node(
            f"{body_id}.replay_exact",
            dsl.all_of(
                dsl.eq(_col("stored_key", KEY, "interval_end"), end),
                dsl.eq(_col("stored_key", KEY, "key_id"), key_id),
                dsl.eq(
                    _col("stored_key", KEY, "availability_attestation_digest"),
                    attestation,
                ),
            ),
            "F_KEY_PARTITION",
    ...
    rotation_digest = dsl.digest(
        ...
        [
            *loc.values(),
            ...
            key_id,
            attestation,
            _col("anchor", ANCHOR, "anchor_digest"),
            next_revision,
        ],
    )
    key_bindings = [(name, loc[name]) for name in COORDS] + [
        ...
        ("interval_end", end),
        ("key_id", key_id),
        ("availability_attestation_digest", attestation),
        ("created_at", dsl.transaction_timestamp()),
    ]
    lifecycle_bindings = [(name, loc[name]) for name in COORDS] + [
        ("lifecycle_revision", next_revision),
        ("entry_kind", dsl.const(F + "lifecycle_entry_kind", "KEY_ROTATION")),
        ("source_position", dsl.const(PG + "bigint", None)),
        ("key_interval_start", start),
        ("key_interval_end", end),
        ("prior_lifecycle_digest", _col("anchor", ANCHOR, "anchor_digest")),
        ("lifecycle_digest", rotation_digest),
        ("created_at", dsl.transaction_timestamp()),
    ]
    anchor_fence_exact = dsl.all_of(
        ...
        dsl.eq(
            _col("anchor", ANCHOR, "last_observation_digest"),
            _col("checkpoint", CHECKPOINT, "last_observation_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "checkpoint_integrity_digest"),
        ...
        dsl.eq(
            _col("anchor", ANCHOR, "policy_digest"),
            _col("generation", GENERATION, "policy_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "principal_digest"),
            _col("generation", GENERATION, "principal_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "binding_digest"),
            _col("generation", GENERATION, "binding_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "source_digest"),
            _col("generation", GENERATION, "source_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "registry_digest"),
            _col("generation", GENERATION, "registry_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "impact_digest"),
            _col("generation", GENERATION, "impact_digest"),
        ),
        dsl.eq(
            _col("anchor", ANCHOR, "key_schedule_digest"),
            _col("generation", GENERATION, "key_schedule_digest"),
        ),
    )
    new_branch = [
        dsl.lock_node(
            f"{body_id}.lock_anchor",
            relation=ANCHOR,
            predicate=_predicate(
                ANCHOR,
                {
        ...
        dsl.assert_node(
            f"{body_id}.anchor_fence_exact", anchor_fence_exact, "F_ANCHOR"
        ),
        dsl.lock_node(
            f"{body_id}.lock_prior_key",
            relation=KEY,
            predicate=dsl.all_of(
                _predicate(KEY, loc),
                dsl.eq(
                    dsl.add(
                        _src(KEY, "interval_end"),
                        dsl.const(PG + "bigint", 1),
                        PG + "bigint",
                    ),
                    start,
            ...
            key_columns=COORDS + ["interval_start"],
            mode="FOR_UPDATE",
            order=5,
            output_symbol="prior_key",
            columns=COLUMNS[KEY],
        ),
        dsl.assert_node(
            f"{body_id}.future_fence",
            dsl.binary(
                "GT", start, _col("checkpoint", CHECKPOINT, "last_contiguous_position")
            ),
            "F_KEY_PARTITION",
        ),
        dsl.assert_node(
            f"{body_id}.interval_order", dsl.binary("GT", end, start), "F_KEY_PARTITION"
        ),
        _insert_reload(
            f"{body_id}.key_insert",
            KEY,
            key_bindings,
            "inserted_key",
            COORDS + ["interval_start"],
        ...
        dsl.update_node(
            f"{body_id}.generation_update",
            relation=GENERATION,
            predicate=_predicate(GENERATION, loc),
            key_columns=COORDS,
            bindings=[("key_schedule_digest", rotation_digest)],
            output_symbol="rotated_generation",
            returning_columns=COLUMNS[GENERATION],
        ),
        dsl.update_node(
            f"{body_id}.checkpoint_update",
            ...
            bindings=[
                ("lifecycle_revision", next_revision),
                ("checkpoint_integrity_digest", rotation_digest),
                ("updated_at", dsl.transaction_timestamp()),
            ],
            output_symbol="rotated_checkpoint",
            returning_columns=COLUMNS[CHECKPOINT],
        ),
    ...
    nodes = [
        ...
        *binding_nodes,
        ...
        dsl.select_node(
            ...
            predicate=existing_pred,
            cardinality="COMPLETE_SET",
            output_symbol="existing_key_set",
            order_by=COORDS + ["interval_start"],
            set_read=True,
        ),
        _if(
            f"{body_id}.replay",
            dsl.eq(_count("existing_key_set", KEY), dsl.const(PG + "bigint", 1)),
            replay_branch,
            new_branch,
        ),
    ]
    locals_ = [(row["id"], row["type"]) for row in binding_symbols] + [
        ...
        ("generation", GENERATION),
        ("checkpoint", CHECKPOINT),
        ("existing_key_set", KEY + "[]"),
        ("stored_key", KEY),
        ("anchor", ANCHOR),
        ("prior_key", KEY),
        ("inserted_key", KEY),
        ("rotation_lifecycle", LIFECYCLE),
        ("rotated_generation", GENERATION),
        ("rotated_checkpoint", CHECKPOINT),
    ]
    return dsl.body(
        body_id,
        "ENTRY_POINT",
        body_id,
        _symbols(
            [("generation_locator", GEN_LOC), ("future_interval", KEY_INPUT)], locals_
        ),
        nodes,
    )
...
def build_consumption_body() -> dict[str, Any]:
    ...
    replay_branch = [
        dsl.assert_node(
            f"{body_id}.reason_exact",
            dsl.eq(_col("generation", GENERATION, "terminal_reason"), closed_reason),
            "F_TERMINAL_REASON",
        ),
        _retry_or_return(body_id, "terminal_replay", "generation", GENERATION),
    ]
    target_state = dsl.case(
        F + "generation_state",
        [
            {
                "when": dsl.eq(
                    closed_reason,
                    dsl.const(F + "generation_terminal_reason", "REVOKED"),
                ),
                "then": dsl.const(F + "generation_state", "REVOKED"),
            }
        ],
        dsl.const(F + "generation_state", "CONSUMED"),
    )
    consumed_at = dsl.case(
        PG + "timestamptz",
        [
            {
                "when": dsl.eq(
                ...
                "then": dsl.const(PG + "timestamptz", None),
            }
        ],
    ...
    consume_branch = [
        dsl.update_node(
            ...
            bindings=[
                ...
                ("terminal_reason", closed_reason),
                ("consumed_at", consumed_at),
            ],
            output_symbol="consumed_generation",
            returning_columns=COLUMNS[GENERATION],
        ),
        dsl.update_node(
            f"{body_id}.barrier_update",
    ...
    return dsl.body(
        ...
        _symbols(
            [
                ("generation_locator", GEN_LOC),
                ("closed_reason", F + "generation_terminal_reason"),
            ],
            locals_,
        ),
        nodes,
...
def _retention_census_nodes(
    body_id: str, scope: dict[str, dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    return [
        dsl.select_node(
            ...
            relation=HEAD,
            columns=COLUMNS[HEAD],
            predicate=_predicate(HEAD, scope),
            cardinality="EXACTLY_ONE",
            output_symbol="retention_head",
            order_by=["practice_id", "source_contract_id", "stream_id"],
        ),
        dsl.select_node(
            f"{body_id}.policies",
            ...
            predicate=policy_pred,
            cardinality="COMPLETE_SET",
            output_symbol="policy_set",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        dsl.select_node(
            f"{body_id}.generations",
            relation=GENERATION,
            columns=COLUMNS[GENERATION],
            predicate=generation_pred,
            cardinality="COMPLETE_SET",
            output_symbol="generation_set",
            order_by=COORDS,
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.checkpoints",
            relation=CHECKPOINT,
            columns=COLUMNS[CHECKPOINT],
            predicate=checkpoint_pred,
            cardinality="COMPLETE_SET",
            output_symbol="checkpoint_set",
            order_by=COORDS + ["last_contiguous_position"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.anchors",
        ...
        dsl.select_node(
            ...
            predicate=pin_pred,
            cardinality="COMPLETE_SET",
            output_symbol="pin_set",
            order_by=["practice_id", "source_contract_id", "stream_id", "pin_id"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.keys",
            ...
            predicate=key_pred,
            cardinality="COMPLETE_SET",
            output_symbol="key_set",
            order_by=COORDS + ["interval_start"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.source_rows",
            relation=SOURCE,
            columns=COLUMNS[SOURCE],
            predicate=_predicate(SOURCE, scope),
            cardinality="COMPLETE_SET",
            output_symbol="source_set",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        dsl.select_node(
            ...
            predicate=receipt_pred,
            cardinality="COMPLETE_SET",
            output_symbol="retention_receipt_set",
            order_by=COORDS + ["source_position"],
            set_read=True,
        ),
        dsl.select_node(
            ...
            cardinality="COMPLETE_SET",
            output_symbol="retention_audit_set",
            order_by=COORDS + ["lifecycle_revision"],
            set_read=True,
        ),
    ]
...
def _generation_row_scope(
    *,
    ...
    return {name: _col("census_generation", GENERATION, name) for name in names}
...
def _retention_proof_nodes(
    ...
    generation_loop = dsl.node(
        ...
        complete_set=True,
        order_by=[{"column": column, "direction": "ASC"} for column in COORDS],
        convergence="REJOIN",
    )
    return [
        dsl.select_node(
            f"{body_id}.required_source_rows",
            relation=SOURCE,
            columns=COLUMNS[SOURCE],
            predicate=required_source_pred,
            cardinality="COMPLETE_SET",
            output_symbol="required_source_set",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        dsl.select_node(
            f"{body_id}.mature_source_rows",
            relation=SOURCE,
            columns=COLUMNS[SOURCE],
            predicate=mature_source_pred,
            cardinality="COMPLETE_SET",
            output_symbol="mature_source_set",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        dsl.select_node(
            f"{body_id}.mature_checkpoints",
            relation=CHECKPOINT,
            columns=COLUMNS[CHECKPOINT],
            predicate=mature_checkpoint_pred,
            cardinality="COMPLETE_SET",
            output_symbol="mature_checkpoint_set",
            order_by=COORDS,
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.mature_receipts",
            relation=RECEIPT,
            columns=COLUMNS[RECEIPT],
            predicate=mature_receipt_pred,
            cardinality="COMPLETE_SET",
            output_symbol="mature_receipt_set",
            order_by=COORDS + ["source_position"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.mature_audits",
            relation=AUDIT,
            columns=COLUMNS[AUDIT],
            predicate=mature_audit_pred,
            cardinality="COMPLETE_SET",
            output_symbol="mature_audit_set",
            order_by=COORDS + ["lifecycle_revision"],
            set_read=True,
        ),
        dsl.select_node(
            f"{body_id}.overlapping_keys",
            relation=KEY,
            columns=COLUMNS[KEY],
            predicate=overlapping_key_pred,
            cardinality="COMPLETE_SET",
            output_symbol="overlapping_key_set",
            order_by=COORDS + ["interval_start"],
            set_read=True,
        ),
        generation_loop,
...
def _retention_census_digest() -> dict[str, Any]:
    return dsl.digest(
        F + "source_retention_census_digest_v1",
        [
            _count("policy_set", POLICY),
            _count("generation_set", GENERATION),
            _count("checkpoint_set", CHECKPOINT),
            _count("anchor_set", ANCHOR),
            _count("pin_set", PIN),
            _count("key_set", KEY),
            _count("source_set", SOURCE),
            _count("retention_receipt_set", RECEIPT),
            _count("retention_audit_set", AUDIT),
            _col("retention_head", HEAD, "stream_epoch"),
...
def _slowest_checkpoint_position() -> dict[str, Any]:
    ...
    return dsl.case(
        ...
        [
            {
                "when": dsl.eq(
                    _count("checkpoint_set", CHECKPOINT), dsl.const(PG + "bigint", 0)
                ),
                "then": dsl.const(PG + "bigint", 0),
            }
        ],
        minimum,
    )
...
def _retention_reason() -> dict[str, Any]:
    census_complete = dsl.all_of(
        dsl.eq(
            _count("checkpoint_set", CHECKPOINT), _count("generation_set", GENERATION)
        ),
        dsl.binary(
            "GTE", _count("anchor_set", ANCHOR), _count("generation_set", GENERATION)
        ),
    )
    ...
    grace_elapsed = dsl.all_of(
        dsl.eq(
            _count("mature_source_set", SOURCE), _count("required_source_set", SOURCE)
        ),
        dsl.eq(
            _count("mature_checkpoint_set", CHECKPOINT),
            _count("checkpoint_set", CHECKPOINT),
        ),
        dsl.eq(
            _count("mature_receipt_set", RECEIPT),
            _count("retention_receipt_set", RECEIPT),
        ),
    ...
    return dsl.case(
        ...
        [
            {
                "when": dsl.eq(
                    _count("generation_set", GENERATION), dsl.const(PG + "bigint", 0)
                ),
                "then": dsl.const(
                    F + "source_retention_reason", "NO_NON_CONSUMED_GENERATION"
                ),
            },
            {
                "when": dsl.unary("NOT", census_complete),
                "then": dsl.const(F + "source_retention_reason", "AMBIGUOUS_CENSUS"),
            },
            {
                "when": dsl.unary("NOT", checkpoint_current),
                "then": dsl.const(F + "source_retention_reason", "CHECKPOINT_LAG"),
            },
            {
                "when": dsl.binary(
                    "GT", _count("pin_set", PIN), dsl.const(PG + "bigint", 0)
                ),
                "then": dsl.const(F + "source_retention_reason", "ACTIVE_PIN"),
            },
            {
                "when": dsl.unary("NOT", key_overlap),
                "then": dsl.const(F + "source_retention_reason", "KEY_OVERLAP"),
            },
            {
                "when": dsl.unary("NOT", grace_elapsed),
                "then": dsl.const(F + "source_retention_reason", "GRACE_PENDING"),
            },
            {
                "when": dsl.eq(
                ...
                "then": dsl.const(F + "source_retention_reason", "EXECUTION_DISABLED"),
            },
        ],
...
def _retention_eligible() -> dict[str, Any]:
    return dsl.all_of(
        dsl.binary(
            "GT", _count("generation_set", GENERATION), dsl.const(PG + "bigint", 0)
        ),
        dsl.eq(
            _count("checkpoint_set", CHECKPOINT), _count("generation_set", GENERATION)
        ),
...
def _retention_locals() -> list[tuple[str, str]]:
    return [
        ...
        ("key_set", KEY + "[]"),
        ("source_set", SOURCE + "[]"),
        ("retention_receipt_set", RECEIPT + "[]"),
        ("retention_audit_set", AUDIT + "[]"),
        ("retention_policy", POLICY),
        ("census_digest", F + "digest_sha256"),
        ("slowest_checkpoint_position", PG + "bigint"),
        ("required_source_set", SOURCE + "[]"),
        ("mature_source_set", SOURCE + "[]"),
        ("mature_checkpoint_set", CHECKPOINT + "[]"),
        ("mature_receipt_set", RECEIPT + "[]"),
        ("mature_audit_set", AUDIT + "[]"),
        ("overlapping_key_set", KEY + "[]"),
        ("key_overlap_covered", PG + "boolean"),
        ("census_generation", GENERATION),
        ("census_checkpoint", CHECKPOINT),
        ("census_anchor", ANCHOR),
        ("census_generation_key_set", KEY + "[]"),
        ("census_generation_pin_set", PIN + "[]"),
    ]
...
def build_retention_evaluation_body() -> dict[str, Any]:
    ...
    result = _composite(
        output_type,
        [
            ("eligible", _retention_eligible()),
            (
                "through_position",
                dsl.local_ref("slowest_checkpoint_position", PG + "bigint"),
            ),
            ("census_digest", dsl.local_ref("census_digest", F + "digest_sha256")),
            ("reason_code", _retention_reason()),
        ],
    )
    ambiguous_result = _composite(
        output_type,
        [
            ...
            (
                "reason_code",
                dsl.const(F + "source_retention_reason", "AMBIGUOUS_CENSUS"),
            ),
        ],
    )
    exact_policy_branch = [
        dsl.select_node(
            ...
            predicate=dsl.all_of(
                _predicate(POLICY, scope),
                dsl.binary(
                    "LTE", _src(POLICY, "effective_at"), dsl.transaction_timestamp()
                ),
            ),
            cardinality="EXACTLY_ONE",
            output_symbol="retention_policy",
            order_by=[
                "practice_id",
                "source_contract_id",
                "stream_id",
        ...
        dsl.let_node(
            f"{body_id}.slowest_checkpoint",
            "slowest_checkpoint_position",
            PG + "bigint",
            _slowest_checkpoint_position(),
        ),
        *_retention_proof_nodes(body_id, scope),
        dsl.let_node(
            f"{body_id}.key_overlap_coverage",
            "key_overlap_covered",
            PG + "boolean",
            _key_overlap_coverage(),
        ),
        dsl.let_node(f"{body_id}.result", "retention_result", output_type, result),
        _retry_or_return(
            body_id, "eligibility", "retention_result", output_type, composite=True
        ),
    ]
    ambiguous_policy_branch = [
        dsl.let_node(
            f"{body_id}.ambiguous_result",
            "ambiguous_retention_result",
            output_type,
            ambiguous_result,
        ),
        _retry_or_return(
    ...
    nodes = [
        ...
        *_retention_census_nodes(body_id, scope),
        dsl.let_node(
            f"{body_id}.census_digest",
            "census_digest",
            F + "digest_sha256",
            _retention_census_digest(),
        ),
        _if(
    ...
    locals_ = (
        [(row["id"], row["type"]) for row in binding_symbols]
        + _retention_locals()
        + [
    ...
    return dsl.body(
        ...
        "ENTRY_POINT",
        body_id,
        _symbols([("practice_source_stream", SCOPE)], locals_),
        nodes,
    )
...
def build_purge_body() -> dict[str, Any]:
    ...
    return dsl.body(
        ...
        body_id,
        _symbols(
            [("practice_source_stream", SCOPE), ("through_position", PG + "bigint")],
            locals_,
        ),
        nodes,
    )
