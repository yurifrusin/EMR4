#!/usr/bin/env python3
"""Provider-free real-isolation rehearsal for structured-source PlanProgram."""
...
IMAGE = "reception-one-structured-source-plan-cell:v1"
CONTAINER = "reception-one-structured-source-plan-cell-v1"
DOCKERFILE = legacy_lane.ARTIFACT_DIR / "Dockerfile"
CELL_SCRIPT = ROOT / "scripts" / "reception_one_structured_source_plan_cell.py"
FRAME = (
    ROOT
    / "orchestration"
    / "continuity"
...
class IsolationError(RuntimeError):
    """A bounded structured-source isolation failure."""
...
def run_isolation() -> dict[str, Any]:
    ...
    if exists(docker, "container", CONTAINER) or exists(
        docker, "image", IMAGE
    ):
        raise IsolationError("owned_residue_preexisted")

    frame = structured.load_object(FRAME)
    program = structured.load_object(FIXTURE)
    ...
    with tempfile.TemporaryDirectory(
        prefix="reception-one-structured-source-cell-"
    ) as raw:
        ...
        try:
            ...
            run_command(
                ...
                [
                    ...
                    CONTAINER,
                    "--hostname",
                    "reception-one-structured-source-cell",
                    "--network",
                    "none",
                    "--read-only",
                    "--user",
    ...
    review, normalized, candidate, note_review = (
        structured.proofread_program(frame, emitted)
    )
    if (
    ...
    return {
        "schema_version": (
            "reception.one.structured_source.isolation_evidence.v1"
        ),
        "result": (
            "reception_one_structured_source_language_real_isolation_pass"
        ),
        "evidence_mode": "authored_synthetic_provider_free_fixture",
        "source_manifest_sha256": structured.canonical_hash(source_hashes),
        "model_input_sha256": structured.canonical_hash(model_input),
        "program_sha256": structured.canonical_hash(emitted),
        "candidate_sha256": structured.canonical_hash(candidate),
        "normalized_plan_sha256": structured.canonical_hash(normalized),
