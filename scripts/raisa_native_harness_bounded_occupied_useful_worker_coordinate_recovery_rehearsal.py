"""Control one typed-coordinate native-Harness DeepSeek edit of a default-off runbook."""
...
TERMINAL_SCHEMA_VERSION = "ariadne.native_harness_useful_worker_coordinate_recovery_terminal.v1"
RUNNER_TERMINAL_SCHEMA_VERSION = (
    "ariadne.native_harness_useful_worker_coordinate_runner_terminal.v1"
)
...
def _accepted_coordinate_binding() -> dict[str, Any]:
    evidence_path = (
        REPO_ROOT
        / "orchestration"
        / "continuity"
    ...
    runner_path = evidence_path.with_name("future-runner.mjs")
    evidence = load_json(evidence_path)
    binding = evidence.get("future_runner")
    if (
        evidence.get("status") != "passed"
        or not isinstance(binding, dict)
        or binding.get("sha256") != sha256_file(runner_path)
        or binding.get("bytes") != len(runner_path.read_bytes())
        or binding.get("sha256")
        != "33e4ceefc612ac2410c95b71d80ccc42e913976379e8c088f0b8eff966853282"
    ):
        raise UsefulWorkerError("accepted_coordinate_runner_binding_invalid")
    return {
        "bytes": binding["bytes"],
        "sha256": binding["sha256"],
        "source_commit": evidence["accepted_runner"]["source_commit"],
    }
...
def runner_source(target_path: str) -> bytes:
    ...
    _accepted_coordinate_binding()
    source = accepted_path.read_text(encoding="utf-8")
    original_target = (
        'const TARGET_PATH = '
        '"/synthetic/workspace/orchestration/harness_settings/'
        'raisa-check-in-rollout-kill-switch.json";'
    )
    if source.count(original_target) != 1:
        raise UsefulWorkerError("accepted_coordinate_runner_target_invalid")
    ...
    early_lifecycle = (
        'tool_lifecycle: { input_result_kind: "unobserved", '
        'post_execute_decision_kind: "unobserved", '
        'conclusion_request_stage: "not_requested", '
        'authoritative_final_result_kind: "unobserved", coordinate: null }'
    )
    if source.count(early_lifecycle) != 1:
        raise UsefulWorkerError("accepted_coordinate_runner_early_terminal_invalid")
    source = source.replace(early_lifecycle, "tool_lifecycle: null")
...
def validate_runner_source(payload: bytes, target_path: str) -> dict[str, Any]:
    ...
    pre_end = source.index('agentCtx.on("tools/post-execute"', pre_start)
    pre_block = source[pre_start:pre_end]
    post_start = pre_end
    post_end = source.index('agentCtx.on("tools/result"', post_start)
    post_block = source[post_start:post_end]
    checks = {
        "accepted_coordinate_runner_bound": _accepted_coordinate_binding()["sha256"]
        == "33e4ceefc612ac2410c95b71d80ccc42e913976379e8c088f0b8eff966853282",
        ...
        "one_result_hook": source.count('agentCtx.on("tools/result"') == 1,
        "one_conclusion": source.count("exec.concludeTurn()") == 1,
        "conclusion_before_dispatch": pre_block.index("exec.concludeTurn()")
        < pre_block.index("return next()"),
        "no_post_execute_conclusion": "concludeTurn()" not in post_block,
        "one_request_success": "summary.request_count === 1" in source,
        "one_edit_success": 'summary.tool_names[0] === "edit"' in source,
        "typed_success_coordinate": 'toolLifecycleCoordinate === "edit_success_accept_concluded"'
        in source,
        "pre_lifecycle_failure_null": source.count("tool_lifecycle: null") == 1,
        "runner_schema_exact": source.count(RUNNER_TERMINAL_SCHEMA_VERSION) == 2,
        "root_service_forwarded": "assertEffectiveToolComposition(agentCtx, presets,"
        in source,
        ...
        "no_retry_shell_subagent": all(
            token not in source.lower()
            for token in ("retry(", "pwsh", "bash", "subagent")
        ),
    }
...
def contract_value() -> dict[str, Any]:
    ...
    return {
        ...
        "source_bindings": {
            ...
            "accepted_coordinate_runner": _accepted_coordinate_binding(),
...
def render_report(terminal: dict[str, Any]) -> str:
    ...
    return f"""# Native Harness useful-worker coordinate-recovery occupied report
