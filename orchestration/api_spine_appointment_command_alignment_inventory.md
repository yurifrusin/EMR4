...
| `POST /api/v1/appointments/proposals/status/{appointment_id:uuid}` | `propose_status_update` | `proposeAppointmentStatus` | `proposal command` | Prepares status-change evidence with waiting-area side-effect visibility, warning/block envelope, freshness, signed evidence, and confirm payload; the UUID converter reserves literal `/status/confirm`; no appointment write. |
...
| `POST /api/v1/appointments/proposals/status/confirm` | `confirm_status_proposal_route` | `confirmAppointmentStatusProposal` | `confirm command` | Canonical status-only path. Requires explicit confirmation, signed evidence, opaque database-version binding, current authority/source recheck, atomic audit/receipt completion and exact stored-byte replay. |
| `POST /api/v1/appointments/proposals/status-confirm` | `confirm_status_proposal_route` | `confirmAppointmentStatusProposal` | `compatibility alias` | Hidden OpenAPI compatibility spelling over the exact canonical handler; owns no independent or fallback write path. |
...
| `POST /api/v1/appointments/proposals/delete/confirm` | `confirm_delete_proposal_route` | `confirmAppointmentDeleteProposal` | `confirm command` | Canonical delete-only path. Requires explicit confirmation, signed evidence, one opaque server-minted `raisa.delete_proposal_version_binding.v1`, current authority/source recheck, atomic audit/receipt completion and exact canonical public-envelope bytes for committed/replay delivery. |
| `POST /api/v1/appointments/proposals/delete-confirm` | `confirm_delete_proposal_route` | `confirmAppointmentDeleteProposal` | `compatibility alias` | Hidden OpenAPI compatibility spelling over the exact canonical handler; owns no independent or fallback write path. |
...
| `POST /api/v1/appointments/proposals/reception-one/compose` | `compose_reception_one_product_context_proposal` | `composeReceptionOneProductContextProposal` | `command-style read` | Default-off development-only authored-synthetic surface. Performs bounded practice-scoped context and slot reads, gives the planner only request-scoped opaque handles, applies the typed proofreader, and releases a proposal or clarification without provider use, confirmation, appointment/audit write, or legacy interpreter-gate change. |
...
- Current backend confirm route names differ from the OpenAPI draft for several canonical paths:
  - status-confirm is now aligned at backend/OpenAPI `POST /api/v1/appointments/proposals/status/confirm`, with the historical hyphenated spelling retained only as a hidden compatibility alias;
  - delete-confirm is now aligned at backend/OpenAPI `POST /api/v1/appointments/proposals/delete/confirm`, with the historical hyphenated spelling retained only as a hidden compatibility alias;
