...
| Item | Value |
|---|---|
| Status | Sprint 158 confirm-client surface checkpoint completed; sprint engine continuing |
...
| Next Candidate Sprints | Sprint 159 should wire Bernie tool-intent update-confirm HTTP `Idempotency-Key` headers before Sprint 160 prepares the Bernie/Diary review-readiness packet and Yuri review pause |
...
route tests required before wiring. Sprint 131 added the guarded staff
create-confirm route-test contract, with executable scope guards and skipped
future behavior tests ready to enable during wiring. Sprint 132 wired the staff
create-confirm route only: it now requires HTTP `Idempotency-Key`, claims and
completes the appointment command idempotency ledger around the appointment and
audit write in one transaction, replays completed same-body requests, fails
closed on conflicts/in-progress/stale/failed-transient rows, rolls back started
claims on business-rule blocks, and keeps Bernie/update/status/delete/raw/
proposal-only families out of scope. Sprint 133 resumed the sprint engine with
a preflight for the next confirmation family,
`POST /api/v1/appointments/proposals/create/confirm-bernie`, and documented the
extra boundary that Bernie confirmation has session-event side effects
(`confirm_submitted` and `confirmation_outcome`) in addition to the
appointment/audit/ledger transaction. It deliberately did not wire
`Idempotency-Key` enforcement to `confirm-bernie`. Sprint 134 added the guarded
route-test contract for `confirm-bernie`, incorporating DeepSeek's review that
`confirmation_outcome` is the concrete double-event replay risk because it must
not rely on session-store idempotency. The current route remains unwired, and
future behavior tests are present but skipped until Sprint 135 wiring.
Sprint 135 wired only `confirm-bernie`: it now requires HTTP
`Idempotency-Key`, claims the appointment command ledger after typed body
validation and before signed evidence/session binding/session event mutation,
replays completed same-body responses without duplicate appointment/audit rows
or duplicate `confirm_submitted`/`confirmation_outcome` events, fails closed on
conflict/in-progress/stale/failed-transient rows, and keeps update/status/delete/
raw/proposal-only families out of scope. Sprint 136 then preflighted the next
confirmation family and accepted DeepSeek's recommendation to choose
`status-confirm` before `update-confirm` or `delete-confirm`: it is
self-contained, has a smaller confirmation body, avoids update-confirm's
proposal revalidation window, and is less destructive than delete-confirm. No
route behavior changed in Sprint 136. Sprint 137 added the guarded
status-confirm route-test contract, preserving no-wiring scope while defining
future tests for missing keys, invalid payloads, first writes, replay,
conflict, in-progress/stale/failed rows, signed-evidence/freshness bypass
prevention, union variants, waiting-area side effects, telemetry, and the
internal commit boundary in `_apply_appointment_status_update`. DeepSeek's
Sprint 137 metadata-hash suggestion is recorded as a Sprint 138 decision rather
than silently changing canonicalization policy. Sprint 138 accepted DeepSeek's
follow-up recommendation to keep full validated confirmation-body hashing for
consistency with staff and Bernie create-confirm, then wired only
`status-confirm`: the route now requires `Idempotency-Key`, claims/replays via
the appointment command ledger, uses `_apply_appointment_status_update(...,
commit=False)`, completes the ledger before the single commit, and preserves
update/delete/raw/proposal-only families out of scope.
Sprint 139 then preflighted the next family and chose `update-confirm` before
`delete-confirm`. DeepSeek affirmed that delete-confirm is structurally simpler,
but update-confirm is the safer next risk-order step because it is reversible
and exercises the update revalidation pattern before the destructive soft-cancel
path. Sprint 139 did not change route behavior. Sprint 140 added the guarded
update-confirm route-test contract without route wiring. It records that
idempotency must live at the route wrapper before `confirm_update_proposal()`
re-runs `propose_update_appointment()`, that `_apply_appointment_update()` needs
a scoped `commit=False` path before ledger completion, that blocked started
claims should roll back transactionally, and that full validated confirmation
body hashing must include signed evidence, freshness, turn/session metadata,
and `confirmed_warnings` unless a future cross-route canonicalization version
change is explicitly approved. Sprint 141 then wired update-confirm only: the
route now requires `Idempotency-Key`, claims/replays before
`confirm_update_proposal()` and update revalidation, uses
`_apply_appointment_update(..., commit=False)`, rolls back started claims on
blocked responses, completes the ledger before the single commit, and leaves
delete/raw/proposal-only families out of scope. Sprint 142 then preflighted
delete-confirm as the final proposal-confirm appointment mutation family. The
...
remains unchanged. Sprint 144 then wired delete-confirm only: the route now
requires `Idempotency-Key`, claims/replays through the appointment command
ledger, uses `_apply_appointment_delete(..., commit=False)` so soft-cancel,
audit row, ledger completion, and commit are transactional, rolls back started
claims on blocked destructive checks, and preserves raw/proposal-only/provider/
GraphQL/H15/memory/trove gates. Sprint 145 then added a non-runtime checkpoint
for the full proposal-confirm appointment mutation family: staff create,
Bernie create, status, update, and delete confirm are recorded as wired, their
operation IDs and route families are guarded, the shared fail-closed decision
map is documented, and raw/proposal-only/provider/GraphQL/H15/memory/trove
gates remain closed. Sprint 146 then added route-level DB-backed integration
tests across all five wired confirmation families. The matrix proves missing
keys, same-key replay, same-key conflicts, active in-progress, stale
in-progress, and failed-transient rows share the same fail-closed behavior and
do not produce duplicate appointment, audit, ledger, or Bernie session-event
side effects.
Sprint 147 then selected proposal-only appointment routes as the next
preflight surface before raw compatibility writes, because they are canonical
OpenAPI command-plane routes and non-mutating. DeepSeek identified the crucial
design distinction: proposal idempotency must be proposal-specific client
discipline and must not copy confirmation-write replay authority. The preflight
...
Sprint 149 then chose deterministic re-evaluation for create-proposal
idempotency before route wiring. Future create-proposal enforcement should
require syntactically valid header keys but must not create proposal ledger
rows, store proposal-envelope replay, return same-key/different-body `409`, or
introduce a separate Bernie proposal path. DeepSeek accepted this as the safest
default and recommended route-level header binding plus operation identity for
logging/review metadata only.
Sprint 150 then wired only `POST /api/v1/appointments/proposals/create` to
require a non-blank `Idempotency-Key` header. The route deterministically
re-evaluates current diary state, rejects missing/blank keys, accepts
same-key/same-body and same-key/different-body retries as fresh proposal
evaluations, creates no `AppointmentCommandIdempotency` rows, and leaves
other proposal families and raw compatibility writes unwired. OpenAPI
`minLength: 8` enforcement is deliberately deferred for a compatibility
alignment sprint. Sprint 151 then added the OpenAPI/FastAPI header alignment guard for create-proposal:
...
Sprint 153 then closed the first concrete client-readiness gap by wiring the
real diary create-proposal caller to send a generated 8+ character
`Idempotency-Key` header, scoped to the booking modal proposal attempt and
cleared when proposal confirmation resets. Backend runtime `minLength: 8`
enforcement remains deferred; no OpenAPI, ledger, route, provider, GraphQL,
H15/H-series, memory/RAG/GraphRAG, or raw compatibility write behavior changed.
Sprint 154 then mapped the remaining diary/API header gap without changing
runtime behavior. It records that only the create-proposal caller currently
emits HTTP `Idempotency-Key`, while five already-enforced confirmation routes
still have missing Diary frontend headers: staff create confirm,
confirm-Bernie, update confirm, status confirm, and delete confirm. It also
extends the proposal-only backend binding inventory to include
`propose_waiting_area_update` as the status/waiting-area handler variant.
Sprint 155 then wired the first already-enforced confirmation client gap:
staff create-confirm now sends a separate stable modal-scoped
`confirmIdempotencyKey`, and the Bernie review confirm adapter sends a stable
Bernie route key for create-confirm-Bernie. It leaves update/status/delete
confirm, Bernie tool-intent confirm, proposal-only binding, raw compatibility
writes, backend ledger/runtime behavior, and strict `minLength: 8` enforcement
unchanged.
Sprint 156 then wired status-confirm and delete-confirm client headers in the
dedicated Diary helper functions. The header key is derived from
`status_proposal_freshness_id` or `delete_proposal_freshness_id`, with a
...
1. If Bernie/API-spine architecture is the priority: continue **Programme 2G**
   with the guarded delete-confirm idempotency route-test contract before
   touching the destructive soft-cancel confirmation path, excluding raw/
   proposal-only families.
...
The default recommendation after Sprint 158 is **Programme 2G**: Sprint 159
should wire Bernie tool-intent update-confirm HTTP `Idempotency-Key` headers,
because that user-clickable path posts to an already-enforced update-confirm
backend route. Sprint 160 should then prepare the Bernie/Diary
review-readiness packet and pause for Yuri if the readiness checks pass.
Proposal-only backend binding and strict `minLength: 8` enforcement remain
deferred behind that review path. Preserve backend ledger semantics, raw
compatibility writes, providers, GraphQL, H15/H-series, memory/RAG/GraphRAG,
and historical diary trove gates.
