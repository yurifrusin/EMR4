...
| Item | Value |
|---|---|
| Status | Sprint 149 create-proposal replay-model decision completed; sprint engine continuing |
...
| Next Candidate Sprints | Sprint 150 create-proposal idempotency wiring using deterministic re-evaluation semantics |
...
Current position after Sprint 149: the Ariadne/Fable 100+ sprint strategy map
...
The default recommendation after Sprint 149 is **Programme 2G**: wire
`POST /api/v1/appointments/proposals/create` to require syntactically valid
`Idempotency-Key` while preserving deterministic re-evaluation semantics. Enable
the Sprint 148 future tests as DB-backed route tests and prove same-key retries
do not create proposal ledger rows, cached proposal replay, slot reservations,
or confirmation-write authority.
