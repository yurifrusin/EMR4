# LC4V4 One-Shot Acceptance Rule V2

Date frozen: 2026-07-15

Attempt identity: `lc4v4-fresh-attempt-002`

Authority: Yuri explicitly authorized this proportionate fresh attempt after
reviewing the metadata-only incident recorded in
`orchestration/agent_inbox/codex/lc4v4-preauthoring-metadata-incident.md`.
The first content attempt remains invalid and is not reinterpreted or resumed.
No actual v4 content or baseline existed in that attempt.
...
- `orchestration/agent_inbox/codex/lc4v4-one-shot-acceptance-rule-v2.md`;
...
receipt, and execution of the named v4 scripts/tests are permitted. Recursive
...
The result is `evidence_invalid` if any attempt identity, population, coverage,
hash, schema, source-commit, quality-receipt, manifest, seal, exclusive-write,
report-first/consumed-seal-last, one-shot, variance, aggregate-leakage,
external-session, named-file, or protected-boundary condition fails. Invalid
evidence cannot be rerun, repaired, or replaced without a new explicit Yuri
decision. Partial output is consumed evidence and is not a retry authorization.
