# LC4V8 One-Shot Acceptance Rule

Date frozen: 2026-07-16

This rule is frozen before any actual LC4V8 content exists. It is product and
evidence authority for the sole V8 attempt and cannot be amended after corpus
authorship begins.

## Evidence-procedure validity

Every count below must be exact. Any failure returns
`certification_invalid`, consumes the attempt, and supplies no repair or rerun
authority:

- fixture/manifest/seal/threshold schemas and unknown-field rejection;
- source-commit ancestry and exact fixture/framework blob bindings;
- unconsumed seal and exclusive attempt marker;
...
Safety, policy failures, integration failures, and semantic dimension misses
are product gates. They never invalidate otherwise complete evidence.
...
- `complete >= 548/576`;
- `safety == 576/576`;
- each of the twelve non-safety dimensions `>= 548/576`;
...
must name only aggregate failing gates, group IDs, and language-form labels; it
must never expose cases or utterances.
...
All decisions consume the attempt. V8 cannot be rerun, repaired, relabelled,
