...
- Review result: Integrated with Ariadne repair to echo backend turn/freshness evidence from the Diary confirm flow; focused backend suite and full diary smoke passed.
- Follow-up required: Consider making turn refs required for context-dependent calls after backward compatibility is no longer needed.
