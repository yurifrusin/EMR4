...
let bernieAutoPreview = (new URLSearchParams(window.location.search)).get("bernie_auto_preview") !== "false";

// ─── Sprint 100 Bernie State Machine Definition ───
class BernieSession {
  constructor() {
    this.state = "INACTIVE"; // INACTIVE, CONTEXT_SELECTION, INSTRUCTION_ENTRY, INTERPRETING, CANDIDATE_SELECTION, SLOT_PREVIEW, CONFIRMING, CONFIRMED
  ...
  updateTurnRef(turnRef) {
    if (turnRef && typeof turnRef === "object") {
      ...
      this.sessionId = turnRef.session_id || this.sessionId;
      if (turnRef.reference_date && !this.referenceDate) {
        this.referenceDate = turnRef.reference_date;
      }
    }
  }
  ...
  init(practitionerId, patientId) {
    this.state = (practitionerId && patientId) ? "INSTRUCTION_ENTRY" : "CONTEXT_SELECTION";
    if (!this.referenceDate) {
      const urlParams = new URLSearchParams(window.location.search);
      this.referenceDate = urlParams.get("reference_date") || localDateKey(diaryDate);
    }
  ...
  reset() {
    ...
    bernieAutoPreview = (new URLSearchParams(window.location.search)).get("bernie_auto_preview") !== "false";
    const toggle = document.getElementById("bernie-auto-preview-toggle");
  ...
  transitionTo(nextState) {
    const validTransitions = {
      ...
      CONTEXT_SELECTION: ["INSTRUCTION_ENTRY", "INACTIVE"],
      INSTRUCTION_ENTRY: ["INTERPRETING", "CONTEXT_SELECTION", "INACTIVE"],
      INTERPRETING: ["CANDIDATE_SELECTION", "SLOT_PREVIEW", "INSTRUCTION_ENTRY", "CONTEXT_SELECTION", "INACTIVE"],
      CANDIDATE_SELECTION: ["SLOT_PREVIEW", "CONTEXT_SELECTION", "INSTRUCTION_ENTRY", "INACTIVE"],
      SLOT_PREVIEW: ["CANDIDATE_SELECTION", "CONFIRMING", "CONTEXT_SELECTION", "INACTIVE"],
      CONFIRMING: ["CONFIRMED", "SLOT_PREVIEW"],
      CONFIRMED: ["INACTIVE", "INSTRUCTION_ENTRY", "CONTEXT_SELECTION"]
    };

    if (validTransitions[this.state] && !validTransitions[this.state].includes(nextState)) {
  ...
  syncToLegacy() {
    ...
    bernieLatestCandidatePayload = this.candidateSnapshot;
    if (this.latestReviewPayload) {
      bernieLatestIdentityEvidence = this.latestReviewPayload.identity_evidence || null;
    }
  }

  syncFromLegacy() {
    this.instructionText = bernieInstructionText;
    this.interpretEnvelope = bernieInterpretResult;
    this.selectedCandidateIndex = bernieSelectedCandidateIndex;
    this.stagedBookingPreview = bernieStagedBookingPreview;
    this.latestReviewPayload = bernieLatestReviewPayload;
    this.candidateSnapshot = bernieLatestCandidatePayload;
  }
  ...
  handleContextChanged(newApptId) {
    ...
    if (appt) {
      ...
      if (apptPracId && apptPatId && !isPatientIdentityUnconfirmed(appt)) {
        this.init(apptPracId, apptPatId);
        return;
      }
    }
    this.init("", "");
...
const bernieSession = new BernieSession();
let lastActiveAppointmentId = null;

const checkinDefaultCache = new Map();

function bernieTurnRequestFields() {
  const fields = {
    session_id: bernieSession.sessionId,
    turns: bernieSession.turns
  };
  if (bernieSession.turnRef) {
    fields.turn_ref = bernieSession.turnRef;
  }
  return fields;
}
...
function enrichBernieConfirmPayload(confirmPayload) {
  const body = {
    ...confirmPayload,
    confirmed: true
  };
  const selection = body.selection_proposal || {};
  const selectedCandidate = selection.selected_candidate || {};
  if (!body.turn_ref && bernieSession.turnRef) {
    body.turn_ref = bernieSession.turnRef;
  }
  if (!body.candidate_freshness_id && selectedCandidate.candidate_freshness_id) {
    body.candidate_freshness_id = selectedCandidate.candidate_freshness_id;
  }
  if (!body.proposal_freshness_id && selection.proposal_freshness_id) {
    body.proposal_freshness_id = selection.proposal_freshness_id;
  }
  return body;
}
...
const BERNIE_STATUS_COPY = {
  ...
  clinic_day_exhausted: "Choose another day",
...
const BERNIE_HEADLINE_COPY = {
  ...
  clinic_day_exhausted: "No times left today",
  clarification: "Clarification required",
  advisory_warnings_only: "Ready to book this appointment"
};

const BERNIE_INFORMATIONAL_CODES = new Set([
  "existing_future_follow_up",
...
function bernieReviewTransition(payload) {
  ...
  if (outcomeKind) {
    ...
    if (policy.availability === "roster_unavailable" || policy.roster_unavailable === true) {
      ...
      state = "no_slots";
    } else if (policy.must_ask_clarification === true || policy.availability === "must_ask_clarification" || policy.availability === "clarification") {
      state = "clarification";
    } else if (policy.must_block_confirmation === true || policy.availability === "blocked") {
      state = "blocked";
    } else if (policy.advisory_warnings_only === true || policy.availability === "advisory_warnings_only") {
      state = "advisory_warnings_only";
...
function bernieStatusCopyForPayload(payload) {
  ...
  if (transition.state === "advisory_warnings_only") return "Ready to book";
  if (transition.state === "clarification") return "Clarification required";
  return bernieStatusCopy(transition.state, payload?.blocks);
}
...
function bernieHeadlineCopyForPayload(payload) {
  ...
  if (transition.state === "confirmation_ready" || transition.state === "advisory_warnings_only") return BERNIE_HEADLINE_COPY.confirmation_ready;
  if (transition.state === "clarification") return "Clarification required";
  return bernieHeadlineCopy(transition.state, payload?.blocks);
}
...
function bernieReviewActionCopy(payload) {
  ...
  if (transition.state === "no_selectable_candidates") {
    return "I need staff review before I can show a time. Nothing is booked yet.";
  }
  if (transition.state === "clarification") {
  ...
  if (payload.status === "clinic_day_exhausted") {
    return "That time window has passed for today. Tell me another day or a later time.";
  }
...
function getCandidateId(slot) {
  if (!slot) return "";
  if (slot.candidate_id) return slot.candidate_id;
  if (slot.id) return slot.id;
  const date = slot.appointment_date || "";
  const time = slot.start_time_local || "";
  const prac = slot.practitioner_id || slot.practitioner?.ahpra_number || "";
  return `slot-${prac}-${date}-${time}`.replace(/[:\s]/g, "-");
}

function getVisibleTurn(event) {
  if (!event) return null;
  if (event.kind === "staff_instruction") {
    return { actor: "staff", text: event.payload.instruction };
  } else if (event.kind === "clarification_reply") {
    return { actor: "staff", text: event.payload.reply };
  } else if (event.kind === "no_slot_suggestion_click") {
    return { actor: "staff", text: event.payload.suggestion };
  } else if (event.kind === "bernie_clarification") {
    return { actor: "bernie", text: event.payload.text };
  }
  return null;
}
...
function bernieComposerPlaceholder() {
  ...
  if (lastBernieEvent && lastBernieEvent.payload?.type === "clarification_question") {
    return "Reply to Bernie...";
  }
  if (bernieSession.state === "SLOT_PREVIEW" || bernieSession.state === "CONFIRMED") {
    return "Ask Bernie another thing...";
  }
  return "Ask Bernie...";
}

async function handleNoSlotSuggestionClick(suggestionText, originalTurnId) {
  bernieSession.clearResponse({ preserveInstruction: false });
  const textarea = document.getElementById("bernie-instruction-input");
  if (textarea) {
    textarea.value = "";
  }
  bernieInstructionText = "";

  bernieSession.addEvent("no_slot_suggestion_click", {
    suggestion: suggestionText,
    original_turn_id: originalTurnId
  });
...
function resolveBerniePilotLaunchRequest({ allowHarnessDefaults = false } = {}) {
  ...
  if (isBerniePilotActive) {
    if (!bernieSession.referenceDate) {
      bernieSession.referenceDate = allowHarnessDefaults ? (queryReferenceDate || "2026-06-27") : (queryReferenceDate || localDateKey(diaryDate));
    }
    if (bernieSession.state === "INACTIVE") {
      bernieSession.init(practitionerId, patientId);
    }
  }
  ...
  if (bernieSelectedCandidateIndex !== null && !isNaN(bernieSelectedCandidateIndex)) {
    body.selected_candidate_index = bernieSelectedCandidateIndex;
    body.reason = "Bernie supervised booking";
  }
...
async function fetchBernieInterpretation(requestBody) {
  ...
  if (!response.ok) {
    ...
    bernieSession.addEvent("bernie_clarification", { text: failedPayload.summary, type: "blocked" });
  ...
  if (reply) {
    ...
    if (!alreadyAdded) {
      const type = data.clarifying_question ? "clarification_question" : "summary";
      bernieSession.addEvent("bernie_clarification", { text: reply, type: type });
    }
  }
  return data;
  ...
  if (issues.length > 0) {
    ...
    if (!isBernieDevOrDebug() && !compactUnderstoodRequest && technicalIssues.length > 0) {
      ...
      technicalIssues.forEach(issue => {
        const item = document.createElement("div");
        item.className = "bernie-interpret-issue";
        item.textContent = bernieIssueDisplayText(issue) || formatBernieCode(issue.code || "detail");
...
function stagedPreviewMatchesColumn(preview, col) {
  if (!preview || !col) return false;
  if (preview.practitioner_id && col.practitioner_id === preview.practitioner_id) {
    return true;
  }
  if (preview.practitioner_ahpra && col.practitioner_ahpra === preview.practitioner_ahpra) {
    return true;
  }
  return false;
}

function renderBernieStagedBookingPreview(columnBody, col, dayStartMins, intervalMins) {
  const preview = bernieStagedBookingPreview;
  if (!preview || preview.appointment_date !== localDateKey(diaryDate)) return;
  if (!stagedPreviewMatchesColumn(preview, col)) return;

  const start = toMins(String(preview.start_time_local || "").slice(0, 5));
  if (!Number.isFinite(start)) return;
  const duration = Number(preview.duration_minutes) || 15;
  const topPx = (start - dayStartMins) * (SLOT_HEIGHT_PX / intervalMins);
  const heightPx = duration * (SLOT_HEIGHT_PX / intervalMins);
  const visualHeightPx = Math.max(heightPx + 22, 58);
  ...
  card.style.height = visualHeightPx + "px";
  ...
  if (preview.isProvisional) {
    ...
    helper.className = "bernie-provisional-text-helper";
    helper.textContent = " (Provisional)";
...
function renderBernieConfirmedState(contentEl) {
  ...
  container.className = "bernie-confirmed-container";
  container.setAttribute("data-testid", "bernie-confirmed-container");
  ...
  resetBtn.setAttribute("data-testid", "bernie-review-reset-button");
...
async function chooseAnotherTime() {
  bernieSession.addEvent("choose_another_time", {});
  ...
  if (bernieSession.candidateSnapshot) {
    renderBernieReview(bernieSession.candidateSnapshot, bernieSession.interpretEnvelope);
  } else {
    renderBernieReview({ status: 'candidate_selection_required', candidate_slots: [] }, bernieSession.interpretEnvelope);
  }
}

async function selectCandidate(slot, index) {
  bernieSession.transitionTo("SLOT_PREVIEW");
  bernieSession.selectedCandidateIndex = index;
  bernieSelectedCandidateIndex = index;

  const candidateId = getCandidateId(slot);
  bernieSession.addEvent("candidate_selection", {
    candidate_id: candidateId,
    index: index,
    slot: slot
  });

  bernieSession.addEvent("proposal_preview", {
    candidate_id: candidateId,
    preview: {
      appointment_date: slot.appointment_date,
      start_time_local: slot.start_time_local,
      duration_minutes: slot.duration_minutes || 15,
      practitioner_id: slot.practitioner_id || slot.practitioner?.ahpra_number || ""
    }
  });
  ...
  if (contentEl) {
    ...
    const loader = document.createElement("div");
    loader.className = "bernie-loading";
    loader.textContent = "Preparing preview...";
    contentEl.appendChild(loader);
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      bernieSession.stagedBookingPreview = {
      ...
      const targetDate = dateFromLocalKey(payload.selected_slot.appointment_date);
      if (targetDate && !isSameClinicDay(diaryDate, targetDate)) {
        diaryDate = targetDate;
        updateDateLabel();
        await loadDiary(false, { scrollToBerniePreview: true });
      } else {
        await loadDiary(true, { scrollToBerniePreview: true });
      }
    ...
    console.error("Supervised booking network error", err);
  }
}

async function stageBernieCandidateForReview(slot, index) {
  await selectCandidate(slot, index);
}
...
function renderGrid(template, slots, apptLookup, typeMap, occupied) {
  ...
  columns.forEach((col, colIdx) => {
    ...
    renderBernieStagedBookingPreview(columnBody, col, dayStartMins, intervalMins);
...
async function loadDiary(silent = false, options = {}) {
  ...
  try {
    ...
    } else if (options.scrollToBerniePreview && bernieStagedBookingPreview) {
      setTimeout(() => {
        const start = toMins(String(bernieStagedBookingPreview.start_time_local || "").slice(0, 5));
        if (Number.isFinite(start)) {
          scrollToTime(start, activeTemplate);
        }
      }, 100);
...
function clearStaleBernieBookingState(reason = "refresh") {
  ...
  const oldDate = bernieSession.referenceDate || localDateKey(diaryDate);
  const newDate = localDateKey(diaryDate);

  // Invalidate current staged preview/evidence
  ...
  // Reset state to INSTRUCTION_ENTRY or context selection
  const { appt } = getActiveBernieSelectedAppointment();
  const explicitContext = getBerniePilotContextValues();
  const practitionerId = explicitContext.practitionerId || (appt ? (appt.practitioner_id || appt.practitioner?.id) : "");
  const patientId = explicitContext.patientId || (appt ? (appt.patient_id || appt.patient?.id) : "");

  if (practitionerId && patientId) {
    bernieSession.state = "INSTRUCTION_ENTRY";
  } else {
    bernieSession.state = "CONTEXT_SELECTION";
  }

  // Update reference date
  bernieSession.referenceDate = newDate;

  // Log date_navigation_clear event
  bernieSession.addEvent("date_navigation_clear", {
    old_date: oldDate,
    new_date: newDate,
    reason: reason
  });
...
function shiftDay(delta) {
  ...
  updateDateLabel();
  const reason = delta > 0 ? "next_day" : "prev_day";
  clearStaleBernieBookingState(reason);
  loadDiary();
}
...
function renderBernieIdentityEvidence(contentEl, evidence, patientEvidence = null, payload = null) {
  ...
  card.setAttribute("data-testid", "bernie-identity-evidence");
...
function renderBernieReview(payload, interpretEnvelope = null) {
  ...
  if (bernieSession.state === "CONFIRMED") {
    renderBernieConfirmedState(contentEl);
    return;
  }
  ...
  if (payload && hasBernieSelectedSlotEvidence(payload) && payload.selected_slot && bernieAutoPreview && !suppressAutoPreview) {
    ...
    if (!hasAmbiguity && !hasCandidates) {
      ...
      if (isDifferent) {
        ...
        bernieSession.stagedBookingPreview = newPreview;
        autoStaged = true;
  ...
  if (
    ...
    headline.textContent = bernieHeadlineCopyForPayload(payload);
  }
  contentEl.appendChild(headline);
  ...
  if ((payload.status === "blocked" || payload.status === "clinic_day_exhausted") && transition.state !== "roster_unavailable" && transition.state !== "no_slots") {
    ...
    const title = document.createElement("span");
    title.className = "bernie-section-title";
    title.textContent = transition.canShowCandidates ? "Available times" : "Next options";
    candidatesContainer.appendChild(title);

    const list = document.createElement("div");
    list.setAttribute("data-testid", "bernie-review-candidates-list");

    const candidateSlots = transition.candidateSlots;
    if (candidateSlots.length === 0) {
      ...
      if (suggestions.length > 0) {
        ...
        list.appendChild(suggestionsContainer);
      }
    }
    ...
    const autoPreviewAllowed = bernieAutoPreview && !autoPreviewDisabled;
    if (reviewParam === "live" && autoPreviewAllowed && !isBernieDevOrDebug() && !suppressAutoPreview && bernieSession.selectedCandidateIndex === null && candidateSlots.length > 0) {
  ...
  if (isBernieDevOrDebug()) {
    const detailsDiag = document.createElement("details");
    detailsDiag.className = "bernie-dev-diagnostic-details";
    detailsDiag.setAttribute("data-testid", "bernie-dev-diagnostic-details");

    const summaryDiag = document.createElement("summary");
    summaryDiag.className = "bernie-dev-diagnostic-summary";
    summaryDiag.textContent = "Developer setup details";
    detailsDiag.appendChild(summaryDiag);
    ...
    diag.appendChild(diagList);
    detailsDiag.appendChild(diag);
    contentEl.appendChild(detailsDiag);
  }
}
...
function renderBernieInstructionInput(contentEl) {
  ...
  textarea.placeholder = bernieComposerPlaceholder();
  ...
  const updateInstructionStatusCopy = () => {
    ...
    if (contextReady && text.length > 0) {
      statusCopy.textContent = "Ready to ask. Nothing is booked or changed until you confirm.";
...
function updateBernieChatTranscriptUI() {
  const transcriptEl = document.getElementById("bernie-chat-transcript");
  const newSessionBtn = document.getElementById("btn-bernie-new-session");
  ...
  if ((bernieSession.turns && bernieSession.turns.length > 0) || bernieSession.serverConflict) {
    ...
    if (visibleTurns.length > 1) {
      ...
      historyDetails.setAttribute("data-testid", "bernie-chat-history");
      const summary = document.createElement("summary");
      summary.textContent = "Show earlier conversation";
      historyDetails.appendChild(summary);
      visibleTurns.slice(0, -1).forEach(turn => {
        const bubble = document.createElement("div");
        bubble.className = `bernie-chat-bubble ${turn.actor}`;
        bubble.textContent = turn.text;
        historyDetails.appendChild(bubble);
      });
      transcriptEl.appendChild(historyDetails);
    }
    const latestTurn = visibleTurns[visibleTurns.length - 1];
    if (latestTurn) {
      hasVisible = true;
      const latestLabel = document.createElement("div");
      latestLabel.className = "bernie-chat-latest-label";
      latestLabel.textContent = "Latest";
      transcriptEl.appendChild(latestLabel);
      const bubble = document.createElement("div");
      bubble.className = `bernie-chat-bubble ${latestTurn.actor}`;
      bubble.textContent = latestTurn.text;
...
async function loadBernieLiveReview() {
  ...
  // If in CONFIRMED state, render the confirmed screen
  if (bernieSession.state === "CONFIRMED") {
    if (contentEl) {
      renderBernieConfirmedState(contentEl);
    }
  ...
  // If in CANDIDATE_SELECTION and already have candidateSnapshot, render without fetching
  ...
  // If in SLOT_PREVIEW and already have a staged preview and review payload, render without fetching
  if (bernieSession.state === "SLOT_PREVIEW" && bernieSession.stagedBookingPreview && bernieSession.latestReviewPayload) {
    ...
    renderBernieReview(bernieSession.latestReviewPayload, bernieSession.interpretEnvelope);
    return;
  }
  ...
  if (!request.ready) {
    bernieSession.transitionTo("CONTEXT_SELECTION");
  ...
  if (bernieSession.selectedCandidateIndex !== null && !isNaN(bernieSession.selectedCandidateIndex)) {
    ...
    supervisedBody.reason = "Bernie supervised booking";
  } else if (request.body.selected_candidate_index !== undefined) {
...
async function initBernieReview() {
  ...
  if (!payload && !isDevFixture) {
    ...
    } else if (reviewParam === "confirmation_ready_stale") {
      payload = {
        ...mockBernieReviewConfirmationReady,
        confirm_affordance: {
          can_show_confirm_ui: false,
          gate: "blocked_stale",
          blocking_reason_codes: ["stale_proposal"]
        },
        blocks: [
          { code: "stale_proposal", message: "Stale proposal; please search again." }
        ],
        confirm_endpoint: null,
        confirm_payload: null
      };
...
Office.onReady(() => {
  ...
  document.getElementById("btn-today").onclick     = () => {
    diaryDate = new Date();
    updateDateLabel();
    clearStaleBernieBookingState("today_click");
    loadDiary();
  };
  document.getElementById("btn-now").onclick       = jumpToNow;
  document.getElementById("btn-refresh").onclick   = doRefresh;
  const bernieNewSessionButton = document.getElementById("btn-bernie-new-session");
  if (bernieNewSessionButton) {
    bernieNewSessionButton.onclick = () => bernieSession.startNewSession();
  }
  const bernieAutoPreviewToggle = document.getElementById("bernie-auto-preview-toggle");
  if (bernieAutoPreviewToggle) {
    bernieAutoPreviewToggle.checked = bernieAutoPreview;
    bernieAutoPreviewToggle.onchange = e => {
      bernieAutoPreview = Boolean(e.target.checked);
      if (!bernieAutoPreview) {
        bernieSession.autoPreviewCandidateKey = null;
        bernieStagedBookingPreview = null;
        bernieStagedBookingFresh = false;
        loadDiary(true);
  ...
  if (dateWrapper && datePicker) {
    ...
    datePicker.onchange = (e) => {
      if (e.target.value) {
        ...
        updateDateLabel();
        clearStaleBernieBookingState("date_picker");
        loadDiary();
      }
    };
  }
