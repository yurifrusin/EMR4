...
| Confirmation evidence is reconstructed or weakened | Preserve server-generated confirm payload opaquely and alter only the existing confirmation acknowledgement fields (`confirmed` plus exact warning codes); use the existing distinct confirmation idempotency key and proposal-supplied allowlisted endpoint. |
