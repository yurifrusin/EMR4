...
| A future hard event is allowed but omitted from the per-event source map | The core rejects every hard event whose configured five-source policy is empty, independently of YAML cross-file tests | Receipt is `revision_required`; no high-authority action |
...
| Windows line-ending conversion changes the same settings fingerprint between worktrees | Normalize YAML newlines to LF before hashing filenames and content; test LF and CRLF equality | Cross-worktree fingerprint test fails; candidate cannot reach review |
