...
| A practitioner becomes inactive after the client recheck | When the target differs from current appointment truth, the existing update proposal adds a typed `practitioner_inactive` block from current same-practice database truth; confirmation re-runs the proposal before writing. |
...
| Arbitrary practitioner id is injected into the command | Resolve only the exact freshly admitted directory id into the target column/reference, pass a frozen admitted identity to the shared composer and reject any column/admission mismatch; reject same, blank, oversized, unlisted and duplicate targets with zero proposal. |
...
This remains a use of the existing REST update command family. GraphQL stays read-only.
Existing proposal and confirmation schemas already admit
`practitioner_id`. The proposal now distinguishes an unchanged historical
practitioner from a changed target and requires the latter to be currently
active; confirmation re-runs that same check. Actor/practice authority,
schedule conflicts, freshness, version, idempotency, audit and atomic commit
remain backend owned. No new route, schema, OpenAPI or generated-client change
is required.
