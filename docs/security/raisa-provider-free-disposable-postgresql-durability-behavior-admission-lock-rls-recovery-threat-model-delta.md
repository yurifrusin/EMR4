# Threat-model delta: admission-lock RLS recovery
...
privileges, immutable-admission invariant and behavior scenario population do
not change.
...
| Threat | Control |
|---|---|
| A legitimate coordinator cannot lock an existing proofread admission and reports false absence | Add exact COORDINATOR `USING` visibility for the contracted row lock |
| Lock visibility accidentally creates admission mutation authority | Use UPDATE policy only for PostgreSQL lock semantics, retain zero direct DML, and require `WITH CHECK` to repeat the exact binding predicate followed by `AND FALSE` |
