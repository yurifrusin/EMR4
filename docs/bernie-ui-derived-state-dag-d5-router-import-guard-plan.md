...
Status: static plan only. The current production-router import ban remains in
force while the D5 response-delivery gate is blocked.

Current guard:
`tests/test_bernie_ui_view_model.py::test_production_routes_do_not_import_selector_yet`
asserts that no `app/routers/*.py` file imports or references
`app.services.bernie.ui_view_model`.

## If D5 Is Approved

Do not delete the guard. Replace it with a finer-grained guard that:
...
D5 gate commit. Until that approval exists, the broad no-router-import guard
continues to be the correct test.
