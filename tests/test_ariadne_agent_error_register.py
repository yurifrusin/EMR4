...
def test_register_is_valid_after_durability_schema_recovery() -> None:
    ...
    assert register["register_revision"] == 50
...
def test_migration_architecture_plan_veto_and_review_bootstrap_are_preserved() -> None:
    ...
    assert "immutable PRIMARY plus one immutable CONFLICT" in plan["correction"][
        "action"
    ]
    assert "before or after source purge" in plan["correction"]["action"]
