...
def test_pattern_report_detects_recurring_control_signals() -> None:
    ...
    assert report["incident_count"] == len(_register()["incidents"])
...
def test_aer_0184_records_input_column_ambiguity_and_collision_proof_lowering() -> None:
    ...
    assert report["register_revision"] == register["register_revision"]
    assert report["incident_count"] == len(incidents)
    assert report["open_incident_ids"] == []
    assert report["counts"]["by_origin"] == dict(
        sorted(Counter(row["origin"] for row in incidents).items())
    )
    assert report["counts"]["by_category"] == dict(
        sorted(Counter(row["category"] for row in incidents).items())
    )
    assert report["counts"]["by_candidate_state"] == dict(
        sorted(Counter(row["candidate_state"] for row in incidents).items())
    )
