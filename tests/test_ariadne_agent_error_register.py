...
def test_register_is_valid_after_durability_schema_recovery() -> None:
    ...
    assert register["register_revision"] == 571
...
def test_seed_separates_agent_behavior_from_transport() -> None:
    ...
    assert len(agent_incidents) == 447
...
def test_aer_0184_records_input_column_ambiguity_and_collision_proof_lowering() -> None:
    ...
    report = build_pattern_report()
    assert report["register_revision"] == 571
    assert report["incident_count"] == 661
    ...
    assert register_metadata_recurrence["incident_count"] == 10
    ...
    assert closeout_fixture_recurrence["incident_ids"] == [
        "AER-0319",
        "AER-0402",
        "AER-0599",
        "AER-0600",
    ]
    assert closeout_fixture_recurrence["incident_count"] == 4
    ...
    assert baton_prose_recurrence["incident_count"] == 4
    ...
    assert population_fixture_recurrence["incident_count"] == 13
    ...
    assert compass_sentinel_recurrence["incident_count"] == 6
    ...
    assert repeated_patch_recurrence["incident_count"] == 6
    ...
    assert checkpoint_bound_recurrence["incident_ids"] == ["AER-0460", "AER-0512", "AER-0655"]
    assert checkpoint_bound_recurrence["incident_count"] == 3
    ...
    peer_link_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.register_related_incident_peer_link_not_symmetric"
    )
    assert peer_link_recurrence["incident_ids"] == ["AER-0564", "AER-0577"]
    assert peer_link_recurrence["incident_count"] == 2
    utf8_read_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "operator.python_path_read_text_utf8_encoding_omitted"
    )
    assert utf8_read_recurrence["incident_ids"] == ["AER-0548", "AER-0575"]
    assert utf8_read_recurrence["incident_count"] == 2
    path_operand_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "operator.explicit_repository_path_operand_not_inventory_resolved"
    )
    assert path_operand_recurrence["incident_ids"] == [
        "AER-0570",
        "AER-0586",
        "AER-0607",
        "AER-0611",
    ...
    assert path_operand_recurrence["incident_count"] == 6
    adapter_inventory_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.preplanning_adapter_observation_inventory_incomplete"
    )
    assert adapter_inventory_recurrence["incident_ids"] == [
        "AER-0517",
        "AER-0612",
    ]
    assert adapter_inventory_recurrence["incident_count"] == 2
    ...
    branch_prefix_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.verifier_preflight_branch_prefix_not_exact_review_prefix"
    )
    assert branch_prefix_recurrence["incident_ids"] == ["AER-0392", "AER-0592"]
    assert branch_prefix_recurrence["incident_count"] == 2
    generative_review_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.generative_acceptance_in_read_only_review_worktree"
    )
    assert generative_review_recurrence["incident_ids"] == ["AER-0040", "AER-0594"]
    assert generative_review_recurrence["incident_count"] == 2
    antigravity_egress_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "harness.antigravity_completed_transport_invalid_egress_without_digest_receipt"
    )
    assert antigravity_egress_recurrence["incident_ids"] == [
        "AER-0424",
        "AER-0595",
    ]
    assert antigravity_egress_recurrence["incident_count"] == 2
    pre_verifier_disposition_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.pre_verifier_parallelism_disposition_and_workspace_receipt_shape_invalid"
    )
    assert pre_verifier_disposition_recurrence["incident_ids"] == [
        "AER-0320",
        "AER-0597",
    ]
    assert pre_verifier_disposition_recurrence["incident_count"] == 2
    regression_literal_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.agent_error_register_regression_assertion_literal_not_canonical"
    )
    assert regression_literal_recurrence["incident_ids"] == [
        "AER-0602",
        "AER-0605",
    ]
    assert regression_literal_recurrence["incident_count"] == 2
    yielded_session_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
    ...
    no_database_runner_recurrence = next(
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        == "orchestrator.no_database_tranche_selected_database_backed_test_suite"
    )
    assert no_database_runner_recurrence["incident_ids"] == [
        "AER-0658",
        "AER-0659",
    ]
    assert no_database_runner_recurrence["incident_count"] == 2
    assert [
        row
        for row in report["recurring_patterns"]
        if row["recurrence_signature"]
        ...
        },
        {
            ...
            "prevention_controls": [
                ...
                "The clockwork reducer must derive new recurrence classifications and residual exclusions from one generated set; tests must not discover them sequentially through failing comparisons.",
                "The clockwork reducer must emit recurrence classification and exhaustive residual checks from the same generated pattern set, so a new recurrence cannot require two manually synchronized fixtures.",
                "The clockwork reducer must emit recurrence rows in deterministic canonical order and tests must consume that projection without independently ordering its narrative fields.",
                ...
                "The clockwork reducer must own contiguous IDs, populations, aggregate dictionaries, recurrence rows and the committed report as projections of one journal reading.",
                "The clockwork reducer must own contiguous IDs, standalone and aggregate populations, recurrence classifications, exact recurrence rows and the committed report as projections of one journal reading.",
            ],
        },
        {
...
def test_aer_0565_advances_source_cutoff_after_date_rollover() -> None:
    ...
    assert register["scope"]["source_cutoff_on"] == "2026-08-20"
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
...
def test_aer_0575_recurrence_moves_utf8_loading_into_typed_path() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0575"]

    assert list(incidents)[574:575] == ["AER-0575"]
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["related_incident_ids"] == []
    assert incident["recurrence_signature"] == (
    ...
    assert "typed Continuity path" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0576_resolves_incident_stage_from_schema_enum() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0576"]

    assert list(incidents)[575:576] == ["AER-0576"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["stage"] == "closeout"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_enum_value_not_schema_resolved"
    )
    assert "schema-enumerated" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0577_derives_recurrence_without_one_way_peer_link() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0577"]

    assert list(incidents)[576:577] == ["AER-0577"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["related_incident_ids"] == []
    assert incident["recurrence_signature"] == (
    ...
    assert "derive the historical grouping" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0578_uses_provider_free_launcher_command_grammar() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0578"]

    assert list(incidents)[577:578] == ["AER-0578"]
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["resource_id"] == "ariadne-provider-free-pytest-launcher"
    assert incident["recurrence_signature"] == (
        "operator.provider_free_pytest_unsupported_pytest_flag"
    )
    assert "typed launcher profile" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0579_derives_every_register_projection_from_one_reading() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0579"]

    assert list(incidents)[578:579] == ["AER-0579"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_population_fixture_update_incomplete"
    )
    assert "one journal reading" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0580_derives_recurrence_classification_and_residual_fixture() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0580"]

    assert list(incidents)[579:580] == ["AER-0580"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_population_fixture_update_incomplete"
    )
    assert "two manually synchronized fixtures" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0581_targets_repeated_count_by_recurrence_identity() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0581"]

    assert list(incidents)[580:581] == ["AER-0581"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "command_scope_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.apply_patch_repeated_value_target_missing_incident_context"
    )
    assert "stable recurrence identity" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0582_consumes_deterministically_ordered_recurrence_projection() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0582"]

    assert list(incidents)[581:582] == ["AER-0582"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_population_fixture_update_incomplete"
    )
    assert "deterministic canonical order" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0583_allocates_unique_attempt_identity() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0583"]

    assert list(incidents)[582:583] == ["AER-0583"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_attempt_id_not_unique"
    )
    assert "allocate attempt identities monotonically" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0584_targets_repeated_attempt_id_by_incident_identity() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0584"]

    assert list(incidents)[583:584] == ["AER-0584"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "command_scope_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.apply_patch_repeated_value_target_missing_incident_context"
    )
    assert "addressed by incident identity" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0585_derives_recurrence_classification_from_one_set() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0585"]

    assert list(incidents)[584:585] == ["AER-0585"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_population_fixture_update_incomplete"
    )
    assert "one generated set" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0586_resolves_explicit_test_path_before_search() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0586"]

    assert list(incidents)[585:586] == ["AER-0586"]
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["recurrence_signature"] == (
        "operator.explicit_repository_path_operand_not_inventory_resolved"
    )
    assert "typed manifest" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0587_rebinds_all_compass_current_position_sentinels() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0587"]

    assert list(incidents)[586:587] == ["AER-0587"]
    assert incident["origin"] == "repository"
    assert incident["category"] == "repository_defect"
    assert incident["recurrence_signature"] == (
        "repository.compass_current_position_literal_stale_after_valid_advance"
    )
    assert "prospective projection" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0588_renders_unambiguous_baton_boundaries() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0588"]

    assert list(incidents)[587:588] == ["AER-0588"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.current_baton_prose_assertion_representation_mismatch"
...
def test_aer_0592_uses_exact_configured_review_prefix() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0592"]

    assert list(incidents)[591:592] == ["AER-0592"]
    assert incident["related_incident_ids"] == ["AER-0593"]
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.verifier_preflight_branch_prefix_not_exact_review_prefix"
    )
    assert "typed execution-policy enum" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0593_binds_serial_runner_to_exact_review_root() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0593"]

    assert list(incidents)[592:593] == ["AER-0593"]
    assert incident["related_incident_ids"] == ["AER-0592"]
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.verifier_preflight_serial_repo_root_not_review_worktree"
    )
    assert "one typed review-worktree receipt" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0594_excludes_generators_from_read_only_verifier_ticks() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0594"]

    assert list(incidents)[593:594] == ["AER-0594"]
    assert incident["related_incident_ids"] == []
    assert incident["category"] == "read_only_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.generative_acceptance_in_read_only_review_worktree"
    )
    assert "filesystem effect" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0595_persists_dirty_worktree_egress_failure() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0595"]

    assert list(incidents)[594:595] == ["AER-0595"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "harness"
    assert incident["category"] == "harness_failure"
    assert incident["recurrence_signature"] == (
    ...
    assert "worktree postcondition failures" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0596_separates_cross_role_attempt_identity() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0596"]

    assert list(incidents)[595:596] == ["AER-0596"]
    assert incident["related_incident_ids"] == []
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_split_attempt_role_identity"
    )
    assert "split attempt identity mismatch" in incident["detection_method"]
    assert "peer eligibility" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0597_uses_reserved_pre_verifier_disposition() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0597"]

    assert list(incidents)[596:597] == ["AER-0597"]
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.pre_verifier_parallelism_disposition_and_workspace_receipt_shape_invalid"
    )
    assert "parallelism_lane_disposition_invalid" in incident["detection_method"]
    assert "configured enum" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0598_requires_materialized_correction_evidence() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0598"]

    assert list(incidents)[597:598] == ["AER-0598"]
    assert incident["category"] == "output_contract_violation"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_correction_evidence_not_materialized"
    )
    assert "missing or not a file" in incident["detection_method"]
    assert "already-materialized" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0599_separates_immutable_receipts_from_mutable_latch_projection() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0599"]

    assert list(incidents)[598:599] == ["AER-0599"]
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["recurrence_signature"] == (
    ...
    assert "immutable event receipts" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0600_removes_historical_identity_from_completed_latch_fallback() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0600"]

    assert list(incidents)[599:600] == ["AER-0600"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "terminal event schema" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0601_separates_peer_links_from_recurrence_membership() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0601"]

    assert list(incidents)[600:601] == ["AER-0601"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["recurrence_signature"] == (
        "orchestrator.agent_error_register_cross_attempt_peer_linkage_invalid"
    )
    assert "never use related_incident_ids" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0602_binds_regression_assertion_to_canonical_control_text() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0602"]

    assert list(incidents)[601:602] == ["AER-0602"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "exact stored controls" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0603_derives_historical_shadow_latch_from_immutable_manifest() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0603"]

    assert list(incidents)[602:603] == ["AER-0603"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "repository"
    assert incident["category"] == "repository_defect"
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["recurrence_signature"] == (
        "repository.immutable_shadow_fixture_bound_to_mutable_live_latch"
    )
    assert "immutable manifest" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0604_measures_live_handover_compactness_before_publication() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0604"]

    assert list(incidents)[603:604] == ["AER-0604"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "repository"
    assert incident["category"] == "repository_defect"
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["recurrence_signature"] == (
        "repository.live_handover_exceeds_compactness_guard"
    )
    assert "measure split-line count before publication" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0605_binds_regression_assertion_to_exact_stored_control() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0605"]

    assert list(incidents)[604:605] == ["AER-0605"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
...
def test_aer_0606_projects_all_register_populations_from_one_reading() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0606"]

    assert list(incidents)[605:606] == ["AER-0606"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "standalone and aggregate populations" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0607_resolves_every_command_path_before_dispatch() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0607"]

    assert list(incidents)[606:607] == ["AER-0607"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "typed manifest before dispatch" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0608_projects_every_navigation_fixture_from_one_tick() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0608"]

    assert list(incidents)[607:608] == ["AER-0608"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "repository"
    assert incident["category"] == "repository_defect"
    assert incident["candidate_state"] == "untrusted_partial_worktree"
...
def test_aer_0609_derives_the_complete_recurrence_composite() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0609"]

    assert list(incidents)[608:609] == ["AER-0609"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "canonical pattern identity" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0610_projects_standalone_and_aggregate_populations_together() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0610"]

    assert list(incidents)[609:610] == ["AER-0610"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "canonical_unchanged"
...
def test_aer_0611_uses_an_inventoried_root_and_native_glob_filter() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0611"]

    assert list(incidents)[610:611] == ["AER-0611"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "native glob filter" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0612_preserves_the_missing_adapter_receipt_rejection() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0612"]

    assert list(incidents)[611:612] == ["AER-0612"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "agent_behavior"
    assert incident["category"] == "output_contract_violation"
    assert incident["candidate_state"] == "untrusted_partial_worktree"
    ...
    assert "complete configured adapter ID set" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0613_prohibits_guessed_sibling_path_operands() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0613"]

    assert list(incidents)[612:613] == ["AER-0613"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "prohibit guessed sibling operands" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0614_inventories_latch_tests_before_exact_inspection() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0614"]

    assert list(incidents)[613:614] == ["AER-0614"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "operator"
    assert incident["category"] == "operator_error"
    assert incident["candidate_state"] == "canonical_unchanged"
    ...
    assert "discover matching test inventory" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0615_projects_latch_source_from_lifecycle_status() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    incident = incidents["AER-0615"]

    assert list(incidents)[614:615] == ["AER-0615"]
    assert incident["related_incident_ids"] == []
    assert incident["origin"] == "repository"
    assert incident["category"] == "repository_defect"
    assert incident["candidate_state"] == "untrusted_partial_worktree"
    ...
    assert "Lifecycle-sensitive latch tests" in (
        incident["correction"]["prevention_control"]
    )
    assert incident["status"] == "corrected"


def test_aer_0616_through_0629_preserve_clockwork_construction_reruns() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    assert list(incidents)[615:629] == [
        "AER-0616",
        "AER-0617",
        "AER-0618",
        "AER-0619",
    ...
    readback = incidents["AER-0616"]
    assert readback["origin"] == "repository"
    assert readback["category"] == "repository_defect"
    assert "journal.json" in readback["observed_error"]

    efficacy = incidents["AER-0617"]
    assert efficacy["origin"] == "agent_behavior"
    assert efficacy["category"] == "evidence_misreport"
    assert "100 percent" in efficacy["observed_error"]
    assert "timing" in efficacy["correction"]["prevention_control"]

    relation = incidents["AER-0618"]
    assert relation["category"] == "output_contract_violation"
    assert "category-to-origin" in relation["expected_invariant"]

    search = incidents["AER-0619"]
    assert search["origin"] == "operator"
    assert search["category"] == "operator_error"
    assert "fixed-string" in search["correction"]["action"]
    output = incidents["AER-0620"]
    boundary = incidents["AER-0621"]
    identity = incidents["AER-0622"]
    peers = incidents["AER-0623"]
    causal = incidents["AER-0624"]
    rejected_tests = incidents["AER-0625"]
    closeout_search = incidents["AER-0626"]
    continuity_status = incidents["AER-0627"]
    final_packet = incidents["AER-0628"]
    boundary_projection = incidents["AER-0629"]
    assert "--output" in output["correction"]["prevention_control"]
    assert "function boundary" in boundary["detection_method"]
    assert output["related_incident_ids"] == []
    assert boundary["related_incident_ids"] == []
    assert output["attempt_id"] != boundary["attempt_id"]
    assert "resource_id mismatch" in identity["detection_method"]
    assert "attempt-peer linkage mismatch" in peers["detection_method"]
    assert causal["origin"] == "repository"
    assert causal["process_severity"] == "material"
    assert "acknowledged event tip" in causal["observed_error"]
    assert "8 <= 7" in rejected_tests["detection_method"]
    assert closeout_search["recurrence_signature"] == search["recurrence_signature"]
    assert continuity_status["category"] == "output_contract_violation"
    assert "DECISION_STATES" in continuity_status["detection_method"]
    assert "four stale projections" in final_packet["observed_error"]
    assert "mutable-current-fixtures" in boundary_projection["expected_invariant"]
    assert all(
...
def test_aer_0641_preserves_non_publish_review_mutation_rejection() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    runner = incidents["AER-0641"]
    assert list(incidents)[640:641] == ["AER-0641"]
    assert runner["origin"] == "repository"
    assert runner["stage"] == "independent_review"
    assert runner["category"] == "repository_defect"
    assert runner["workflow_disposition"] == "review_rejected"
    assert runner["recurrence_signature"] == (
        "repository.non_publish_runner_rewrote_review_evidence"
    )
    assert runner["correction"]["status"] == "corrected_fresh_attempt"
    assert runner["status"] == "corrected"


def test_aer_0642_preserves_causal_claim_enum_correction() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    causal = incidents["AER-0642"]
    assert list(incidents)[641:642] == ["AER-0642"]
    assert causal["origin"] == "agent_behavior"
    assert causal["category"] == "output_contract_violation"
    assert causal["recurrence_signature"] == (
        "orchestrator.agent_error_register_causal_claim_enum_not_schema_resolved"
    )
    assert causal["causal_claim_level"] == "observation_only"
    assert causal["correction"]["status"] == "corrected_fresh_attempt"
    assert causal["status"] == "corrected"


def test_aer_0643_and_0644_preserve_closeout_fixture_corrections() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    latch = incidents["AER-0643"]
    compass = incidents["AER-0644"]
    assert list(incidents)[642:644] == ["AER-0643", "AER-0644"]
    assert latch["recurrence_signature"] == (
        "repository.active_operation_completed_stage_exceeded_schema_bound"
    )
    assert compass["recurrence_signature"] == (
        "repository.compass_current_position_fixture_stale_after_accepted_advance"
    )
    assert latch["category"] == compass["category"] == "repository_defect"
    assert latch["correction"]["status"] == "corrected_fresh_attempt"
    assert compass["correction"]["status"] == "corrected_fresh_attempt"
    assert latch["status"] == compass["status"] == "corrected"


def test_aer_0645_preserves_terminal_latch_replay_decoupling() -> None:
    replay = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0645"
    ]
    assert replay["origin"] == "repository"
    assert replay["category"] == "repository_defect"
    assert replay["recurrence_signature"] == (
        "repository.historical_replay_coupled_to_terminal_live_latch"
    )
    assert replay["correction"]["status"] == "corrected_fresh_attempt"
    assert replay["status"] == "corrected"


def test_aer_0646_preserves_budgeted_replay_correction() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0646"
    ]
    assert incident["origin"] == "repository"
    assert incident["recurrence_signature"] == (
        "repository.test_only_replay_correction_exceeded_frozen_line_budget"
    )
    assert "849 lines" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0647_preserves_exact_pytest_selector_correction() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0647"
    ]
    assert incident["origin"] == "agent_behavior"
...
def test_aer_0648_completes_register_origin_count_update() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0648"
    ]
    assert incident["origin"] == "repository"
    ...
    assert "435" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0649_and_0650_complete_register_recurrence_and_diagnostic_shape() -> None:
    incidents = {row["incident_id"]: row for row in _register()["incidents"]}
    recurrence = incidents["AER-0649"]
    diagnostic = incidents["AER-0650"]
    assert recurrence["recurrence_signature"] == (
        "repository.agent_error_register_exact_count_update_incomplete"
    )
    assert diagnostic["recurrence_signature"] == (
        "orchestrator.pattern_report_row_shape_assumed_without_inspection"
    )
    assert recurrence["status"] == diagnostic["status"] == "corrected"


def test_aer_0651_preserves_yielded_session_evidence_correction() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0651"
    ]
    assert incident["origin"] == "agent_behavior"
    ...
    assert "final exit code" in incident["expected_invariant"]
    assert incident["status"] == "corrected"


def test_aer_0652_requires_portable_verifier_manifests() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0652"
    ]
    assert incident["origin"] == "agent_behavior"
    assert incident["recurrence_signature"] == (
        "orchestrator.verifier_manifest_depended_on_untracked_historical_evidence"
    )
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["workflow_disposition"] == "review_rejected"
    assert "tracked-only portability" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0653_decouples_historical_replay_from_live_latch() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0653"
    ]
    assert incident["recurrence_signature"] == (
        "orchestrator.accepted_rehearsal_replay_depended_on_mutable_live_latch"
    )
    assert incident["related_incident_ids"] == []
    assert "immutable full Git commit" in incident["correction"]["prevention_control"]
    assert incident["status"] == "corrected"


def test_aer_0654_preserves_yielded_session_recurrence() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0654"
    ]
    assert incident["recurrence_signature"] == (
    ...
    assert incident["workflow_disposition"] == "revision_required"
    assert "session_id" in incident["correction"]["action"]
    assert incident["status"] == "corrected"


def test_aer_0655_preserves_bounded_terminal_checkpoint_recurrence() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0655"
    ]
    assert incident["recurrence_signature"] == (
    ...
    assert "500-character" in incident["expected_invariant"]
    assert incident["status"] == "corrected"


def test_aer_0656_preserves_repeated_value_patch_recurrence() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0656"
    ]
    assert incident["recurrence_signature"] == (
    ...
    assert "AER-0556" in incident["observed_error"]
    assert incident["status"] == "corrected"


def test_aer_0657_preserves_provider_free_manifest_fixture_mismatch() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0657"
    ]
    assert incident["recurrence_signature"] == (
        "orchestrator.provider_free_verifier_manifest_included_conftest_database_suite"
    )
    assert "--noconftest" in incident["expected_invariant"]
    assert "36 setup errors" in incident["observed_error"]
    assert incident["workflow_disposition"] == "review_rejected"
    assert incident["status"] == "corrected"


def test_aer_0658_contains_out_of_scope_database_backed_verification() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0658"
    ]
    assert incident["recurrence_signature"] == (
    ...
    assert "shared PostgreSQL schema" in incident["observed_error"]
    assert incident["related_incident_ids"] == []
    assert incident["correction"]["status"] == "control_added"
    assert incident["status"] == "contained"


def test_aer_0659_contains_immediate_database_runner_recurrence() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0659"
    ]
    assert incident["recurrence_signature"] == (
    ...
    assert "immediately following" in incident["observed_error"]
    assert "engine-enforced" in incident["correction"]["prevention_control"]
    assert incident["status"] == "contained"


def test_aer_0660_preserves_clockwork_incident_identifier_case_rejection() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0660"
    ]
    assert incident["recurrence_signature"] == (
        "orchestrator.transaction_manifest_incident_identifier_used_human_case"
    )
    assert "incident_id_invalid" in incident["observed_error"]
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["workflow_disposition"] == "revision_required"
    assert incident["status"] == "corrected"


def test_aer_0661_preserves_clockwork_direct_executable_rejection() -> None:
    incident = {row["incident_id"]: row for row in _register()["incidents"]}[
        "AER-0661"
    ]
    assert incident["recurrence_signature"] == (
        "orchestrator.clockwork_command_manifest_used_direct_tool_executable"
    )
    assert "tick_command_contract" in incident["observed_error"]
    assert ".venv/Scripts/python.exe -m ruff" in incident["correction"]["action"]
    assert incident["candidate_state"] == "canonical_unchanged"
    assert incident["status"] == "corrected"
