...
def test_register_is_valid_after_durability_schema_recovery() -> None:
    ...
    assert register["register_revision"] == 143
    assert register["scope"]["coverage"] == "bounded_known_preserved_incidents"
    assert [row["incident_id"] for row in register["incidents"]] == [
        f"AER-{index:04d}" for index in range(1, 169)
    ]
    assert [
...
def test_pattern_report_detects_recurring_control_signals() -> None:
    ...
    assert report["incident_count"] == 168
    ...
    assert report["counts"]["by_category"] == {
        ...
        "repository_defect": 27,
        "transport_timeout": 8,
    }
    assert report["counts"]["by_candidate_state"] == {
        "accepted_candidate_changed": 46,
