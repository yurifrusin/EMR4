...
def test_register_is_valid_after_durability_schema_recovery() -> None:
    ...
    assert register["register_revision"] == 366
    assert register["scope"]["coverage"] == "bounded_known_preserved_incidents"
    assert [row["incident_id"] for row in register["incidents"]] == [
        f"AER-{index:04d}" for index in range(1, 418)
    ]
    assert [
...
def test_seed_separates_agent_behavior_from_transport() -> None:
    ...
    assert len(agent_incidents) == 290
...
def test_pattern_report_detects_recurring_control_signals() -> None:
    ...
    assert report["incident_count"] == 417
...
def test_aer_0184_records_input_column_ambiguity_and_collision_proof_lowering() -> None:
    ...
    report = build_pattern_report()
    assert report["register_revision"] == 366
    assert report["incident_count"] == 417
    ...
    assert report["counts"]["by_category"] == {
        ...
        "evidence_misreport": 56,
        "harness_failure": 48,
        "output_contract_violation": 125,
    ...
    assert manual_sha_recurrence["incident_ids"] == ["AER-0286", "AER-0399"]
    assert manual_sha_recurrence["incident_count"] == 2
