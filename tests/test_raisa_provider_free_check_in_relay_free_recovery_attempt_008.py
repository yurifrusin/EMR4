...
def _bind_terminal_paths(
    ...
    envelope = root / "attempt-008-execution-envelope.json"
...
def test_static_admission_binds_repaired_controls_before_database() -> None:
    ...
    assert result["hard_condition_ids"] == [
        "P06", "P07", "P08", "P09", "P10", "P11", "P12", "P13", "P14"
    ]
    assert result["repaired_base_harness_sha256"] == (
...
def test_hash_bindings_are_exact_and_accepted_sources_remain_immutable() -> None:
...
def test_repaired_start_vector_is_exact_and_unsupported_option_absent() -> None:
    source = Path(attempt.accepted.__file__).read_text(encoding="utf-8")
    normalized = " ".join(source.split())
    assert '"start", "--attach", "--interactive", container_id' in normalized
    assert '"--sig-proxy=false"' not in source
...
def test_terminal_binding_context_restores_after_exception() -> None:
    ...
    with pytest.raises(RuntimeError):
        with attempt._attempt_008_terminal_bindings():
...
def test_envelope_schema_is_closed_non_retriable_and_typed() -> None:
    ...
    assert list(Draft202012Validator(schema).iter_errors(hostile))
    finalized = copy.deepcopy(example)
    finalized["cleanup_status"] = "cleanup_verified"
    finalized["finalized_cleanup_projection_preserved"] = False
    assert list(Draft202012Validator(schema).iter_errors(finalized))
...
def test_pass_routes_exact_terminal_and_restores_bindings(
    ...
    assert result["finalized_cleanup_projection_preserved"] is True
...
def test_base_owned_failure_retains_finalized_cleanup(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    envelope, attestation, evidence, failure = _bind_terminal_paths(
        monkeypatch, tmp_path
    ...
    def run() -> tuple[dict[str, object], None]:
        payload: dict[str, object] = {
            "result": "failed_closed",
            "stage": "redaction",
            "code": "failure_evidence_rejected",
            "cleanup": {"status": "cleanup_verified"},
            "retry_count": 0,
...
def test_preexecution_controller_failure_is_sanitized_and_not_finalized(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    envelope, attestation, evidence, failure = _bind_terminal_paths(
        monkeypatch, tmp_path
    ...
    monkeypatch.setattr(
        attempt,
        "static_check",
        lambda **_: {"source_head": "d" * 40, "status": "passed"},
    )
    monkeypatch.setattr(
        attempt.accepted,
        "run_rehearsal",
    ...
    assert result["finalized_cleanup_projection_preserved"] is False
    assert terminal["stage"] == "attempt_008_execution"
...
def test_cli_exposes_only_check_and_execute() -> None:
    ...
    assert '"resume_count": 0' in source
