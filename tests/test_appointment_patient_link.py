...
def test_update_cannot_clear_all_patient_identity(client, gp_user, practitioner, schedule):
    """PUT cannot leave an appointment without patient_id or provisional name."""
    token = make_token(gp_user)
    create = client.post(
        "/api/v1/appointments",
        json={
            "patient_name_provisional": "Temporary Identity",
            "practitioner_id": str(practitioner.id),
            "start_time": _start(15, 45),
            "duration_minutes": 15,
        },
        headers={"Authorization": f"Bearer {token}"},
    )
    ...
    resp = client.put(
        f"/api/v1/appointments/{appt_id}",
        json={"patient_id": None, "patient_name_provisional": None},
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 422
