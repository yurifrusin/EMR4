...
def test_compass_advances_to_observability_first_cf_d2_plan() -> None:
    compass = _load("orchestration/continuity/emr4-compass.json")
    ...
    assert "command-time authority and source-truth" in unlocks
    assert "restart or unknown-commit" in limits
    assert "no watcher runtime" in compass["orientation_statement"].lower()
