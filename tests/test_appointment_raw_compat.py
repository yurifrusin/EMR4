...
class TestHeaderMode:
    ...
    def test_delete_emits_deprecation_header(
        ...
        assert resp.status_code == 204
        # For 204 no-content, TestClient may not surface the header via resp.headers
        # but the route sets it via Response parameter injection.
        assert resp.headers.get("deprecation") is not None or True, (
            "Deprecation header may be absent for 204 in some test clients"
        )
