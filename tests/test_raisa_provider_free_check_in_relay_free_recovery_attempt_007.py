...
def _bind_terminal_paths(
    ...
    envelope = root / "attempt-007-execution-envelope.json"
...
def test_static_admission_binds_repaired_start_and_hostile_gates() -> None:
...
def test_terminal_binding_context_restores_after_exception() -> None:
    ...
    with pytest.raises(RuntimeError):
        with attempt._attempt_007_terminal_bindings():
...
def test_unexpected_controller_failure_routes_sanitized_terminal_once(
    ...
    assert result["automatic_retry_count"] == 0
    assert terminal["stage"] == "attempt_007_execution"
