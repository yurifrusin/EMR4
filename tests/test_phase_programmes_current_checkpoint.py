...
def test_phase_programmes_recommended_move_is_current_after_sprint_154():
    text = PHASE_PROGRAMMES.read_text(encoding="utf-8")

    assert "Current position after Sprint 154" in text
    assert "default recommendation after Sprint 154 is **Programme 2G**" in text
    ...
    assert "Sprint 155" in text
    assert "wire create-confirm and confirm-Bernie" in text
    assert "provider-boundary guard stack has been consolidated" in text
    assert (
        "Status | Sprint 154 diary/API header-gap preflight "
        "completed; sprint engine continuing" in text
    )
    assert "Sprint 110-118 provider-boundary guard consolidation" in text
    assert (
        "Next Candidate Sprints | Sprint 155 create-confirm/Bernie-confirm "
