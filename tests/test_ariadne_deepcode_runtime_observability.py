...
import orchestration_harness.deepcode_artifact as deepcode_artifact
from orchestration_harness.deepcode_artifact import parse_artifact_marker
from scripts.ariadne_deepcode_liveness import (
    _process_state,
    capture_snapshot,
    classify_liveness,
)
...
def test_terminal_marker_parser_rejects_prose_earlier_conflicts_and_incomplete_artifacts():
    assert (
        parse_artifact_marker(
            "The expected STATUS: complete is documented here.\n", "completion"
        )["valid"]
        is False
    )
    assert (
        parse_artifact_marker("STATUS: complete\nSTATUS: failed\n", "completion")[
            "valid"
        ]
        is False
    )
...
def test_deepcode_adapter_contains_no_independent_authoritative_grammar():
    source = inspect.getsource(deepcode_artifact)

    assert "parse_artifact_verdict" in source
    assert "re.compile" not in source
    assert "DECISION: PASS" not in source
    assert "STATUS: COMPLETE" not in source


def test_revision_required_is_complete_for_liveness_without_integration_authority(
    tmp_path: Path,
):
    ...
    snapshot = capture_snapshot(
        tmp_path, Path("artifact.md"), Path("receipt.json"), Path("outbox")
    )
    classification = classify_liveness(None, snapshot)

    assert classification["status"] == "completed"
    assert snapshot["artifact"]["artifact_valid"] is True
    assert snapshot["artifact"]["review_verdict"] == "revision_required"
    assert snapshot["artifact"]["integration_authorized"] is False


def test_liveness_elapsed_time_does_not_classify_unchanged_state_as_failure(
    tmp_path: Path,
):
...
def test_liveness_changed_file_and_artifact_signals_mean_progress_or_completion(
    tmp_path: Path,
):
...
def test_transcript_is_redacted_and_bounded(tmp_path: Path):
    ...
    assert (
        receipt["terminal_transcript"]["byte_truncated"] is True
        or receipt["terminal_transcript"]["event_count_truncated"] is True
    )
