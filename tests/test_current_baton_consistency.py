...
def test_live_baton_rows_accept_reorientation_and_keep_cf_d2_deferred() -> None:
    ...
    assert "Continuity 254 / Compass 236" in current
    assert "zero product/runtime/import/recovery/migration/operational" in current
    assert "126 conformance call expressions in 21 test/review files" in current
    assert "four separate direct database fixture obligations" in current
    assert "unknown external-consumer population" in current
    assert "All four raw routes remain mounted" in current
    assert "Status confirm-first" in current
    assert "45 classified stale-harness failures" in current
    assert "9c7444ecce69b51ca5cac80818e8997724a11f13" in current
    ...
    assert (
        "Provider-free compatibility conformance-harness temporal/idempotency readiness repair"
        in next_work
    )
    assert "eight named files" in next_work
    assert "all 311 ordinary compatibility tests green" in next_work
    assert (
        "without changing application code, route behavior, expected semantics"
        in next_work
    )
...
def test_current_rows_preserve_closed_surface_boundary() -> None:
    ...
    for phrase in (
        "provider-free compatibility conformance-harness temporal/idempotency readiness repair",
        "test-only deterministic fixture corrections",
        "all 311 ordinary compatibility tests green",
