...
NODE_ID = (
    "raisa-provider-free-compatibility-conformance-harness-temporal-idempotency-readiness-repair"
)
...
def test_live_baton_rows_accept_reorientation_and_keep_cf_d2_deferred() -> None:
    ...
    assert "Continuity 255 / Compass 237" in current
    assert "266 pass / 45 fail" in current
    assert "33 temporal-fixture and 12 missing-proposal-header" in current
    assert "passed 311/311" in current
    assert "exactly eight test files" in current
    assert "application tree" in current
    assert "status-code assertion set" in current
    assert "313/313" in current
    assert "48c1821af79f9d22b7c029fdbba8c4f984d239e5" in current
    ...
    assert "48c1821af79f9d22b7c029fdbba8c4f984d239e5" in relation
    assert "Provider-free unmounted status transaction-kernel protocol rehearsal" in next_work
    assert "authored-synthetic closed state machines and transaction schedules" in next_work
    assert "practice -> appointment -> idempotency record" in next_work
    assert "atomic appointment mutation plus attributable audit plus completed receipt" in next_work
    assert "typed loser outcomes" in next_work
...
def test_current_rows_preserve_closed_surface_boundary() -> None:
    ...
    for phrase in (
        "provider-free unmounted status transaction-kernel protocol rehearsal",
        "authored-synthetic closed state machines and transaction schedules",
