"""LC4V4 content-blind certification framework.

This module is the content-blind LC4V4 certification framework.  It validates
...
The fixed production shape is:
  - 24 groups (lc4v4_group_001 through lc4v4_group_024)
...
-------------------
- This module never loads a real v4 corpus, manifest, seal, or report.
- Protected holdouts v1, v2, and v3 remain sealed and are never imported.
...
LC4V4_CORPUS_IDENTITY: str = "lc4-holdout-v4"
LC4V4_MANIFEST_SCHEMA: str = "lc4v4.manifest.v1"
LC4V4_SEAL_SCHEMA: str = "lc4v4.seal.v1"
LC4V4_EVALUATOR_VERSION: str = "lc4v4.aggregate_evaluator.v1"
LC4V4_EVALUATION_ID: str = "lc4-holdout-v4-baseline-001"
LC4V4_GROUP_COUNT: int = 24
LC4V4_VARIANTS_PER_GROUP: int = 12
LC4V4_SURFACE_PER_GROUP: int = 9
LC4V4_MT_PER_GROUP: int = 3
LC4V4_TOTAL_SCENARIOS: int = 288
LC4V4_TOTAL_TRAJECTORIES: int = 72
LC4V4_REPEAT_COUNT: int = 2
LC4V4_TOTAL_SAMPLES: int = 576
LC4V4_GROUP_PREFIX: str = "lc4v4_group_"
LC4V4_REPORT_SCHEMA: str = "lc4v4.aggregate_evaluation.v1"
_SHA256_RE = re.compile(r"^sha256:[0-9a-f]{64}$")
_COMMIT_RE = re.compile(r"^[0-9a-f]{40}$")
_SCENARIO_ID_RE = re.compile(r"^lc4v4_(?:var|mt)_\d{3}_\d{2}$")
...
# Canonical lattice dimension vocabularies
...
def _validated_group_scenarios(
    ...
    expected_group_id = f"{LC4V4_GROUP_PREFIX}{group_index:03d}"
    ...
    if not isinstance(raw_surface, list) or len(raw_surface) != LC4V4_SURFACE_PER_GROUP:
        raise ValueError(f"{expected_group_id}: invalid surface variant count")
    if not isinstance(raw_multi, list) or len(raw_multi) != LC4V4_MT_PER_GROUP:
    ...
    def validate(raw: Any, *, position: int, multi_turn: bool) -> ReceptionScenarioSpec:
        ...
        expected_id = (
            f"lc4v4_mt_{group_index:03d}_{position:02d}"
            if multi_turn else f"lc4v4_var_{group_index:03d}_{position:02d}"
        )
...
def build_manifest(corpus_dir: pathlib.Path) -> dict[str, Any]:
    ...
    group_files: list[pathlib.Path] = sorted(
        corpus_dir.glob(f"{LC4V4_GROUP_PREFIX}*.json")
    )

    if len(group_files) != LC4V4_GROUP_COUNT:
        raise ValueError(
            f"Expected {LC4V4_GROUP_COUNT} group files, found {len(group_files)}"
        )

    expected_names: list[str] = [
        f"{LC4V4_GROUP_PREFIX}{i:03d}.json"
        for i in range(1, LC4V4_GROUP_COUNT + 1)
    ]
    ...
    if total_scenarios_count != LC4V4_TOTAL_SCENARIOS:
        raise ValueError(
            f"Total scenarios {total_scenarios_count} != expected "
            f"{LC4V4_TOTAL_SCENARIOS}"
        )
    if total_trajectories_count != LC4V4_TOTAL_TRAJECTORIES:
        raise ValueError(
            f"Total trajectories {total_trajectories_count} != expected "
            f"{LC4V4_TOTAL_TRAJECTORIES}"
        )
    ...
    manifest: dict[str, Any] = {
        "schema_version": LC4V4_MANIFEST_SCHEMA,
        "corpus_identity": LC4V4_CORPUS_IDENTITY,
        "group_count": LC4V4_GROUP_COUNT,
        "variants_per_group": LC4V4_VARIANTS_PER_GROUP,
        "surface_variants_per_group": LC4V4_SURFACE_PER_GROUP,
        "multi_turn_per_group": LC4V4_MT_PER_GROUP,
        "total_scenarios": LC4V4_TOTAL_SCENARIOS,
        "total_trajectories": LC4V4_TOTAL_TRAJECTORIES,
        "repeat_count": LC4V4_REPEAT_COUNT,
        "total_production_samples": LC4V4_TOTAL_SAMPLES,
        "evaluation_id": LC4V4_EVALUATION_ID,
...
def reconstruct_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    # Schema identity
    if manifest.get("schema_version") != LC4V4_MANIFEST_SCHEMA:
        raise ValueError(
            f"Manifest schema version mismatch: "
            f"{manifest.get('schema_version')!r} != {LC4V4_MANIFEST_SCHEMA!r}"
        )
    if manifest.get("corpus_identity") != LC4V4_CORPUS_IDENTITY:
        raise ValueError(
    ...
    _require_equal(manifest, "variants_per_group", LC4V4_VARIANTS_PER_GROUP)
    _require_equal(manifest, "surface_variants_per_group", LC4V4_SURFACE_PER_GROUP)
    _require_equal(manifest, "multi_turn_per_group", LC4V4_MT_PER_GROUP)
    _require_equal(manifest, "total_scenarios", LC4V4_TOTAL_SCENARIOS)
    _require_equal(manifest, "total_trajectories", LC4V4_TOTAL_TRAJECTORIES)
    _require_equal(manifest, "repeat_count", LC4V4_REPEAT_COUNT)
    _require_equal(manifest, "total_production_samples", LC4V4_TOTAL_SAMPLES)
    _require_equal(manifest, "evaluation_id", LC4V4_EVALUATION_ID)
    _require_equal(manifest, "evaluator_version", LC4V4_EVALUATOR_VERSION)

    # File entries
    files = manifest.get("files")
    if not isinstance(files, list) or len(files) != LC4V4_GROUP_COUNT:
    ...
    for i, entry in enumerate(files):
        ...
        expected_filename = f"{LC4V4_GROUP_PREFIX}{i + 1:03d}.json"
...
def load_verified_scenarios(corpus_dir: pathlib.Path) -> list[ReceptionScenarioSpec]:
    ...
    group_files = sorted(corpus_dir.glob(f"{LC4V4_GROUP_PREFIX}*.json"))
    if [p.name for p in group_files] != [
        f"{LC4V4_GROUP_PREFIX}{i:03d}.json" for i in range(1, LC4V4_GROUP_COUNT + 1)
    ]:
        raise ValueError("LC4V4 group file population drift")
    ...
    if len(scenarios) != LC4V4_TOTAL_SCENARIOS or len(ids) != len(set(ids)):
        raise ValueError("LC4V4 scenario population or identity drift")
    return scenarios
...
def create_seal(
    ...
    *,
    ...
    seal: dict[str, Any] = {
        "seal_version": LC4V4_SEAL_SCHEMA,
...
def verify_seal(
    ...
    *,
    ...
    if seal.get("seal_version") != LC4V4_SEAL_SCHEMA:
        raise ValueError(
            f"Seal version mismatch: {seal.get('seal_version')!r}"
        )
    if seal.get("evaluator_version") != LC4V4_EVALUATOR_VERSION:
        raise ValueError(
            f"Seal evaluator version mismatch: "
            f"{seal.get('evaluator_version')!r} != {LC4V4_EVALUATOR_VERSION!r}"
        )
    if seal.get("evaluation_id") != LC4V4_EVALUATION_ID:
        raise ValueError(
            f"Seal evaluation ID mismatch: "
            f"{seal.get('evaluation_id')!r} != {LC4V4_EVALUATION_ID!r}"
        )
    if seal.get("repeat_count") != LC4V4_REPEAT_COUNT:
        raise ValueError(
            f"Seal repeat count mismatch: "
            f"{seal.get('repeat_count')} != {LC4V4_REPEAT_COUNT}"
        )
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if key_lower in _PROHIBITED_AGGREGATE_KEYS or (
                ...
                raise ValueError(
                    f"Aggregate report contains prohibited case-level key {key!r} "
                    f"at {path}"
                )
...
def _check_forbidden_string(value: str, path: str) -> None:
    ...
    prohibited_patterns: list[str] = [
        "lc4v4_group_", "lc4v4_var_", "lc4v4_mt_",
    ]
...
def evaluate_aggregate(
    ...
    *,
    ...
    corpus_hash: str,
    source_commit: str,
    repeats: int = LC4V4_REPEAT_COUNT,
) -> dict[str, Any]:
    ...
    """
    if repeats != LC4V4_REPEAT_COUNT:
        raise ValueError(
            f"Expected {LC4V4_REPEAT_COUNT} repeats, got {repeats}"
        )
    ...
    if total_scenarios != LC4V4_TOTAL_SCENARIOS:
        raise ValueError(
            f"Expected {LC4V4_TOTAL_SCENARIOS} scenarios, got {total_scenarios}"
        )
    ids = [scenario.scenario_id for scenario in scenarios]
    if len(ids) != len(set(ids)) or not all(_SCENARIO_ID_RE.fullmatch(sid) for sid in ids):
        raise ValueError("Scenario IDs must be unique and LC4V4-namespaced")
    trajectory_count = 0
    ...
    if trajectory_count != LC4V4_TOTAL_TRAJECTORIES:
    ...
    report_no_hash: dict[str, Any] = {
        "schema_version": LC4V4_REPORT_SCHEMA,
        "evaluation_id": LC4V4_EVALUATION_ID,
        "evaluator_version": LC4V4_EVALUATOR_VERSION,
        "source_commit": source_commit,
        "manifest_hash": manifest_hash,
        "corpus_hash": corpus_hash,
        "total_groups": LC4V4_GROUP_COUNT,
        "total_scenarios": LC4V4_TOTAL_SCENARIOS,
        "total_trajectories": LC4V4_TOTAL_TRAJECTORIES,
...
def _compute_variance(
    ...
    return {
        ...
        "total_repeats": LC4V4_REPEAT_COUNT,
...
def check_aggregate_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    - Forbidden-key lint
    - After consumption, only aggregate validation may run and it must not
      accept a corpus, manifest, or seal path.
    ...
    if report.get("schema_version") != LC4V4_REPORT_SCHEMA:
        fail("report_schema_version_invalid")
    if report.get("evaluation_id") != LC4V4_EVALUATION_ID:
        fail("report_evaluation_id_invalid")
    if report.get("evaluator_version") != LC4V4_EVALUATOR_VERSION:
    ...
    for key, expected in (
        ...
        ("total_scenarios", LC4V4_TOTAL_SCENARIOS),
        ("total_trajectories", LC4V4_TOTAL_TRAJECTORIES),
        ("total_samples", LC4V4_TOTAL_SAMPLES),
        ("repeat_count", LC4V4_REPEAT_COUNT),
    ):
    ...
    def score_ok(value: Any) -> bool:
        return (
            ...
            and value["total"] == LC4V4_TOTAL_SAMPLES
    ...
    if not isinstance(per_dim, dict) or set(per_dim) != expected_dimension_keys:
        ...
        if (
            per_dim["scenario_count"] != LC4V4_TOTAL_SCENARIOS
            or per_dim["sample_count"] != LC4V4_TOTAL_SAMPLES
            or per_dim["repeats_per_scenario"] != LC4V4_REPEAT_COUNT
        ):
    ...
    if not (
        ...
        and all(type(value) is int and 0 <= value <= LC4V4_TOTAL_SAMPLES for value in layers.values())
    ):
    ...
    if not (
        ...
        and 0 <= variance["variant_scenario_count"] <= LC4V4_TOTAL_SCENARIOS
        and type(variance["variant_sample_count"]) is int
        and 0 <= variance["variant_sample_count"] <= LC4V4_TOTAL_SAMPLES
        and variance["total_repeats"] == LC4V4_REPEAT_COUNT
    ...
    if not (
        ...
        and 1 <= coverage["distinct_cell_count"] <= LC4V4_TOTAL_SCENARIOS
            ...
            if valid_entries:
                for entry in entries:
                    ...
                    if not (
                        ...
                        break
                    total += entry["total"]
            if not valid_entries or total != LC4V4_TOTAL_SAMPLES:
...
def validate_lc4v4_isolation() -> None:
    ...
    for node in ast.walk(tree):
        ...
        for module_name in imported:
            if module_name.startswith(_PROHIBITED_IMPORT_PREFIXES):
                raise RuntimeError(
                    f"LC4V4 certification imports prohibited module: {module_name}"
                )


__all__ = [
    "LC4V4_CORPUS_IDENTITY",
    "LC4V4_MANIFEST_SCHEMA",
    "LC4V4_SEAL_SCHEMA",
    "LC4V4_EVALUATOR_VERSION",
    "LC4V4_EVALUATION_ID",
    "LC4V4_GROUP_COUNT",
    "LC4V4_VARIANTS_PER_GROUP",
    "LC4V4_SURFACE_PER_GROUP",
    "LC4V4_MT_PER_GROUP",
