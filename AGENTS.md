...
| Current result | A5.1/B4.1 passes as `model_required_bureau_a5_b4_command_runtime_pass` at source HEAD `c93bbfa7e656a97a85c5b4532525caa362c6c781`. A5.1 provides a default-off Receptionist-only exact check-in command with signed one-use evidence, current-state/waiting-area revalidation, atomic appointment/audit/patient-free-event/idempotency completion and deterministic readback. B4.1 provides a default-off Admin/PracticeOwner three-step default-location command with server-held attestation evidence and atomic practitioner-version/evidence/audit/unpublished-patient-free-outbox/idempotency completion. The widened 389-test regression set, canonical fast/Bandit profiles and one-head disposable PostgreSQL migration lifecycle pass. The A5 worker scope breach is corrected only through the named Sol recovery lease. The first Antigravity transport timed out before review admission during Google OAuth restoration; AER-0022 preserves it without raw authorization material. A fresh Gemini 3.6 Flash/high exact-HEAD review then passed 261 tests with no material finding and a clean unchanged worktree. No product provider call, patient/product data, ordinary-practice enablement, deployment, production, release, Pages, protected evidence or protected-ref action occurred. |
| Next implementation | Freeze and implement Bureau C4's narrowest provider-free authored-synthetic allowlisted-actuator simulator under standing programme authority. It may accept only typed one-use execution evidence for an exact allowlisted runbook and must reject model-supplied shell/SQL, unknown parameters, scope expansion, stale plans, replay and partial verification. No real database, live target, C5 action, provider product-runtime call, patient/product data, deployment, production, release, Pages, protected evidence or protected-ref movement is opened. Preserve and exclude `docs/branding/` and all existing Consultant/Gate-minus-one/A3-B3/A4 receipt/state files. |
...
On 2026-08-03 Yuri authorised a durable agent-error register. Revision 20 records 26 bounded known incidents: 20 agent-behavior observations, three harness failures, one repository defect and two transport timeouts, with no incident open. AER-0017 is corrected by the accepted evidence-only terminal reconciliation; AER-0018 contains Review 7's rejected duplicate-decision envelope; AER-0019 is corrected by the fresh-Windows-worktree LF/hash proof and Review 9 pass; and AER-0020 preserves the request-contract recovery review whose otherwise clean no-finding report repeated its terminal marker after a background completion notification. Its asynchronous-finalization control then passed in a fresh exact-HEAD worktree with one terminal decision, so AER-0020 is corrected without erasing the rejected envelope. AER-0021 preserves the A5.1 implementer's out-of-packet `app/schemas/diary_events.py` edit and rejects its self-pass; it is corrected only through the named Sol recovery lease, exact path reconciliation, deterministic runtime/database tests and the fresh independent veto. AER-0022 preserves the first A5.1/B4.1 final-review transport's Google OAuth timeout before review admission; no prompt, decision, receipt or raw authorization material was produced, and the same exact clean candidate later passed in a fresh authenticated process. AER-0023 preserves C4's first fail-closed pre-planning receipt using the unsupported `pre_plan` event; a distinct receipt selected exact approved `pre_sprint_planning` plus `sprint_planning` values before planning continued. AER-0024 preserves C4's first fail-closed worker-dispatch state with an unsupported adapter-probe method and missing workspace receipt; a distinct exact-settings-derived state supplied the allowed method and matching complete worker receipt before any launch. AER-0025 rejects the first C4 worker self-pass after independent review reproduced malformed-input evidence loss, wrong-target false success and five related authority/audit/schema/entropy/issuance gaps. AER-0026 rejects its one bounded same-lane repair self-pass after independent review reproduced non-exact reviewer role, execution-time authority drift and duplicate one-use effects across shared-store runtime instances; both worker commits remain untrusted and the correction loop is escalated to Sol's recovery lease. Corrections never erase immutable failure evidence and recurrence is keyed by the full classification composite. The implementer self-pass with material acceptance gaps is now an exact two-occurrence pattern. Duplicate verifier terminal decisions remain an exact four-occurrence pattern; detached verifier worktrees and unapproved continuation-event selections each remain exact two-occurrence patterns. The AER-0020 control requires every background notification to complete before one final response and forbids a post-decision follow-up while retaining exact-single-decision wrapper admission. AER-0014 activates an executable exact-HEAD/clean/non-protected-branch preflight before verifier receipts or launches. AER-0015 preserves the first Gate -1 review's transport-accounting error. AER-0016 preserves the preflight-blocked A3/B3 reservation, and AER-0017 preserves the provider-contacted terminal child/parent accounting split and adds evidence-only reconciliation. The register drives a controlled correction loop: preserve, classify, apply a narrow control, regress it mechanically, then admit a corrected attempt only after the applicable deterministic and independent gates pass. It is evidence-backed harness learning, not autonomous model fine-tuning or comparative provider/agent quality scoring. Future qualifying rejected reviews, worktree postcondition failures, command-scope breaches, evidence conflicts or worker transports without a closeout must be registered before a corrected attempt is accepted.
...
new result supersedes the historical no-repeat pause for its completed
  ...
  read/UI descendant, and the paired A5.1/B4.1 command runtime also passes.
  C4 is now the exact next descendant under standing authority; later planned
  boundaries follow through exact derive-freeze-execute descendants rather
  than fresh-permission pauses.
