...
| Model-required Bureau C5 plan and recovery state | The exact disposable live-development-recovery plan and threat delta pass at plan HEAD `cc81ccbcc1676d616ca4bae5cb5702b8dbede7b6` through `docs/emr4-model-required-bureau-c5-disposable-live-development-recovery-plan.md`, `docs/security/emr4-model-required-bureau-c5-disposable-live-development-recovery-threat-model-delta.md` and `orchestration/agent_inbox/codex/model-required-bureau-c5-plan-sol-acceptance.md`. DeepSeek worker commit `45a7a76e0705a2b534779847866507017370c557` and its self-pass are rejected as AER-0027 after `orchestration/agent_inbox/codex/model-required-bureau-c5-worker-independent-audit.md` reproduced fourteen material authority, admission, process, rollback, cleanup, pinning, schema and accounting gaps. The source is untrusted and may be adopted only through the named Sol recovery lease; no live C5 target, socket, provider or actuator action is eligible before deterministic recovery and a fresh exact-HEAD Gemini veto pass. |
| Current result | C4 passes as `model_required_bureau_c4_allowlisted_actuator_simulator_pass` at exact source HEAD `955b6a566f7097f58929dcb2fa9c4ed0aaad8b29`. One fixed local in-memory authored-synthetic `restart-api-synthetic.v1` transition and exact rollback are admitted behind opaque one-use evidence, exact current authority, shared cross-runtime evidence/idempotency/attempt transaction semantics, immutable attempt audit and distinct fresh readback. The original worker and its bounded repair self-passes remain rejected as AER-0025/AER-0026; Sol's named recovery lease closes all ten reproduced findings, and agent-error register revision 21 marks them corrected without relabelling either failed candidate. Acceptance evidence, the focused and widened inherited suites, Ruff, compilation, Bandit, JSON and whitespace checks pass. Fresh Gemini 3.6 Flash/high exact-HEAD review passed 389 tests with zero material finding, one terminal pass and a clean unchanged worktree. Candidate-runtime external effects were zero; exactly one authorised Antigravity source-review transport was non-zero. No product/runtime provider call, patient/product/protected data, real database or live target, mounted route, C5 action, deployment, production, release, Pages, protected evidence or protected-ref action occurred. |
| Next implementation | Preserve the rejected C5 worker provenance, then recover the provider-free implementation under Sol ownership inside the exact frozen C5 plan. Add direct adversarial regressions for every reproduced finding, pass the widened serial deterministic/API-Spine/security gates, and obtain a fresh exact-HEAD Gemini 3.6 Flash/high veto. Only those gates can open a distinct pre-execution receipt for the exact C5 rehearsal. The accepted Context Fabric remains a separate staged successor direction and is not implemented by C5. Patient/product/protected data, real practice databases, ordinary services, production, deployment, release, Pages, protected evidence and protected-ref movement remain closed. Preserve and exclude `docs/branding/` and all existing Consultant/Gate-minus-one/A3-B3/A4 receipt/state files. |
...
new result supersedes the historical no-repeat pause for its completed
  ...
  Sol recovery lease and fresh exact-head veto. Exact C5 planning is now next
  under standing authority; later planned boundaries follow through exact
