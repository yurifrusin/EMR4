...
| **Current active track** | Sprint 236 Bernie UI derived-state DAG D1/D2 implemented the Fable-approved pure selector lane: `docs/bernie-ui-derived-state-dag-plan.md` now records Fable's required constraints, `app/services/bernie/ui_view_model.py` projects `BernieSessionSnapshotOut` into display-only view-model state, and authored synthetic fixtures/tests prove confirmation-state conditioning, fail-closed enum handling, negative pre-confirm copy, no write-echo fields, and no route/provider/DB/memory/H15/trove imports |
| **Next recommended work** | Continue only with a review/inventory sprint for Bernie UI DAG D3, not UI wiring yet: inventory current Diary/Bernie switch points and map them to the new view-model fields, preserving D4 wiring as a later reviewed route-intercepted UI consumer slice; do not open provider, memory, GraphQL, write, H15/H-series/trove, practitioner-directory readiness, or live UI/provider gates without explicit review; at every sprint start explicitly announce Claude, Antigravity, and DeepSeek use/non-use, invocation mode (`scripts\drive_agent_headless.py`/Claude CLI, `agy.exe`/Antigravity CLI, direct Codex `deepseek-worker` spawn), Claude/Antigravity/DeepSeek cleanliness state, current DeepSeek lane count and reuse/cleanup plan, and any extra DeepSeek substitution for unavailable Claude/Antigravity lanes; clean stale worker artifacts before progressing and reannounce clean state |
