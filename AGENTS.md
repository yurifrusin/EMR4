...
| Next implementation | Proceed under Yuri's explicit user-selected replacement with `raisa-provider-free-docker-created-state-profile-conformance-repair`: Freeze and complete one provider-free no-credential Docker Created-state representation rehearsal: create one exact internal network and one exact never-started container, retain only sanitized structural evidence, clean both exactly, then correct and bind the relay-free harness's network-identity and credential-versus-ownership-nonce predicates without starting PostgreSQL or repeating a database attempt. The active latch is the exact authority boundary. Protected refs remain closed; preserve `docs/branding/` and stage explicit paths only. |
