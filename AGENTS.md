...
| **Current active track** | T1 Stateful Diary Scenario Laboratory is underway. T1.1 adds deterministic scenario-local time, allowlisted seeded appointments, a first-class DB-backed supervised-booking turn, confirmation from the authoritative supervised payload, per-turn appointment/audit deltas, and optional redacted evidence JSON. The create-confirm-repeat exact-duplicate scenario and independently seeded duplicate scenario pass; the full scenario/integrity baseline is green with the pre-existing expected-xfail and missing-directory skip. Details are in `docs/bernie-t1-stateful-scenario-laboratory.md`. The cross-phase order remains `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T1.2: add an allowlisted mid-scenario external-state event whose fixture mutation is explicitly separated from Bernie/system write deltas, then add overlap, same-day-distinct, terminal-status, and stale-confirm golden cases. T1.3 should add selected controlled-backend Playwright evidence. Keep autonomous/model-to-database writes, patient-specific consultant runtime, reception triage, PHI-enabled new providers, live clinical pilots, deployment/release, external clients, H15/historical-trove runtime use, memory/RAG/GraphRAG authority, and GraphQL mutations closed. Dependabot alert 5 remains open; do not force overrides. |
