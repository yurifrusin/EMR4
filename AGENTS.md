...
| Implementation/test worker | GPT Sol owns the serial custody chain. Exactly one separately named DeepSeek V4 Flash/high native-Harness worker turn is planned under the active typed coordinate-recovery latch after deterministic admission; Claude Code `--bare` is historical lineage only and no retry, resume or silent transport fallback is permitted. Gemini 3.7 Flash/high is reserved only for a fresh serial veto if an exact candidate is admitted. |
...
| Ariadne agent error and correction register acceptance | Revision 617: `docs/ariadne-agent-error-correction-register-revision-617.md`; 957 bounded incidents, all corrected/contained and none open. AER-0957 records the preexecution rejection of a contract generated before its bound schema bytes were final; the deterministic exact-equality and provider-free gates now require the regenerated descendant before attempt preparation. Durable state: `orchestration/continuity/ariadne-agent-error-register/`. |
