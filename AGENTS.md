...
| **Current active track** | Bernie S22-S24 duplicate recovery is integrated through `1c3d94ee`. Exact duplicate classification now requires patient, practitioner, date, and temporal agreement plus matching optional supplied constraints. An exact duplicate short-circuits before slot search and returns a safe, non-writing `existing_booking_found` advisory with minimal appointment evidence and alternative-search suggestions. It remains recoverable through the existing `no_slot` session state. The diary explicitly announces and reads back the existing appointment, never exposes candidates or confirmation for the finding, and offers keyboard-operable alternative searches. DeepSeek Flash implemented the backend, Sol corrected material authority/state defects, and Gemini 3.5 Flash implemented the bounded UI. Details are in `docs/bernie-s22-s24-duplicate-recovery-closeout.md`. |
| **Next recommended work** | Run a separate accessibility and modality-independent booking-authority tranche. Preserve the current deterministic proposal/confirm boundary and authenticated receptionist authorization, but ensure a blind receptionist can review validation evidence, authorize through a non-visual control, and receive an accessible authoritative receipt without sighted diary inspection. Add accessibility-tree, keyboard, and live-status tests. Keep hands-free operation as a separately bounded delegation policy. Do not open unrestricted autonomous writes, provider/live-provider, schema/database, deployment/release, external client, H15/H-series, historical-trove, memory/RAG/GraphRAG, GraphQL mutation, or new confirmation authority. The Flash adapter calibration from S19-S21 is now recorded as provisional provider-cost evidence; refine it with future exact deltas. Dependabot alert 5 remains open; do not force overrides. |
