...
| **Current active track** | Sprint 267 practitioner-directory GraphQL runtime/resolver gate packet is complete locally: Yuri approved a gate packet only for `Query.practice.practitioners`; `docs/api-spine/practitioner-directory-graphql-runtime-gate.{json,md}` selects Strawberry as the preferred future runtime, defines the bearer-token auth context, error taxonomy, depth/cost/alias/introspection posture, dependency security checks, resolver contract, and implementation test matrix. Resolver code, dependency install, endpoint wiring, schema runtime code, readiness flag changes, deployment, production, external-client, write, provider, memory/RAG/GraphRAG, H15/H-series, and trove gates remain closed. DeepSeek returned PASS. |
| **Next recommended work** | Pause for explicit Yuri approval before Sprint 268 installs `strawberry-graphql`, mounts `/api/v1/graphql`, or adds resolver/schema runtime code. If approved, the smallest next slice should add the dependency and a tightly bounded `/api/v1/graphql` endpoint with auth/depth/cost guards and `Query.practice.practitioners` resolver tests only; otherwise continue plan/review work. D5 expansion remains paused unless separately approved. Use each worker only where it has a distinct artifact or veto surface. |
