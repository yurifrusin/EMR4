...
| Next implementation | Begin the provider-free authored-synthetic Practice Context Fabric and Bureau Memory Bank contract descendant. Freeze typed `ContextNeedCandidate`, backend `ContextAuthorityBinding`, `ContextNeed`, deterministic `ContextScopeGrant`, `BureauMemorySelector`, `BureauMemoryItem`, `ContextFrame`, `ContextFrameSet`, selector/weave and proofreader traces, plus an unmounted GraphQL read-only documentation surface. The Memory Bank must remain a derived, lossy, minimal, expiring and rebuildable projection over authorised completed-action/audit sources, never raw audit access or provider-model memory. Patient/product/protected data, real practice databases, persistence, temporal retention, provider/external retrieval, ordinary services, product runtime, commands, writes, deployment, production, release, Pages, protected evidence and protected-ref movement remain closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state files. |
