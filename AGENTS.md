...
| **Current active track** | Sprint 279 practitioner-directory Office add-in GraphQL selector route-intercepted evidence is complete locally. Sprint 278's default-off switch remains unchanged in `docs/diary/diary.js` with `ENABLE_GRAPHQL_PRACTITIONERS = false`, no runtime/user override, and no live GraphQL traffic by default. Sprint 279 added `review/test_diary_graphql_practitioner_switch.py`, proving default-off REST-only behavior with zero GraphQL requests, test-harness-only enabled GraphQL fallback for `FORBIDDEN`, `BAD_USER_INPUT`, and transport failure, `practice=null` empty/no-REST/no-leak behavior, `defaultLocation=null` row preservation, and sensitive canaries absent from rendered page text. Evidence lives in `docs/api-spine/practitioner-directory-office-addin-graphql-switch-route-intercepted-evidence.{json,md}` with DeepSeek PASS review in `orchestration/agent_inbox/codex/review-deepseek-sprint279-office-addin-graphql-switch-route-intercepted.md`. Antigravity was invoked for Office add-in UX review but timed out; Claude was stood down because no API contract/backend/schema/auth model changed. Browser tooling was available, but the committed Playwright harness was the correct evidence path because this proof depends on route interception. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, default-on, or field-expansion gate is opened. |
| **Next recommended work** | Sprint 280 should be a docs/tests-only default-on decision packet for the Office add-in practitioner selector GraphQL path, or a stop for Yuri approval before any live/default-on traffic. Do not make the GraphQL path default-on, add telemetry, or claim deployment/production/readiness without a separate approval. D5 expansion remains paused unless separately approved. |
