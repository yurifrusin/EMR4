...
| Required Git relation | The task branch `codex/ariadne-bernie-davida-parallel-seam` must contain the accepted AES-C2 closeout as a descendant of exact independently reviewed source `d54f0476448f1218cd55477d42b958721359eae8`, itself a descendant of the required resumed HEAD `648c8ec9805af63729264ea9c22fd695f062a741` and published conformance source `01d355f42df5981341196f3aa0caec2cccce7a2d`. Local/origin `master` and `handoff/current` remain aligned at protected `2e34bdad732fdab32fbf778280b3d3c70d66d602`; no protected ref movement is authorised. User-owned untracked `docs/branding/` and all unrelated untracked files remain preserved and excluded. |
...
| Current result | At Continuity 239 / Compass 221, AES-C2 passes as `raisa_agent_execution_surface_containment_gate_aes_c2_provider_free_broker_simulator_pass` over exact independently reviewed source `d54f0476448f1218cd55477d42b958721359eae8`. Its exact 26 authored-synthetic scenarios resolve as two released simulations, four non-dispatches and 20 terminal stops; all 18 hostile attempt/result mutations and 14 hostile contract mutations reject without release. Broker-owned registry identity, synthetic noncredential custody, dispatch-time immutable-generation/current-authority/revocation/kill recheck and exact budget commit remain fail closed. Exactly three pure Python calls occur with zero external effect. The focused and fresh Gemini 95-test packets, final maintained 155-test static packet and canonical 161-test fast profile pass with zero real runtime, adapter, provider, network, filesystem, database/source, tool, command or product/patient operation. |
| Next implementation | AES-C3 hostile containment rehearsal is the next safe planned tranche: after a fresh complete five-source rehydration and receipt, freeze authored-synthetic provider-free attacks across local-file references, template/deserialization, metadata and credential probing, arbitrary relay or encoded egress, cumulative probing, stale lease and cross-generation replay against the exact AES-C0/C1/C2 contracts. It authorises no real runtime broker or adapter, provider call, patient/clinical/product data or product read, filesystem, database/source or watcher access, credential/IAM/metadata access, generic network, executable tool, command/write, deployment, production, release, Pages or protected-ref movement. Continue under the standing uninterrupted-development authority; preserve and exclude `docs/branding/` and all unrelated untracked files. |
...
| Raisa Practice Context Fabric direction | On 2026-08-05 Yuri accepted the backend-owned temporal Practice Context Fabric as Raisa's permanent cross-Bureau integration direction and subsequently added the Bureau Memory Bank. Query-shaped `ContextNeed` candidates must pass deterministic purpose, principal, role, tenant, source, field, time-window and freshness policy before a typed expiring `ContextFrameSet` reaches a Bureau and same-packet proofreader. Current facts, committed events, historical operational states, recent collective work, private session state, durable domain threads and cited knowledge evidence remain distinct threads with provenance and authority ceilings. The Bureau Memory Bank is a read-only, purpose-filtered, lossy, expiring and rebuildable projection over authorised completed-action or audit-derived records; it is never raw audit access, a transcript store, current truth, command evidence or provider-model memory. The Fabric is not a shared prompt, database dump, vector/RAG authority, new source of truth, workflow executor or command plane. `RECEPTION ONE™` and candidate `Clinician One` are branded workspace/projection families, not authority boundaries: atomic backend capability grants govern Consultant, requests/referrals, prescribing/medicines, billing/claims and every other Bureau independently, even where their typed work interweaves across branded surfaces. Its provider-free contract, Current operational weave, patient-free temporal weave, intent-shaped retrieval and complete unmounted Rayleen source-adapter/invalidation/fresh-generation lifecycle pass at exact reviewed source HEADs; its authored-synthetic model-required intent-shaping descendant passes at `44f341481b55f99a18a47838da0f2b7e43a2f73e` with one deterministically admitted Sydney Vertex call. Its source-specific durability architecture, state-machine rehearsal, declarative migration/transaction architecture, exact function/trigger-body architecture and inert DDL lowering pass. The first disposable PostgreSQL-16 server behavior slice now also passes all twenty frozen serial entry-point, trigger, RLS, idempotency and outer-rollback scenarios at reviewed source `f3383dc4099b4ee590014bea62dddb146f5d2a16`, with exact cleanup; concurrency, restart, unknown-commit, rotation, retention, purge, performance and operations remain unproved. This decision grants no patient/clinical/product data, further provider/external retrieval, real observation or source read, operational temporal retention, runtime/persistence/wiring, command/write, requests/referrals, prescribing/medicines, billing/claims, deployment, production, release, Pages or protected-ref authority. Controlling artifacts are `docs/raisa-practice-context-fabric-direction.md`, the accepted Context Fabric closeouts, `implementation_plan.md`, `orchestration/bernie_interaction_model.md` and the `raisa-practice-context-fabric` Compass horizon item. |
| Agent Execution Surface and Containment Gate direction | On 2026-08-08 Yuri accepted a hard prerequisite before any occupied Bureau receives real product-derived context or any executable tool, filesystem, database, network, credential, provider-executed tool, command-adjacent capability or reusable runtime authority. The prerequisite Context Fabric serial durability behavior slice, architectural-health review, bounded CI/lifecycle repair, AES-C0 architecture, AES-C1 admission rehearsal and AES-C2 inert broker simulation now pass. AES-C0 freezes six closed message types, the external deterministic capability-broker boundary, immutable generations, no ambient credentials or metadata, exact egress, independent cumulative reasoning/information/action/denial/time budgets, generation-wide revocation, inert typed context, supply-chain identity, explicit no-fallback state and an externally owned kill switch. AES-C1 proves the exact manifest/grant/lease/current-authority intersection, default denial, revocation and budget stops over authored-synthetic unmounted objects with zero execution. AES-C2 proves broker-owned identity, synthetic custody, dispatch-time control-state recheck and exact budget commit around one pure inert function with zero external effect. AES-C3 hostile containment is next and may challenge only authored-synthetic provider-free local-file, template/deserialization, metadata/credential probing, arbitrary or encoded egress, cumulative probing, stale-lease and cross-generation replay surfaces. Controlling artifacts are `docs/raisa-agent-execution-surface-containment-gate-plan.md`, the accepted AES-C0 through AES-C2 plan/threat/closeout artifacts, the accepted architectural-health review and `implementation_plan.md`. This direction grants no real broker or adapter, provider call, product/patient data, credential/IAM change, network, filesystem capability, executable tool, command, deployment, production, release, Pages or protected-ref authority. |
### Compact historical evaluation and transition state

The detailed language-evaluation chronology is indexed in `docs/handover-ledgers/bernie-language-evaluation.md`; the active acceptance documents in the Current Baton remain authoritative. The compact facts needed
