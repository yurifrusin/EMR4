...
| **Current active track** | Sprint 280 practitioner-directory Office add-in GraphQL selector default-on approval packet is in progress locally as docs/tests only. Sprint 278's default-off switch remains unchanged in `docs/diary/diary.js` with `ENABLE_GRAPHQL_PRACTITIONERS = false`, no runtime/user override, and no live GraphQL traffic by default. Sprint 279 route-intercepted evidence is complete and pushed at commit `4fb3e750`, proving default-off REST-only behavior with zero GraphQL requests, test-harness-only enabled GraphQL fallback for `FORBIDDEN`, `BAD_USER_INPUT`, and transport failure, `practice=null` empty/no-REST/no-leak behavior, `defaultLocation=null` row preservation, and sensitive canaries absent from rendered page text. Sprint 280 adds `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-approval-packet.{json,md}` and `tests/test_practitioner_directory_office_addin_graphql_default_on_approval_packet.py`; the packet decision is pending Yuri approval and authorizes no runtime code changes. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, default-on, or field-expansion gate is opened. |
| **Next recommended work** | Stop for Yuri approval before changing `ENABLE_GRAPHQL_PRACTITIONERS` to true or sending Office add-in GraphQL traffic by default. If Yuri approves the Sprint 280 packet, the next sprint may implement only the single-consumer default-on flip with REST fallback retained. D5 expansion remains paused unless separately approved. |
