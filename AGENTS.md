...
| **Current active track** | Sprint 292 D5 next-step approval payload draft is complete. `docs/bernie-ui-derived-state-d5-next-step-approval-payload-draft.{json,md}` is explicitly `draft_only_not_applied` / `no_new_d5_approval_applied`: reviewer/date are blank, acknowledgement is false, all candidate scope fields are false, and the historical D5 first-slice approval is neither renewed nor extended. The Ariadne resource-allocation policy now defines the next harness design surface: a subscription/API-key worker pool with model/reasoning defaults, user-set instance ceilings, current availability and cost tiers; ranked SSDLC role preferences/fallbacks; and a mandatory generalist profile that covers all roles with self-review transparently labelled when only one agent exists. The orchestrator proposes an economical staffing plan by default and the user may amend any model, reasoning level, ceiling, or assignment. GPT Terra remains the current primary orchestrator, DeepSeek 4 Pro can be a rehydrated fallback, ordinary Claude review should normally use Opus medium or Sonnet, Fable is an exceptional high-leverage consultant only, and Antigravity/DeepSeek workers are assigned per bounded packet rather than boxed into permanent roles. The historical unused DeepSeek worker cleanup is not a global retirement. No live worker adapter, enforcement, branch movement, runtime wiring, D5 expansion, frontend JavaScript change, provider/live-provider wiring, Access AI, memory/RAG/GraphRAG, H15/H-series, historical diary runtime input, GraphQL delivery/readiness, external patient client, confirm payload/write behavior change, model-to-database write, deployment, production, telemetry, global readiness gate, autonomous commit/release authority, or EMR4 runtime behavior is opened. |
| **Next recommended work** | Claude Fable's explicit worker-pool consultation is preserved at `orchestration/agent_inbox/codex/review-claude-fable-ariadne-worker-pool-allocation-plan.md`. Its recommendation is S4a-S4d advisory-only: first separate transport, reachability, quota availability, capability, cost, independence, authority, and user override in strict schemas; then add a deterministic pure allocator/replay corpus, advisory CLI, and two-sprint manual EMR4 pilot through the existing packet workflow. Do not launch workers through the harness or enable enforcement. In parallel, D5 remains closed unless Yuri separately authorizes the draft-only route-intercepted copy-evidence option. |
