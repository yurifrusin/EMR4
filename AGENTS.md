...
| Required Git relation | The accepted fresh-generation source is task HEAD `9516b85542a4de1fcef305423ec15fd34f7731aa`; bounded tracked closeout and next-plan changes are permitted before task-branch push. Local/origin `master` and `handoff/current` remain aligned at protected `2e34bdad732fdab32fbf778280b3d3c70d66d602`; user-owned untracked `docs/branding/` remains preserved and excluded |
...
| Next implementation | Under Yuri's standing uninterrupted-gate authority, proceed directly to an architecture-only provider-free plan for the first default-off live-source observation boundary. It may define how authenticated practice-scoped change metadata creates only a payload-free invalidation signal and how that signal requests a separately authorised fresh read. The observer must never become truth, return data or receive command authority. This planning tranche may not implement a database/feed/watcher/listener, product read, patient/product data path, persistence, route, provider, command, deployment, production, release, Pages or protected-ref movement. Any live implementation, real-data read, persistence, cross-Bureau/clinical source, command or write remains separately closed. Consultant, requests/referrals, prescribing/medicines, billing/claims and other future Bureau implementation authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
