...
| **Current active track** | Sprint 269 practitioner-directory GraphQL runtime shell is complete locally: `app/graphql/router.py` mounts `/api/v1/graphql`, `app/graphql/context.py` reuses `get_current_user`/`get_db`, and `app/graphql/schema.py` exposes only authenticated `graphqlHealth` with query-only schema plus depth/alias/token guards. Evidence lives in `docs/api-spine/practitioner-directory-graphql-runtime-shell.{json,md}` with tests in `tests/test_practitioner_directory_graphql_runtime_shell.py`; Sprint 267/268 guard tests were updated to allow the shell while still blocking `Query.practice.practitioners`. No practitioner resolver, readiness flag change, deployment, production, external-client, write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. DeepSeek returned PASS. |
| **Next recommended work** | Continue to Sprint 270: add only the approved `Query.practice.practitioners(activeOnly, limit, offset)` schema/resolver path against `app/services/practice/practitioner_directory_read.py::list_practitioner_directory`, preserving REST parity and all adjacent closed gates. D5 expansion remains paused unless separately approved. Use DeepSeek as a resolver-boundary/security reviewer; use Claude/Antigravity only if an available lane has a distinct backend/API or consumer/privacy review surface rather than ritual enumeration. |
