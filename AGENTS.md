...
| **Current active track** | Sprint 282 publication hygiene for the Sprint 281 practitioner-directory Office add-in GraphQL selector default-on runtime is complete locally and pending push. Sprint 281 is pushed at `d3dda16e657a4eb51b845a509c5cff071f530c43` on `master` and `handoff/current`; `docs/diary/diary.js` has `ENABLE_GRAPHQL_PRACTITIONERS = true` for the single `office_addin_diary_booking_practitioner_selector` consumer, with no runtime/user override and REST fallback retained. Sprint 282 adds `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-publication-status.{json,md}` and guard tests proving pushed refs, single-consumer scope, REST fallback, and closed adjacent gates. DeepSeek returned PASS in `orchestration/agent_inbox/codex/review-deepseek-sprint282-default-on-publication-status.md`. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, runtime-user-override, or field-expansion gate is opened. |
