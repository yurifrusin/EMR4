...
| **Current active track** | Sprint 273 practitioner-directory GraphQL internal consumer harness is complete locally: `tests/graphql_practitioner_consumer_harness.py` defines a reusable test-only consumer-contract harness for `Query.practice.practitioners`, with coverage in `tests/test_practitioner_directory_graphql_internal_consumer_harness.py` and evidence in `docs/api-spine/practitioner-directory-graphql-internal-consumer-harness.{json,md}`. It proves authenticated success, HTTP 401 auth failure, GraphQL `BAD_USER_INPUT`, GraphQL `FORBIDDEN`, `practice(id:) = null`, approved projection only, sensitive-field rejection, no idempotency-key requirement or behavior change, default/max/offset pagination, role/default active scope, practice scoping, default-location null behavior, no audit writes, and source isolation. Claude returned PASS on API/contract review, DeepSeek returned PASS on security/static boundaries, and Antigravity was deliberately reserved for Sprint 274 where there is a true Office add-in consumer/UX surface. No production UI wiring, runtime schema change, readiness flag change, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, or field-expansion gate has been opened. |
| **Next recommended work** | Continue to Sprint 274: draft the Office add-in practitioner selector GraphQL consumer proposal and comparison boundary, using Antigravity through `agy.exe` for the distinct consumer/UX review surface and Claude/DeepSeek only where they add separate API/security veto value. Do not switch taskpane runtime traffic to GraphQL without the Sprint 274 proposal evidence and a subsequent consumer gate decision. D5 expansion remains paused unless separately approved. |
