...
| **Current active track** | Sprint 275 practitioner-directory Office add-in GraphQL fetch-wrapper test plan is complete locally: `docs/api-spine/practitioner-directory-office-addin-graphql-fetch-wrapper-test-plan.{json,md}` and `tests/test_practitioner_directory_office_addin_graphql_fetch_wrapper_test_plan.py` define the blocked-by-default mocked test cases required before any taskpane runtime switch. Antigravity returned PASS on Office add-in UX/test expectations through `agy.exe`; DeepSeek returned PASS on static/gate guardrails; Claude was deliberately stood down because no API contract, resolver, or runtime auth/error model changed. The plan keeps the `2026-08-06` expiry visible, requires mocked success/empty/HTTP-401/GraphQL-error/practice-null/defaultLocation-null/projection-drift/expiry/fallback coverage, and forbids `taskpane.js` edits, hidden flags, shadow fetches, telemetry endpoints, backend changes, readiness changes, writes/audit writes, providers, memory/RAG/GraphRAG, H15/H-series, trove, mutations, subscriptions, deployment, production, external-client exposure, and field expansion. |
| **Next recommended work** | Either pause for a separate consumer switch approval before any taskpane runtime implementation, or run one more non-runtime Sprint 276 that adds mocked client-contract scaffolding under tests only. Do not edit `EMR4 Sidebar/src/taskpane/taskpane.js` or send GraphQL traffic from the Office add-in without explicit switch approval. D5 expansion remains paused unless separately approved. |
