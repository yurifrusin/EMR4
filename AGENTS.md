...
| **Current active track** | Sprint 271 GraphQL practitioner contract hardening is complete locally: no new runtime field was added beyond `graphqlHealth`, `Query.practice(id)`, and `Practice.practitioners(activeOnly, limit, offset)`. Evidence lives in `docs/api-spine/practitioner-directory-graphql-contract-hardening.{json,md}` and tests in `tests/test_practitioner_directory_graphql_contract_hardening.py`. The hardening proves runtime SDL parity for the approved practitioner slice, token-budget fail-closed behavior for both `graphqlHealth` and the practitioner resolver path, alias-flood preemption by the 500-token budget while the alias limiter remains configured, structural depth <=6 for the current shallow slice, blocked global readiness snapshots, and cross-document `must_remain_false` consistency across shell/resolver/hardening evidence. No mutation/subscription, readiness flag change, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. DeepSeek returned PASS and its recommendations were integrated. |
| **Next recommended work** | Continue to Sprint 272: prepare a practitioner GraphQL release-boundary packet deciding whether this scoped GraphQL route is internally usable, still blocked, or needs one more bounded hardening pass; do not flip global readiness, deployment, production, external-client, write, provider, memory, H15/trove, mutation, or subscription gates without explicit approval. D5 expansion remains paused unless separately approved. Use DeepSeek for boundary/security review; use Claude/Antigravity only if an available lane has a distinct artifact or veto surface, such as a backend/API or consumer/privacy review surface, rather than ritual enumeration. |
