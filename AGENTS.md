...
| **Current active track** | Sprint 278 practitioner-directory Office add-in GraphQL selector switch is complete locally and default-off: Yuri approved continuing with "Ok let's keep going" on 2026-07-09, the Sprint 277 packet now records `approved_for_default_off_office_addin_graphql_practitioner_selector_switch` through `2026-08-06`, and `docs/diary/diary.js` contains `ENABLE_GRAPHQL_PRACTITIONERS = false` plus the exact approved `Query.practice.practitioners` fetch path behind that source-controlled gate. The actual approved consumer is the Office add-in diary surface, not the main taskpane file. With the gate off, the existing REST route remains the render source and no GraphQL traffic is sent by default. The implementation preserves the approved projection (`id`, `displayName`, `roleLabel`, `active`, `defaultLocation { id name }`), one-shot REST fallback for GraphQL non-401 transport/body failures, existing HTTP 401 logout behavior, `practice=null` empty no-leak behavior, `defaultLocation=null` row preservation, malformed row dropping via `id`/`displayName` filter, and no localStorage/query-param/Office-settings/server-config override. Antigravity returned PASS on Office add-in UX guardrails, DeepSeek returned PASS with cautions integrated, and Claude was stood down because no API contract/backend/schema/auth model changed. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, default-on, or field-expansion gate is opened. |
| **Next recommended work** | Continue with Sprint 279 route-intercepted browser/static evidence for the default-off REST path and enabled-path GraphQL fallback shape. Do not make the GraphQL path default-on, add telemetry, or claim deployment/production/readiness without a separate approval. D5 expansion remains paused unless separately approved. |
