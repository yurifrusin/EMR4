...
| **Current active track** | T2 Deterministic Bernie Behaviour Matrix is complete through T2.3 for supervised booking. The gate executes 746 generated classifier combinations, 17 isolated DB-backed route combinations, the stateful T1 corpus, and focused E3 outcomes. The final aggregate collects 186 nodes and passes with one expected xfail and one expected skip. T2.3 corrected candidate-test isolation/no-write defects and an async focus-timer regression; no route defect was found. Shared-conftest pytest processes must now run serially. Details are in `docs/bernie-t2-deterministic-behaviour-matrix.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T3 Nondeterministic Bernie Shadow Evaluation: define a provider-neutral evaluation envelope and model-version ledger, replay the T1/T2 semantic corpus through live models with all writes disabled, score intent/entities/time/clarification/tool selection/unsafe claims deterministically, and report variance/cost/latency separately from correctness. No model promotion or write authority is implied. Preserve all clinical, deployment, provider-PHI, historical-trove runtime, memory/RAG/GraphRAG, and production gates. Dependabot alert 5 remains open; do not force overrides. |
