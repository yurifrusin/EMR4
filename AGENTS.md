...
| **Current active track** | Sprint 229 practitioner-directory REST/GraphQL drift contract integrated in implementation commit `ad58563a`: a static parity contract now defines the canonical five-field projection, shared read-service invariants, sensitive-field exclusion parity, `activeOnly`/admin-inactive/pagination/order/error parity, and two known blocked drifts (`defaultLocation` full SDL shape and missing GraphQL pagination args) while keeping REST route, GraphQL runtime dependency/resolver, schema, read service, database query, readiness, provider, memory, GraphQL mutation, and write gates blocked |
| **Next recommended work** | If Yuri explicitly approves first-route implementation, start the bounded `GET /api/v1/practice/practitioners` REST route/schema/read-service implementation sprint using the Sprint 227 gate plus the Sprint 229 drift contract, and keep GraphQL/providers/memory/write gates closed; otherwise continue safely with another plan-only API Spine preflight such as first-route security/audit test harness or SDL pagination/default-location resolution proposal, not runtime code; at every sprint start explicitly announce Claude, Antigravity, and DeepSeek use/non-use, invocation mode (`scripts\drive_agent_headless.py`/Claude CLI, `agy.exe`/Antigravity CLI, direct Codex `deepseek-worker` spawn), Claude/Antigravity/DeepSeek cleanliness state, current DeepSeek lane count and reuse/cleanup plan, and any extra DeepSeek substitution for unavailable Claude/Antigravity lanes; clean stale worker artifacts before progressing and reannounce clean state; keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, runtime graph execution, REST/GraphQL implementation without explicit approval, or model-to-database-write gates |
