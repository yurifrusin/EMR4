...
| **Current active track** | Sprint 285 monitoring/readiness-boundary packet for the already default-on practitioner-directory Office add-in GraphQL selector is published at `2c6cd5146b1c8c9538873f4a3f2e3a2970191077` on `master` and `handoff/current`; the integration worktree is clean. Sprint 281 default-on runtime is pushed at `d3dda16e657a4eb51b845a509c5cff071f530c43`, Sprint 282 publication status at `514970f656eb55991c3c0def62bfd6d522ea3f74`, Sprint 283 local backend smoke at `7fe7565e6c33c17d17dc5547c0f372ac00fdb4c0`, and Sprint 284 rollback packet at `16eed7ed5fbf47d978712f752be164610335d8b2`. `docs/diary/diary.js` still has `ENABLE_GRAPHQL_PRACTITIONERS = true` for the single `office_addin_diary_booking_practitioner_selector` consumer, with no runtime/user override, no telemetry endpoint, and REST fallback retained. Sprint 285 adds `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-monitoring-boundary.{json,md}` and guard tests defining observable-without-new-instrumentation checks, operator watchpoints, readiness blockers, and must-not-claim boundaries. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, runtime-user-override, or field-expansion gate is opened. |
| **Next recommended work** | Sprint 286 should correct stale closeout/handover publication wording, then Sprint 287 should choose the next already-approved non-runtime evidence track. Pause before any deployment or production readiness claim; do not expand the GraphQL/default-on path, add telemetry, or claim readiness without explicit approval. D5 expansion remains paused unless separately approved. |
