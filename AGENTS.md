...
| Ariadne agent error and correction register acceptance | `docs/ariadne-agent-error-correction-register-plan.md`, `docs/ariadne-agent-error-correction-register-revision-2.md`, `docs/ariadne-agent-error-correction-register-revision-3.md`, `docs/ariadne-agent-error-correction-register-revision-4.md`, `docs/ariadne-agent-error-correction-register-revision-5.md`, `docs/ariadne-agent-error-correction-register-revision-6.md`, `orchestration/continuity/ariadne-agent-error-register/agent-error-register.schema.json`, `orchestration/continuity/ariadne-agent-error-register/agent-error-register.json`, `scripts/ariadne_agent_error_register.py`, `scripts/ariadne_serial_pytest.py`, `tests/conftest.py`, `orchestration/continuity/ariadne-agent-error-register/pattern-report.json`, `orchestration/harness_settings/verifier_execution_policy.yaml`, `tests/test_ariadne_agent_error_register.py`, `tests/test_ariadne_serial_pytest.py`, `tests/test_ariadne_verifier_execution_policy.py`, `scripts/compact_agents_acceptance_index.py`, `docs/handover-ledgers/current-baton-acceptance-index.manifest.json`, `tests/test_agents_acceptance_index.py`, `orchestration/agent_inbox/codex/fifth-pair-independent-review.md`, `orchestration/agent_inbox/codex/bernie-davida-fifth-pair-antigravity-detached-branch-failure-receipt.json`, `orchestration/agent_inbox/antigravity/bernie-davida-fifth-pair-review-receipt.json`, `orchestration/agent_inbox/codex/model-required-bureau-architecture-preacceptance-receipt.json`, `orchestration/agent_inbox/codex/model-required-bureau-architecture-pre-verifier-acceptance-receipt.json`, `docs/ariadne-agent-error-correction-register-closeout.md`, and `orchestration/agent_inbox/codex/ariadne-agent-error-register-sol-acceptance.md` |
...
| Current result | The supervised provider-free Office active-practitioner consumer and its five lifecycle descendants remain complete at Continuity 206 / Compass 187. The architecture-only Bernie/Davida parallel seam and all first five pairs now pass without advancing those maps. The fifth pair adds exact authored-synthetic `route_intercepted_browser` evidence for the default-off native-Diary application-session composition and a non-executing backend-owned Davida proposal-to-confirm REST architecture for one future practitioner default-location change. Native review found two high blockers: path-only browser interception and direct-pytest lock bypass. Both were repaired with exact HTTP method/path admission and a `tests/conftest.py` cross-process lock. Reviewer environment mutation and a later detached Antigravity preflight were contained and registered. The agent-error register is now revision 6 with 13 preserved incidents: 11 agent-behavior, one harness and one transport; the only exact recurrence remains duplicate terminal decisions. The new AER-0013 preserves root's unapproved `pre_acceptance` event and its corrected distinct receipt; neither attempt dispatched a worker or called a model. Root passed 133 serial tests plus exact evidence, Ruff, Node, JSON/YAML, process and diff gates. Fresh Gemini 3.6 Flash/high then passed the unchanged clean candidate `f551a91d861baa65d04fae8f50dfee0a52440035` with 101 tests and zero findings. Yuri subsequently approved the architecture doctrine `model-required cognition; deterministic authority`: every named intelligent user or operator loop requires an accepted provider model, while typed context, deterministic proofreading, backend authority, applicable human gates, commands and readback remain independently mandatory. Rayleen is present-tense waiting-room intelligence; Bernie is prospective scheduling intelligence; Davida is institutional/practice-administration intelligence; controlled recovery/update is a separate technical control plane. The architecture and development plan are documented, but no worker has been dispatched and no runtime or development successor has begun. No provider call, real identity/data, patient/clinical data, arrival or administrative write, recovery actuator, migration, external update, cloud/IAM, deployment, production or release is established. |
| Next implementation | Paused by Yuri before further subagent development. The planned first successor is gate zero only: freeze the shared four-plane model-required Bureau contract, closed schemas, capability/context partitions, outage semantics, threat model and deterministic artifact tests. Only after gate zero passes Sol architectural acceptance and a fresh Gemini 3.6 Flash/high veto may the separable Rayleen read/projection plus NLU-evaluation lane, Davida NLU-parity lane and controlled-recovery system-anatomy/diagnostic lane be dispatched. The update-supply-chain lane follows the shared technical provenance vocabulary. The earlier proposed live-local Diary application-session rehearsal and actual Davida default-location command implementation remain material product/read/write gates and do not start under this plan. Provider/model selection or calls, any external data transmission, patient-facing Rayleen, product reads/writes, actuators, shell/SQL/cloud/IAM, database migrations, reference-data download/import/activation, cost/licence, deployment, production, release, protected refs/Pages and protected evidence remain closed. `docs/branding/` remains user-owned, preserved and excluded; the candidate hierarchy may still be used in future authorised UI renders. |
...
On 2026-08-03 Yuri authorised a durable agent-error register. Revision 6 now records 13 bounded known incidents: 11 agent-behavior observations, one harness failure and one transport timeout. Corrections never erase immutable failure evidence, recurrence is keyed by the full classification composite, and the only exact recurrence remains the two-occurrence duplicate terminal-decision pattern. AER-0013 preserves root's unapproved `pre_acceptance` envelope, which failed closed before dispatch or model call; the corrected distinct receipt uses the settings-approved `pre_verifier_acceptance` event. The register drives a controlled correction loop: preserve, classify, apply a narrow control, regress it mechanically, then admit a corrected attempt only after the applicable deterministic and independent gates pass. It is evidence-backed harness learning, not autonomous model fine-tuning or comparative provider/agent quality scoring. Future qualifying rejected reviews, worktree postcondition failures, command-scope breaches, evidence conflicts or worker transports without a closeout must be registered before a corrected attempt is accepted.
