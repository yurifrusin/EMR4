...
| **Current active track** | Sprint 206 API Spine raw compatibility consumer/signal readiness preflight integrated in implementation commit `8244f36c`: a static preflight now ties each raw appointment compatibility write to backend signal calls, current Diary frontend fallback call-site IDs, and the `keep_audit_mode` decision because no frontend currently consumes the `Deprecation` header |
| **Next recommended work** | Continue the next tranche with either a deliberate frontend deprecation-header consumer design/test preflight before any `appointment_raw_compat_mode` change, or a small read-model gap inventory for the newly explicit practitioner/reminder/message/directory gaps; use Ariadne plus three worker lanes for general sprint work (Claude through `scripts\drive_agent_headless.py` with `--mint-session`, Antigravity via `C:\Users\sarashera\AppData\Local\agy\bin\agy.exe`, and DeepSeek, with extra DeepSeek substitutions when a lane is unavailable), let DeepSeek workers finish before integration, keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, or model-to-database-write gates |
