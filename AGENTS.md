...
| **Current active track** | T3 Nondeterministic Bernie Shadow Evaluation is in progress after T2 completed at `b8cd2078`. T3.1-T3.4 provide the immutable provider-neutral contract/model ledger, exact semantic and independent safety scoring, a strict four-case authored projection from known T1/T2 source scenarios, a default-disabled injected-adapter repeat runner with separate variance/cost/latency reporting, and a narrow machine-checked live-replay gate. The T3 suite has 48 passing tests. External prompts, provider tools, raw-response persistence, patient/practice inputs, mutations, runtime wiring, and promotion remain blocked; static provider-adapter implementation is allowed. Details are in `docs/bernie-t3-shadow-evaluation.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T3.5: implement DeepSeek and Gemini adapters behind the protocol without executing external calls. Include provider availability checks, exact model ledger entries, bounded request construction, strict normalized-output validation, and no write-capable tools. Then review the gate requirements before any cost-incurring replay. No model promotion or write authority is implied. Preserve all clinical, deployment, provider-PHI, historical-trove runtime, memory/RAG/GraphRAG, and production gates. Dependabot alert 5 remains open; do not force overrides. |
