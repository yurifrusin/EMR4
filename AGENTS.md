...
| **Current active track** | Sprint 274 practitioner-directory Office add-in GraphQL consumer proposal is complete locally: `docs/api-spine/practitioner-directory-office-addin-graphql-consumer-proposal.{json,md}` and `tests/test_practitioner_directory_office_addin_graphql_consumer_proposal.py` define a proposal-only comparison boundary for future taskpane use of `Query.practice.practitioners`. Antigravity returned PASS on Office add-in consumer/UX guardrails through `agy.exe`; DeepSeek returned PASS on API/static guardrails; Claude was deliberately stood down because no API contract or resolver shape changed. The proposal keeps the existing `2026-08-06` expiry, forbids any Sprint 274 hidden flag/runtime shadow fetch/client telemetry/taskpane change, separates HTTP `401` from GraphQL `extensions.code`, preserves the approved field projection, and keeps all readiness, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, and field-expansion gates closed. |
| **Next recommended work** | Continue to Sprint 275 only as a blocked-by-default Office add-in GraphQL fetch-wrapper test plan: mocked taskpane success/error/projection/fallback tests and a switch-gate checklist, with no runtime traffic until a separate consumer switch approval. Use Antigravity only if the sprint touches user-visible taskpane behavior or copy; otherwise use DeepSeek for static guardrails and keep Claude stood down unless the API contract changes. D5 expansion remains paused unless separately approved. |
