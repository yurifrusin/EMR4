...
| **Current active track** | T1 Stateful Diary Scenario Laboratory is complete through T1.3. The harness now supports deterministic scenario-local time, allowlisted seeded and mid-scenario external appointment state, DB-backed supervised booking and confirmation, exact product-write deltas separated from fixture events, and redacted evidence. Golden cases cover create-confirm-repeat exact duplicate, true overlap, same-day distinct, terminal status, and a separately committed concurrent booking that blocks stale confirmation. A focused Playwright flow proves the authoritative first receipt, repeated existing-booking state, accessible alternative actions, coherent focus, and exactly one confirm request. T1 also corrected the diary projection so root `existing_booking` details reach the UI. Details are in `docs/bernie-t1-stateful-scenario-laboratory.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T2 Deterministic Receptionist Behaviour Matrix, using the T1 stateful laboratory to pin exact duplicate/overlap/same-day/terminal/no-slot/ambiguity/concurrency semantics and receptionist copy. Preserve E1 route truth and focused E3 UI-contract checks. Keep autonomous/model-to-database writes, patient-specific consultant runtime, reception triage, PHI-enabled new providers, live clinical pilots, deployment/release, external clients, H15/historical-trove runtime use, memory/RAG/GraphRAG authority, and GraphQL mutations closed. Dependabot alert 5 remains open; do not force overrides. |
