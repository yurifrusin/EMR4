...
| **Current active track** | Sprint 202 Fable-aligned API Spine appointment read-model route inventory integrated in implementation commit `3564f9d0`: a static markdown inventory and parser test now tie GraphQL appointment/diary/audit/Bernie read roots to current appointment-router GET/read routes, mark partial/external/unmapped coverage, and keep legacy compatibility writes outside the read graph without touching runtime routes, providers, database, H15/H-series, trove, memory/RAG/GraphRAG, or GraphQL mutations |
| **Next recommended work** | Continue bounded backend-readiness with a command/read-model deprecation map for legacy compatibility writes or a static GraphQL read-root external-router inventory for viewer/practice/patient/directory roots; use Ariadne plus three worker lanes for general sprint work (Claude through `scripts\drive_agent_headless.py` with `--mint-session` and durable handin/submit packets, Antigravity via `C:\Users\sarashera\AppData\Local\agy\bin\agy.exe`, and DeepSeek, with extra DeepSeek substitutions when a lane is unavailable), let DeepSeek workers finish before integration, keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, or model-to-database-write gates |
