...
| Next implementation | Under Yuri's standing planned-gate authority, the next safe descendant is a separately derived and frozen authored-synthetic model-required intent-shaping rehearsal over the accepted deterministic retrieval contract. The plan must bind exact `gemini-2.5-flash` through the Bernie Vertex development project in `australia-southeast1`, retain a positive thinking budget, closed schema, deterministic proofreader, exact call/cost ledger, keyless ADC, no fallback and zero patient/product/runtime/command authority before any dispatch. Patient/product/protected data, real sources, watchers, event transport, persistence, operational retention, external evidence retrieval, ordinary services, product runtime/routes, commands, writes, deployment, production, release, Pages, protected evidence and protected-ref movement remain closed. Consultant, requests/referrals, prescribing/medicines, billing/claims and other future Bureau implementation authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
