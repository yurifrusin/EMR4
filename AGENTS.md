...
| Ariadne agent error and correction register acceptance | Revision 394: `docs/ariadne-agent-error-correction-register-revision-394.md`; 453 bounded incidents, all corrected/contained and none open. AER-0452 corrects a manually expanded plan-source SHA against machine `git rev-parse`; AER-0453 replaces the native Harness sparse-worktree/credential-isolation claim with a container and isolated-broker enclosure before any occupied dispatch. Durable state: `orchestration/continuity/ariadne-agent-error-register/`. |
