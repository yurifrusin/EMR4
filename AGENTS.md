...
| Next implementation | Under Yuri's standing uninterrupted-gate authority, the next safe descendant is the candidate plan in `docs/raisa-provider-free-disposable-postgresql-durability-parse-catalogue-rehearsal-plan.md`. Before any Docker or PostgreSQL execution, deterministic plan checks and a fresh exact-HEAD independent veto must pass. The frozen candidate permits only the exact accepted inert artifact, an already-local `postgres:16-bookworm` image with `--pull=never`, one owned `--network=none` disposable container, four empty repository-authored synthetic prerequisite table shapes, atomic SQL admission, fixed catalogue/privilege readback and exact-ID owned cleanup. Missing client/daemon/image fails without install, pull or fallback. Function/trigger/RLS behavior, concurrency, applied Alembic migration, operational credentials or persistence, live outbox/feed/watcher/listener/source access, product/patient data, application/API/Diary changes, providers, commands/writes, deployment, production, release, Pages and protected-ref movement remain later or closed gates. Cross-Bureau/clinical implementation and Consultant, requests/referrals, prescribing/medicines and billing/claims authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
