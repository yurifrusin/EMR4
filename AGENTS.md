...
| **Current active track** | Sprint 156 status/delete confirm client header emission implemented in commit `475cfef` and closed/pushed through `4dc1f23`; Diary status-confirm and delete-confirm now send freshness-derived HTTP `Idempotency-Key` headers while raw PATCH/DELETE fallbacks stay header-free |
