...
| **Current active track** | Sprint 281 practitioner-directory Office add-in GraphQL selector default-on runtime is complete locally. Yuri approved the Sprint 280 packet with "I approve the default-on packet." on 2026-07-09. `docs/diary/diary.js` now has `ENABLE_GRAPHQL_PRACTITIONERS = true` for the single `office_addin_diary_booking_practitioner_selector` consumer, with no runtime/user override and REST fallback retained. Sprint 281 evidence lives in `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-runtime.{json,md}` and `review/test_diary_graphql_practitioner_switch.py`, proving GraphQL-first success, approved variables, one-shot REST fallback for `FORBIDDEN`, `BAD_USER_INPUT`, and transport failure, GraphQL HTTP 401 logout/no-REST-fallback behavior, `practice=null` empty/no-REST behavior, `defaultLocation=null` row preservation, and sensitive canaries absent from rendered page text. DeepSeek returned PASS after requiring and verifying the 401 test. Antigravity was invoked for Office add-in UX review but timed out; Claude was stood down because no API contract/backend/schema/auth model changed. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, runtime-user-override, or field-expansion gate is opened. |
| **Next recommended work** | Continue only with post-default-on safety evidence such as a narrow non-intercepted local backend fake-data check or rollback/monitoring packet. Do not claim deployment, production, global GraphQL readiness, telemetry, external-client readiness, provider/memory/H15/trove access, writes, mutations, subscriptions, or field expansion without a separate approval. D5 expansion remains paused unless separately approved. |
