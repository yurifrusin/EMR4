...
| **Current active track** | Ariadne is a portable Markdown/YAML/script protocol, not a dedicated user platform, GUI, native runtime, or agent-launch layer. `docs/ariadne-harness-conductor-verifier-protocol.md` separates authority: the conductor plans and allocates workers (Fable preferred, Opus fallback); the verifier checks that plan against settings (DeepSeek Flash preferred); the orchestrator alone vets submissions, integrates, commits, and pushes `master`. The orchestrator must not reassign verified work; changes in availability return to conductor then verifier. S4a provides strict worker-pool/allocation schemas and S4b adds `orchestration_harness/allocator.py`, a pure deterministic allocator replayed against authored synthetic availability probes. Transport is explicitly distinct from time-bounded reachability/quota observation; a DeepSeek bridge result is neither capability nor authority. The normal three-resource and constrained single-generalist scenarios pass deterministically. A generalist must cover every SSDLC role and any fallback is labelled `self_review` with its reduced independence; unfillable required roles remain explicit. No live worker adapter, availability probe, enforcement, conductor/verifier branch movement, runtime wiring, D5 expansion, frontend JavaScript change, provider/live-provider wiring, Access AI, memory/RAG/GraphRAG, H15/H-series, historical diary runtime input, GraphQL delivery/readiness, external patient client, confirm payload/write behavior change, model-to-database write, deployment, production, telemetry, global readiness gate, autonomous release authority, or EMR4 runtime behavior is opened. |
