...
| **Current active track** | Sprint 204 API Spine legacy compatibility write deprecation map integrated in implementation commit `28d5f617`: a static map now ties each raw appointment compatibility write to its proposal/confirm replacement family, read-model witness routes, raw compatibility evidence tag, and current `audit/header/off` signal posture without changing runtime behavior |
| **Next recommended work** | Continue bounded backend-readiness with a static GraphQL read-root external-router inventory for viewer/practice/patient/directory roots, or add a raw-compat consumer/signal readiness preflight before any deprecation-header mode change; use Ariadne plus three worker lanes for general sprint work (Claude through `scripts\drive_agent_headless.py` with `--mint-session` and durable handin/submit packets, Antigravity via `C:\Users\sarashera\AppData\Local\agy\bin\agy.exe`, and DeepSeek, with extra DeepSeek substitutions when a lane is unavailable), let DeepSeek workers finish before integration, keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, or model-to-database-write gates |
