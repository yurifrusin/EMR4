...
| **Current active track** | Sprint 266 practitioner-directory GraphQL SDL alignment is complete locally: the non-runtime SDL now uses `PracticeLocationBrief { id, name }`, changes `Practitioner.defaultLocation` to `PracticeLocationBrief`, and adds `limit: Int = 50` plus `offset: Int = 0` to `Practice.practitioners`. The Sprint 229 drift contract and Sprint 228/231 guard tests now record those former drifts as resolved while keeping GraphQL resolver/server/runtime readiness false. DeepSeek returned PASS. No GraphQL runtime dependency, GraphQL endpoint, resolver code, deployment, production, external-client, write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. |
| **Next recommended work** | Prepare the practitioner-directory GraphQL runtime/resolver approval gate or packet before adding any GraphQL dependency/server/resolver code, because this will be the first GraphQL runtime surface in the repo. The gate should choose the runtime library, auth context strategy, error taxonomy, depth/cost posture, and test matrix. Keep readiness flags false until implementation and verification are complete. D5 expansion remains paused unless separately approved. Use each worker only where it has a distinct artifact or veto surface. |
