...
| **Current active track** | Sprint 272 practitioner-directory GraphQL release-boundary packet is complete locally as a proposed approval surface, not an approval. Evidence lives in `docs/api-spine/practitioner-directory-graphql-release-boundary.{json,md}` and tests in `tests/test_practitioner_directory_graphql_release_boundary.py`. The packet proposes internal authenticated staff consumer development for `Query.practice.practitioners` only, but `internal_consumer_development=false` until Yuri supplies an approval slip with `approved_contract_commit`, `go_no_go_acknowledged=true`, and `approval_expires_on`. Global readiness snapshots remain blocked and no mutation/subscription, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. DeepSeek returned PASS, but also correctly identified Yuri approval as the next decision point. |
| **Next recommended work** | Pause for Yuri go/no-go on the Sprint 272 GraphQL release-boundary packet. If approved, update the approval slip exactly for commit `pending until Sprint 272 commit is pushed`, then run the release-boundary and hardening suites before allowing internal authenticated staff consumer development. If not approved, keep GraphQL implemented/tested but not internally consumable beyond test harnesses. D5 expansion remains paused unless separately approved. Use workers only when they have a distinct artifact or veto surface. |
