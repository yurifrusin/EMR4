...
| **Current active track** | Ariadne is now specified as a portable Markdown/YAML/script protocol, not a dedicated user platform, GUI, native runtime, or agent-launch layer. `docs/ariadne-harness-conductor-verifier-protocol.md` separates authority: the conductor plans and allocates workers (Fable preferred, Opus fallback); the verifier checks that plan against settings (DeepSeek Flash preferred); the orchestrator alone vets submissions, integrates, commits, and pushes `master`. The orchestrator must not reassign verified work; changes in availability return to conductor then verifier. No live worker adapter, enforcement, conductor/verifier branch movement, runtime wiring, D5 expansion, frontend JavaScript change, provider/live-provider wiring, Access AI, memory/RAG/GraphRAG, H15/H-series, historical diary runtime input, GraphQL delivery/readiness, external patient client, confirm payload/write behavior change, model-to-database write, deployment, production, telemetry, global readiness gate, autonomous release authority, or EMR4 runtime behavior is opened. |
| **Next recommended work** | The conductor-verifier protocol now records high-assurance worker write protection: sandboxed workers receive read-only source and output only patch/evidence artifacts; the verifier admits them and a separate protected orchestrator identity alone applies, verifies, commits, and pushes `master`. Shared-user worktrees/sparse checkouts are explicitly lower-assurance compatibility mode. Security sprint 2026-07-11 upgraded `EMR4 Sidebar` direct `webpack-dev-server` to 6.0.0 and `office-addin-manifest` to 2.1.6, removing their direct vulnerable uuid paths; production dependency audit is clean and add-in build/manifest validation pass. Dependabot alert 5 remains open because six development-only Microsoft toolkit transitive dependencies still resolve `uuid@8.3.2`; do not force transitive overrides. The Ariadne S4a-S4d worker-pool work remains advisory-only and D5 remains closed unless Yuri separately authorizes the draft-only route-intercepted copy-evidence option. |
