...
| **Current active track** | Sprint 277 practitioner-directory Office add-in GraphQL consumer switch approval packet is complete locally and pending Yuri approval: `docs/api-spine/practitioner-directory-office-addin-graphql-consumer-switch-approval-packet.{json,md}` and `tests/test_practitioner_directory_office_addin_graphql_consumer_switch_approval_packet.py` define the exact default-off/internal-only switch surface for a future `Query.practice.practitioners` taskpane implementation. Antigravity returned PASS on consumer/UX release guardrails through `agy.exe`; DeepSeek returned PASS with concerns integrated; Claude was deliberately stood down because no API contract, resolver, backend route, schema, or runtime auth/error model changed. The packet consumes the existing `2026-08-06` runway, specifies a source-controlled build-time/static taskpane gate defaulting false, forbids runtime user overrides, defines one-shot REST fallback for GraphQL non-401 transport/body failures, separates `practice=null` from ordinary empty lists, drops malformed rows, keeps all runtime implementation/live traffic/taskpane edits unauthorized now, and keeps deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, and field-expansion gates closed. |
| **Next recommended work** | Stop for Yuri approval. If Yuri explicitly approves `approved_for_default_off_office_addin_graphql_practitioner_selector_switch` with expiry `2026-08-06`, Sprint 278 may implement only the default-off Office add-in practitioner selector GraphQL path under the packet. Without that approval, do not edit `EMR4 Sidebar/src/taskpane/taskpane.js` or send GraphQL traffic from the Office add-in. D5 expansion remains paused unless separately approved. |
