...
| **Current active track** | Sprint 270 practitioner-directory GraphQL resolver is complete locally: `/api/v1/graphql` now exposes `Query.practice(id)` and the single approved `Practice.practitioners(activeOnly: Boolean = true, limit: Int = 50, offset: Int = 0)` field. The resolver uses only `app/services/practice/practitioner_directory_read.py::list_practitioner_directory`, converts REST projection fields to GraphQL types, returns `null` for mismatched `practice(id)` without querying other practices, maps invalid bounds to `BAD_USER_INPUT`, unauthorized inactive reads to `FORBIDDEN`, and unexpected resolver failures to `INTERNAL_ERROR`. Evidence lives in `docs/api-spine/practitioner-directory-graphql-resolver-runtime.{json,md}` with tests in `tests/test_practitioner_directory_graphql_resolver.py`. No mutation/subscription, readiness flag change, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. DeepSeek returned PASS. |
| **Next recommended work** | Continue to Sprint 271: harden the GraphQL practitioner contract with release-boundary evidence, schema/runtime parity checks, depth/alias/token negative tests if practical, and explicit confirmation that global readiness snapshots remain blocked despite the scoped resolver. D5 expansion remains paused unless separately approved. Use DeepSeek for security/boundary review; use Claude/Antigravity only if an available lane has a distinct artifact or veto surface, such as a backend/API or consumer/privacy review surface, rather than ritual enumeration. |
