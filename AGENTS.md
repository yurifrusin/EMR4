...
| Ariadne agent error and correction register acceptance | Revision 402: `docs/ariadne-agent-error-correction-register-revision-402.md`; 463 bounded incidents, all corrected/contained and none open. AER-0452 corrects a manually expanded plan-source SHA; AER-0453 replaces the native Harness sparse-worktree/credential-isolation claim with a container and isolated-broker enclosure; AER-0454 contains the provider-free rc.7 read-only-rootfs/HMR incompatibility; AER-0455 restores one-process-per-gate validation; AER-0456 corrects the sparse-worker provider-free tools-mode enum; AER-0457 corrects the first worker pre-dispatch state; AER-0458 restores the no-pipeline readback control; AER-0459 contains the incomplete model-facing tool inventory and consumes the rejected occupied attempt without retry; AER-0460 corrects the overlong precommit latch checkpoint; AER-0461 and AER-0462 correct the Continuity node's path and typed evidence inventories; AER-0463 corrects the pre-existing stale Compass current-node sentinel. Durable state: `orchestration/continuity/ariadne-agent-error-register/`. |
