...
| **Current active track** | Ariadne is a portable Markdown/YAML/script protocol, not a dedicated user platform, GUI, native runtime, or agent-launch layer. `CLAUDE.md` now expressly defines Claude as conductor only when it receives an Ariadne planning invocation: Fable/high reasoning is preferred and Opus/medium is the ordinary fallback; Claude cannot launch workers, alter a verifier-passed allocation, integrate, commit, or push. S4a/S4b provide strict settings and a synthetic-only pure allocator. The pool distinguishes the DeepSeek Flash verifier from `deepseek-flash-workers` (one to three bounded worker lanes) and includes the Antigravity platform resource `antigravity-gemini-flash-3-5-worker` (zero or one current lane). Antigravity is the agentic platform/transport; Gemini Flash 3.5 is its current default model, while future Gemini models or additional Flash instances require their own worker-pool entries and capacity settings. `orchestration/harness_settings/sprint_worker_policy.yaml` tells the conductor to decide the Antigravity artifact/veto lane and DeepSeek lane count from boundary, separability, ownership, verification, resources, cost, and user override inputs. DeepSeek Flash verifies the submitted plan against the same settings; the verifier cannot choose/reassign workers or integrate. S4c has not run and remains the required later advisory plan/report CLI. No live worker adapter, availability probe, enforcement, conductor/verifier branch movement, runtime wiring, D5 expansion, frontend JavaScript change, provider/live-provider wiring, Access AI, memory/RAG/GraphRAG, H15/H-series, historical diary runtime input, GraphQL delivery/readiness, external patient client, confirm payload/write behavior change, model-to-database write, deployment, production, telemetry, global readiness gate, autonomous release authority, or EMR4 runtime behavior is opened. |
| **Next recommended work** | Keep S4c closed pending Yuri approval: an advisory CLI that reads the committed settings plus supplied local synthetic probe JSON and writes a plan/report artifact only. It must not probe providers, launch agents, write worktrees, or grant authority. `docs/ariadne-harness-bootstrapper-strategy.md` records the proposed first end-user experience: a conversational bootstrap agent performs read-only discovery, asks plain-language setup questions, proposes generated YAML/Markdown settings, requests scoped writes, and emits manual steps for credentials or OS actions. The first pilot is dry-run discovery against EMR4 followed by a disposable no-master-credential clone, never the active integration checkout. The conductor-verifier protocol records high-assurance worker write protection; shared-user worktrees are lower-assurance compatibility mode. Security sprint 2026-07-11 removed direct vulnerable uuid paths but Dependabot alert 5 remains open through six development-only Microsoft toolkit transitive dependencies; do not force overrides. D5 remains closed unless Yuri separately authorizes the draft-only route-intercepted copy-evidence option. |
