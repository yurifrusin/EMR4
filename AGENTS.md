...
| Model-required Bureau A3/B3 request-contract recovery active plan | `docs/emr4-model-required-bureau-a3-b3-request-contract-recovery-plan.md`, `docs/security/emr4-model-required-bureau-a3-b3-request-contract-recovery-threat-model-delta.md`, `orchestration/continuity/model-required-bureau-a3-b3-request-contract-recovery/`, `scripts/model_required_bureau_a3_b3_recovery_contracts.py`, `scripts/model_required_bureau_a3_b3_recovery_broker.py`, `scripts/model_required_bureau_a3_b3_recovery_live.py`, `scripts/model_required_bureau_a3_b3_recovery_acceptance.py`, `tests/test_model_required_bureau_a3_b3_request_contract_recovery.py`, and `orchestration/agent_inbox/codex/model-required-bureau-a3-b3-request-contract-recovery-preplan-receipt.json` |
| Current result | The immutable accepted parent remains `model_required_bureau_a3_b3_occupied_terminal_rejection`: one restored-ADC Rayleen call failed content-shape admission before proofreading, released nothing and never started Davida. Yuri has now authorised uninterrupted bounded request-contract recovery under the same exact Vertex `gemini-2.5-flash` / Bernie project, identity and Sydney regional boundary. The active provider-free descendant replaces unbounded automatic thinking plus 512 total output tokens with a positive fixed 1,024-token thinking budget and 2,048-token output cap, adds raw-free response-shape telemetry, uses wholly distinct ids/accounting/runtime names and admits both authored-synthetic lanes with zero provider calls and complete cleanup. The earlier zero-thinking provider-free rehearsal is preserved as superseded diagnostic evidence. No occupied recovery call has yet occurred. |
| Next implementation | Commit the exact tracked-clean recovery candidate to the non-protected task branch, obtain a fresh exact-HEAD Gemini 3.6 Flash/high source veto, rerun the read-only Bernie Vertex preflight, then execute Rayleen before Davida under the new single-use ledgers. Continue through distinct in-scope remedies until both lanes admit or the four-call/USD 1 envelope is exhausted; never issue an unchanged retry or start Davida before Rayleen release. A4/A5/B4, C4, product/runtime/data/write surfaces, deployment, release, Pages and protected refs remain closed. Preserve and exclude `docs/branding/` and the four existing Consultant/Gate-minus-one pre-push files. |
