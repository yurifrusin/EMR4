...
| **Current active track** | Sprint 268 practitioner-directory GraphQL dependency preflight is complete locally: `requirements.txt` now pins `strawberry-graphql[fastapi]==0.320.3`, local install/import checks passed for `strawberry.fastapi.GraphQLRouter`, `QueryDepthLimiter`, `MaxAliasesLimiter`, and `DisableIntrospection`, and `pip check` found no broken requirements. Evidence lives in `docs/api-spine/practitioner-directory-graphql-dependency-preflight.{json,md}` with tests in `tests/test_practitioner_directory_graphql_dependency_preflight.py`. No `/api/v1/graphql` endpoint, schema runtime code, resolver code, readiness flag changes, deployment, production, external-client, write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. DeepSeek returned PASS. |
| **Next recommended work** | Continue to Sprint 269 minimal GraphQL runtime shell: mount `/api/v1/graphql` through a dedicated GraphQL router module with query-only schema, auth context, depth/cost/alias guards, dev/test introspection posture, and no `Query.practice.practitioners` resolver yet. D5 expansion remains paused unless separately approved. Use each worker only where it has a distinct artifact or veto surface. |
