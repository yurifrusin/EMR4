...
| Current result | The provider-free patient-free unmounted intent-shaped temporal retrieval rehearsal passes as `raisa_provider_free_practice_context_fabric_intent_shaped_temporal_retrieval_rehearsal_pass` at exact independently reviewed source HEAD `b24b56bda296f3713b5e2c0e52545c749e71540a`. Five closed intents select only the minimum granted Current, Bureau Memory and bitemporal Historical components. The backend binding—not a workspace brand—owns Bureau, purpose, session, component, time and disclosure authority; the accepted lowercase/uppercase upstream vocabularies use an exact mapping; all upstream proofreaders are recomputed; the catalog digest is binding-sealed; Current remains four-source atomic and same-Bureau because it contains private application-session state; Memory crosses only under an exact bilateral grant; invalidated or expired Current and missing history fail closed; and ambiguous opaque references return alternatives without identity assertion. A fresh Gemini 3.6 Flash/high Antigravity veto returned one schema-constrained `pass` at the unchanged clean HEAD and ran 199/199 named provider-free tests. This proves only pure authored-synthetic intent-to-context selection, not natural-language/provider behaviour, patient/product access, live retrieval, database/watcher, persistence/retention, runtime, clinical or administrative command authority. |
| Next implementation | The exact authored-synthetic model-required intent-shaping descendant is now frozen in `docs/raisa-authored-synthetic-model-required-practice-context-fabric-intent-shaping-rehearsal-plan.md`, its design and threat delta. Implement its provider-free contract and isolated one-use controller, pass deterministic gates and a fresh exact-HEAD Gemini 3.6 Flash/high source veto, then under Yuri's standing authority run the read-only preflight and at most two `gemini-2.5-flash` calls through `bernie-emr4-dev`, Bernie keyless impersonated ADC and `australia-southeast1`. The request retains `thinkingBudget: 1024`, `maxOutputTokens: 2048`, one candidate, USD 0.50 cumulative, no fallback and zero patient/product/API/runtime/command authority. Patient/product/protected data, real sources, watchers, event transport, persistence, operational retention, external evidence retrieval, ordinary services, product runtime/routes, commands, writes, deployment, production, release, Pages, protected evidence and protected-ref movement remain closed. Consultant, requests/referrals, prescribing/medicines, billing/claims and other future Bureau implementation authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
