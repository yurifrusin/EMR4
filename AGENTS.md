...
| **Current active track** | Sprint 235 practitioner-directory post-implementation review added `docs/api-spine/practitioner-directory-post-implementation-readiness-review.json` and `.md`, recording that the bounded `GET /api/v1/practice/practitioners` slice at `5b3b9102` is implemented and runtime-tested while `rest_route_ready`, SDL/GraphQL, providers, memory/RAG/GraphRAG, H15/H-series/trove imports, writes, deployment, and production readiness remain false/blocked |
| **Next recommended work** | Continue Fable's approved near-term Bernie UI derived-state DAG lane with D1/D2 only: amend the plan per Fable's required changes, add a pure provider-free/route-free/DB-free `app/services/bernie/ui_view_model.py` selector with authored synthetic fixtures/tests, and keep D3/D4 UI wiring behind the already completed prompt-thread fake-provider backend pass and a separate review; do not open provider, memory, GraphQL, write, H15/H-series/trove, or practitioner-directory readiness gates without explicit review; at every sprint start explicitly announce Claude, Antigravity, and DeepSeek use/non-use, invocation mode (`scripts\drive_agent_headless.py`/Claude CLI, `agy.exe`/Antigravity CLI, direct Codex `deepseek-worker` spawn), Claude/Antigravity/DeepSeek cleanliness state, current DeepSeek lane count and reuse/cleanup plan, and any extra DeepSeek substitution for unavailable Claude/Antigravity lanes; clean stale worker artifacts before progressing and reannounce clean state |
