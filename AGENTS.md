...
| Next implementation | Under Yuri's standing authority, the next safe descendant is the provider-free unmounted Rayleen invalidation/reassembly seam. It may bind the accepted adapter-built source envelope and immutable parent frame set to the accepted patient-free temporal dependency manifest, process only typed payload-free authored-synthetic invalidation signals, retire the old set and emit one inert requirement for a fresh authorised adapter/assembler run. It may not listen to a live database/feed, patch the old frame, treat event metadata as truth, persist state, mount a route, read product data, call a provider or execute a command. Any live real-data read, watcher/feed, new route, cross-Bureau/clinical source, command or write remains a separately closed boundary. Patient/product/protected data, real sources, event transport, persistence, operational retention, external evidence retrieval, ordinary services, product runtime/routes, commands, writes, deployment, production, release, Pages, protected evidence and protected-ref movement remain closed. Consultant, requests/referrals, prescribing/medicines, billing/claims and other future Bureau implementation authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
