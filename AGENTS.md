...
| Current result | The exact A3/B3 authored-synthetic occupied advisory boundary is active under Yuri's selected Vertex AI `gemini-2.5-flash`, Bernie project/service-account/keyless-ADC, `australia-southeast1`, no-fallback, four-call and USD 1 ceilings. Provider-free readiness and source review 4 passed, but the first occupied launch stopped at `impersonated_adc_refresh_failed` before any prompt, provider call, inference, attempt ledger or cell. Candidate-runtime call count is zero. AER-0016 preserves the exact open Rayleen USD 0.25 reservation and adds hash-bound same-reservation resume controls; this corrected recovery source still requires a fresh exact-HEAD veto. Checkpoint: `docs/emr4-model-required-bureau-a3-b3-preflight-blocked-checkpoint.md`. |
| Next implementation | Yuri restores the already approved Bernie keyless impersonated ADC outside the occupied runtime; Codex may not revoke, replace, recreate or reconfigure it. After restoration and the pending fresh recovery-source veto, rerun the exact read-only preflight and continue serial A3 then B3 only if every control passes. A4/A5/B4, C4, update/migration activation, product/runtime/data/write surfaces, deployment, release, Pages and protected refs remain closed. Preserve and exclude `docs/branding/` and the four existing Consultant/Gate-minus-one pre-push files. |
...
or candidate-runtime provider call. Restoring that existing ADC is now the one
human-only continuation action; this grants Codex no credential, IAM or cloud-
