...
| **Current active track** | Sprint 264 practitioner-directory internal runtime consumer wiring is complete locally: the Office add-in Diary booking practitioner selector/list now loads `GET /api/v1/practice/practitioners?activeOnly=true&limit=200` through existing `apiFetch`, normalizes only `id`, `displayName`, and display-safe `defaultLocation`, prefers route UUIDs when selection can be preserved, and falls back to legacy template/AHPRA behavior for older columns. The Sprint 263 approval packet was corrected to the actual camelCase REST field names. DeepSeek returned PASS; no GraphQL/provider/memory/write/deployment/external-client/H15/trove gates were opened. |
| **Next recommended work** | Sprint 265 should collect runtime/browser evidence for the named Office add-in Diary booking practitioner selector/list consumer: endpoint request observed, existing staff auth path, tenancy/practice scoping, no sensitive fields, no writes, no provider/memory/H15/trove access, legacy fallback behavior, and the `limit=200` cap posture. If that evidence passes, move next to the already planned practitioner-directory GraphQL resolver/SDL alignment block; otherwise fix REST-consumer evidence gaps first. D5 expansion remains paused unless separately approved. Use each worker only where it has a distinct artifact or veto surface. |
