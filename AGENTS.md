...
| **Current active track** | T3 Nondeterministic Bernie Shadow Evaluation is in progress after T2 completed at `b8cd2078`. T3.1 adds the provider-neutral, immutable contract and model-version ledger in `app/services/ai/evals/bernie_shadow_eval.py`, with exact deterministic scoring for intent, entities, date/time, clarification, and tool selection. Authority expansion fails separately; cost and latency cannot affect correctness. The module has no provider, route, database, diary mutation, or persistence imports. Details are in `docs/bernie-t3-shadow-evaluation.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T3.2: project an explicitly selected authored-synthetic subset of the T1/T2 corpus into `ShadowCase` values through a source-safe loader. Reject PHI-shaped fields, generated expectations, mutable diary state, and expected tools absent from the case allowlist. Live provider calls remain closed until the corpus boundary and adapter contract are reviewed. No model promotion or write authority is implied. Preserve all clinical, deployment, provider-PHI, historical-trove runtime, memory/RAG/GraphRAG, and production gates. Dependabot alert 5 remains open; do not force overrides. |
