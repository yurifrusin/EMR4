...
| **Current active track** | Sprint 284 rollback packet for the already default-on practitioner-directory Office add-in GraphQL selector is complete locally and pending worker review/commit/push. Sprint 281 default-on runtime is pushed at `d3dda16e657a4eb51b845a509c5cff071f530c43`, Sprint 282 publication status at `514970f656eb55991c3c0def62bfd6d522ea3f74`, and Sprint 283 local backend smoke at `7fe7565e6c33c17d17dc5547c0f372ac00fdb4c0`. `docs/diary/diary.js` still has `ENABLE_GRAPHQL_PRACTITIONERS = true` for the single `office_addin_diary_booking_practitioner_selector` consumer, with no runtime/user override and REST fallback retained. Sprint 284 adds `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-rollback-packet.{json,md}` and guard tests defining the exact one-line rollback to `ENABLE_GRAPHQL_PRACTITIONERS = false` if needed, while explicitly not rolling back now and not deleting the GraphQL query, resolver, backend route, or REST fallback. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, runtime-user-override, or field-expansion gate is opened. |
| **Next recommended work** | If Sprint 284 review passes, close and push it, then continue only to a small post-default-on monitoring/readiness-boundary packet or pause before any deployment/production readiness claim. Do not claim deployment, production, global GraphQL readiness, telemetry, external-client readiness, provider/memory/H15/trove access, writes, mutations, subscriptions, or field expansion without a separate approval. D5 expansion remains paused unless separately approved. |
