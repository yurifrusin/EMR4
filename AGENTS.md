...
| **Current active track** | Sprint 283 local backend smoke evidence for the already default-on practitioner-directory Office add-in GraphQL selector is complete locally and pending worker review/commit/push. Sprint 281 is pushed at `d3dda16e657a4eb51b845a509c5cff071f530c43`, Sprint 282 publication status is pushed at `514970f656eb55991c3c0def62bfd6d522ea3f74`, and `docs/diary/diary.js` has `ENABLE_GRAPHQL_PRACTITIONERS = true` for the single `office_addin_diary_booking_practitioner_selector` consumer, with no runtime/user override and REST fallback retained. Sprint 283 adds `docs/api-spine/practitioner-directory-office-addin-graphql-default-on-local-backend-smoke.{json,md}` and `tests/test_practitioner_directory_office_addin_graphql_default_on_local_backend_smoke.py`, extracting the committed Office add-in GraphQL document and posting it through local FastAPI `TestClient` `/api/v1/graphql` with fake DB rows. It proves practice scoping, active-only filtering, default-location projection, sensitive canary absence, and no audit writes without route interception. No deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, telemetry, readiness, server-config-endpoint, runtime-user-override, or field-expansion gate is opened. |
| **Next recommended work** | If Sprint 283 review passes, close and push it, then continue to Sprint 284 rollback packet for the same single Office add-in practitioner selector consumer. Do not claim deployment, production, global GraphQL readiness, telemetry, external-client readiness, provider/memory/H15/trove access, writes, mutations, subscriptions, or field expansion without a separate approval. D5 expansion remains paused unless separately approved. |
