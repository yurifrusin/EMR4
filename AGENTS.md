...
| **Current active track** | T2 Deterministic Bernie Behaviour Matrix is complete through T2.1. The routine gate now executes 260 generated half-open interval scenarios in about five seconds, supplemented by 15 independently authored boundary cases, the existing classifier/slot-search corpus, and three route-intercepted Playwright outcome scenarios. T2.1 corrected async-result focus loss, the Edit request selector, and canonical `roster_unavailable` view-model mapping without adding confirmation authority. Details are in `docs/bernie-t2-deterministic-behaviour-matrix.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T2.2: extend the economical generated matrix across active/terminal status, practitioner/location, roster/break, normalized bounds, and stale-proposal axes while retaining separate authored DB-backed semantic and persistence truth. Preserve E1 route truth and focused E3 UI-contract checks. Keep autonomous/model-to-database writes, patient-specific consultant runtime, reception triage, PHI-enabled new providers, live clinical pilots, deployment/release, external clients, H15/historical-trove runtime use, memory/RAG/GraphRAG authority, and GraphQL mutations closed. Dependabot alert 5 remains open; do not force overrides. |
