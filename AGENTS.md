...
| **Current active track** | Sprint 272 practitioner-directory GraphQL release-boundary packet is approved for scoped internal authenticated staff consumer development of `Query.practice.practitioners` only through `2026-08-06`. Evidence lives in `docs/api-spine/practitioner-directory-graphql-release-boundary.{json,md}` and tests in `tests/test_practitioner_directory_graphql_release_boundary.py`. The approval slip records `approved_contract_commit=d4ed14d3`, `go_no_go_acknowledged=true`, and `approval_expires_on=2026-08-06`; `internal_consumer_development=true` is the only newly authorized release-boundary value. Global readiness snapshots remain blocked and no mutation/subscription, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, or trove gate has been opened. |
| **Next recommended work** | Continue only with internal authenticated staff consumer development against `Query.practice.practitioners`, bounded by the approved field set and expiry `2026-08-06`, or run a new approval packet before any global readiness, deployment, production, external-client, write, provider, memory, H15/trove, mutation, subscription, or field-expansion work. D5 expansion remains paused unless separately approved. Use workers only when they have a distinct artifact or veto surface. |
