...
| **Current active track** | Sprint 205 API Spine external-router read-root inventory integrated in implementation commit `547ddd18`: a static map now ties the GraphQL roots `viewer`, `practice`, `patient`, and `directorySearch` to current auth/diary/patient/clinical/search read sources and explicit read-model gaps without creating GraphQL resolvers or changing runtime behavior |
| **Next recommended work** | Continue bounded backend-readiness with a raw-compat consumer/signal readiness preflight before any deprecation-header mode change, or add a small read-model gap inventory for the newly explicit practitioner/reminder/message/directory gaps; use Ariadne plus three worker lanes for general sprint work (Claude through `scripts\drive_agent_headless.py` with `--mint-session` and durable handin/submit packets, Antigravity via `C:\Users\sarashera\AppData\Local\agy\bin\agy.exe`, and DeepSeek, with extra DeepSeek substitutions when a lane is unavailable), let DeepSeek workers finish before integration, keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, or model-to-database-write gates |
