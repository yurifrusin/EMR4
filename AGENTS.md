...
| **Current active track** | Sidecar Ariadne multi-agent SSDLC harness blueprint checkpoint. Previous Bernie UI derived-state publication work is already on `master`/`handoff/current` through Sprint 290 and the AGENTS DeepSeek protocol repair at `4e046a46`. Yuri asked to preserve the new orchestration-harness ideas as permanent documentation. The blueprint in `docs/ariadne-multi-agent-ssdlc-harness-blueprint.md` now captures a cross-platform, SSDLC-native, mandate-bounded orchestration harness: user-approved operating envelopes, deterministic authority outside the orchestrator LLM, elastic role obligations that work with one or many agents, worker packets, drift detectors, evidence gates, a context rehydration gate for fresh/compacted sessions, Codex-first but platform-neutral adapters, optional later GUI/Omnigent integration, and an EMR4-first then extract-later strategy. This is documentation-only and opens no live subagent execution, D5 expansion, route delivery, frontend JavaScript change, provider/live-provider wiring, Access AI, memory/RAG/GraphRAG, H15/H-series, historical diary runtime input, GraphQL delivery/readiness, external patient client, confirm payload/write behavior change, model-to-database write, deployment, production, telemetry, global readiness gate, autonomous commit/release authority, or EMR4 runtime behavior. |
| **Next recommended work** | If Yuri continues the sidecar harness track, start with schema/replay-only work plus an automated context rehydration check: mandate, action grammar, role, boundary, evidence, worker-packet, checkpoint, and next-action classification contracts with synthetic replay tests and mock worker results. Keep Codex as the first operator, but keep the core platform-neutral and extraction-ready. Do not enable live agent adapters, branch movement, runtime wiring, provider calls, database writes, deployment, production readiness claims, memory/RAG/GraphRAG, H15/H-series, historical diary access, GUI authority, Omnigent dependency, or automatic release behavior without separate explicit approval. |
