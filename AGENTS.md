...
| **Current active track** | T3 Nondeterministic Bernie Shadow Evaluation is in progress after T2 completed at `b8cd2078`. T3.1-T3.3 now provide the immutable provider-neutral contract/model ledger, exact semantic and independent safety scoring, a strict four-case authored projection from known T1/T2 source scenarios, and a default-disabled injected-adapter repeat runner with separate variance/cost/latency reporting. The T3 suite has 41 passing tests. No provider SDK, route, database, diary mutation, persistence import, live call, or runtime wiring was added. Details are in `docs/bernie-t3-shadow-evaluation.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T3.4: review the adapter boundary and add a default-blocked live-replay gate. Then implement DeepSeek and Gemini adapters behind the protocol using exact model ledger entries, bounded repeats, synthetic inputs, redacted artifacts, and deterministic read-only tools only. Enabling a cost-incurring live replay is a meaningful execution boundary; contract and fake-adapter work may continue without user pause. No model promotion or write authority is implied. Preserve all clinical, deployment, provider-PHI, historical-trove runtime, memory/RAG/GraphRAG, and production gates. Dependabot alert 5 remains open; do not force overrides. |
