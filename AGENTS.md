...
| **Current active track** | Sprint 265 practitioner-directory runtime consumer evidence is complete locally: route-intercepted Playwright now proves the Office add-in Diary booking practitioner selector/list calls `GET /api/v1/practice/practitioners?activeOnly=true&limit=200` through existing bearer-token `apiFetch`, renders display-safe route UUID/name/location data, preserves legacy AHPRA fallback, fails closed on route 401 with the auth banner and token clearing, makes no smoke-mode route call, and renders 200 returned rows. Backend route tests reconfirm auth, tenancy, sensitive-field absence, active-only policy, default-location scoping, and the `limit <= 200` contract. DeepSeek returned PASS. No deployment, production, external-client, write, provider, memory/RAG/GraphRAG, H15/H-series, trove, or GraphQL readiness gate has been opened. |
| **Next recommended work** | Move to practitioner-directory GraphQL SDL/resolver alignment as the next sprint block, because the named REST consumer evidence now passes. Keep GraphQL readiness false until implemented and separately verified; keep deployment/production/external-client/write/provider/memory/H15/trove gates closed. D5 expansion remains paused unless separately approved. Use each worker only where it has a distinct artifact or veto surface. |
