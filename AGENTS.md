...
| **Current active track** | Sprint 276 practitioner-directory Office add-in GraphQL mock contract scaffold is complete locally: `tests/practitioner_directory_office_addin_graphql_mock_contract.py`, `tests/test_practitioner_directory_office_addin_graphql_mock_contract.py`, `tests/practitioner_directory_office_addin_graphql_mock_contract/DRIFT.md`, and `docs/api-spine/practitioner-directory-office-addin-graphql-mock-contract-scaffold.{json,md}` define a Python tests-only contract reference for a future taskpane wrapper. Antigravity returned PASS on Office add-in UX/test guardrails through `agy.exe`; DeepSeek returned PASS with concerns integrated; Claude was deliberately stood down because no API contract, resolver, backend route, or runtime auth/error model changed. The scaffold consumes mocked response dictionaries only, imports no `app` code, does not edit `taskpane.js`, classifies HTTP `401` separately from GraphQL body errors, handles `practice=null` and `defaultLocation=null`, discards projection drift, parameterizes disabled-gate behavior, represents REST fallback only as an event, and keeps all runtime switch, hidden flag, shadow fetch, telemetry, readiness, deployment, production, external-client, write/audit-write, provider, memory/RAG/GraphRAG, H15/H-series, trove, mutation, subscription, and field-expansion gates closed. |
| **Next recommended work** | Pause for a separate consumer switch approval before any Office add-in taskpane runtime implementation or live GraphQL traffic. Do not edit `EMR4 Sidebar/src/taskpane/taskpane.js` for this path without explicit switch approval. D5 expansion remains paused unless separately approved. |
