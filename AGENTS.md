...
| Required Git relation | The architectural-health review closeout is committed at task source `8ce3a591fa0e63ad2d68bf95a8d7e24369dd872f` on `codex/ariadne-bernie-davida-parallel-seam`; subsequent task-branch commits may contain only its bounded conformance-repair closeout and AES-C0 descendants. Local/origin `master` and `handoff/current` remain aligned at protected `2e34bdad732fdab32fbf778280b3d3c70d66d602`; no protected ref movement is authorised. User-owned untracked `docs/branding/` and all unrelated untracked files remain preserved and excluded. |
...
| Current result | At Continuity 236 / Compass 218, the bounded provider-free conformance repair passes as `raisa_codebase_conformance_repair_pass` over task baseline `8ce3a591fa0e63ad2d68bf95a8d7e24369dd872f`. One explicit manifest selects 202 ordinary maintained Python files without protected enumeration; protected CI now requires its exact Python 3.11 runtime, in-memory compilation, Ruff/leakage and a bounded no-conftest static packet. The historical five-row API gap inventory remains unchanged while a current lifecycle index records practitioner REST/GraphQL as implemented, mounted and read-only but not deployed or production-ready; reminder, message, RACGP and Cochrane surfaces remain gaps. Baton/plan consistency tests remove the two stale review-next instructions. The sole product-source edit removes one unused import and changes no behavior. |
| Next implementation | No user decision fork is present. Begin AES-C0 architecture and contract for the Agent Execution Surface and Containment Gate. Freeze exact capability classes, the external broker trust boundary, immutable per-generation manifests, no-ambient-authority and no-fallback states, route classification, command separation, denial budgets, revocation and an externally owned kill switch. This is architecture-only: no broker implementation, product/runtime behavior, applied migration, operational credentials/persistence, outbox/feed/watcher/listener/source access, live or product/patient/clinical data, provider call, model execution, tool, command/write, deployment, production, release, Pages or protected-ref movement is opened. Preserve and exclude `docs/branding/` and unrelated untracked files. |
...
| Agent Execution Surface and Containment Gate direction | On 2026-08-08 Yuri accepted a hard prerequisite before any occupied Bureau receives real product-derived context or any executable tool, filesystem, database, network, credential, provider-executed tool, command-adjacent capability or reusable runtime authority. The prerequisite Context Fabric serial durability behavior slice, intervening architectural-health review and bounded CI/lifecycle conformance repair now pass. AES-C0 architecture and contract is next. The Gate uses an external deterministic capability broker, immutable per-generation manifests, no ambient credentials or metadata access, exact egress, cumulative action/denial budgets, generation-wide revocation, inert typed context, supply-chain identity and an externally owned kill switch. Reasoning, information, egress and action-authority budgets remain independent. Controlling artifacts are `docs/raisa-agent-execution-surface-containment-gate-plan.md`, `docs/security/raisa-agent-execution-surface-containment-gate-threat-model-delta.md`, the accepted architectural-health review and `implementation_plan.md`. This direction grants no broker implementation, provider call, product/patient data, credential/IAM change, tool, command, deployment, production, release, Pages or protected-ref authority. |
