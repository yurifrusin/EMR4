...
| **Current active track** | Yuri approved the practitioner-directory REST first slice on 2026-07-08 in `docs/api-spine/practitioner-directory-approved-gate.json` with decision `approved_for_rest_route_first_slice`, reviewer `yuri`, expiry `2027-07-01`, and approved contract commit `ce23212d538fbba24e5061def2142b817d5528ad`; the external read-model readiness DAG and safe aggregate snapshot now record the approval gate decision while all runtime/provider/GraphQL/memory/write/readiness flags remain false/blocked and no route code has been implemented |
| **Next recommended work** | Start the bounded `GET /api/v1/practice/practitioners` REST route/schema/read-service implementation sprint using the approved gate plus Sprints 227-234 drift/security/SDL/breakdown/approval/DAG contracts and Sprint 230 security preflight; keep SDL/GraphQL/providers/memory/write/readiness gates closed, do not claim deployment or production readiness, and include runtime tests for authn/authz, practice scoping, inactive inclusion, sensitive-field exclusion, default-location scoping, deterministic ordering, pagination bounds, no writes, no provider/Access AI/RAG/GraphRAG, no H15/H-series/trove imports, and unchanged readiness snapshot; at every sprint start explicitly announce Claude, Antigravity, and DeepSeek use/non-use, invocation mode (`scripts\drive_agent_headless.py`/Claude CLI, `agy.exe`/Antigravity CLI, direct Codex `deepseek-worker` spawn), Claude/Antigravity/DeepSeek cleanliness state, current DeepSeek lane count and reuse/cleanup plan, and any extra DeepSeek substitution for unavailable Claude/Antigravity lanes; clean stale worker artifacts before progressing and reannounce clean state |
