...
| Next implementation | Proceed under standing authority with `raisa-canonical-check-in-operational-evidence-acquisition-root-user-decision`: Await Yuri's choice between defer_keep_closed and commence_nonproduction_evidence_acquisition, including a target non-production environment and practice scope on commence. The active latch is the exact authority boundary. Protected refs remain closed; preserve `docs/branding/` and stage explicit paths only. |
