...
| **Current active track** | Practitioner-directory REST first slice implemented on 2026-07-08 under Yuri's approved gate `docs/api-spine/practitioner-directory-approved-gate.json` with decision `approved_for_rest_route_first_slice`, reviewer `yuri`, expiry `2027-07-01`, and approved contract commit `ce23212d538fbba24e5061def2142b817d5528ad`; the slice adds `GET /api/v1/practice/practitioners`, minimal response schemas, a practice-scoped read service, and runtime tests, while SDL/GraphQL/providers/memory/write/readiness/deployment gates remain false/blocked |
| **Next recommended work** | Pause for a bounded post-implementation review/readiness decision before changing `rest_route_ready`, adding SDL/GraphQL resolver coverage, provider/memory/RAG/GraphRAG wiring, write authority, deployment claims, or broad external exposure; if Yuri approves the next practitioner-directory step, run it as a separate gate-aware sprint with the same authn/authz, practice scoping, sensitive-field exclusion, no-write, no-provider, H15/H-series/trove isolation, and readiness-snapshot invariants; at every sprint start explicitly announce Claude, Antigravity, and DeepSeek use/non-use, invocation mode (`scripts\drive_agent_headless.py`/Claude CLI, `agy.exe`/Antigravity CLI, direct Codex `deepseek-worker` spawn), Claude/Antigravity/DeepSeek cleanliness state, current DeepSeek lane count and reuse/cleanup plan, and any extra DeepSeek substitution for unavailable Claude/Antigravity lanes; clean stale worker artifacts before progressing and reannounce clean state |
