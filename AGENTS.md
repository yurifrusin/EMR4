...
| **Current active track** | T2 Deterministic Bernie Behaviour Matrix is complete through T2.2. The economical service gate now executes 746 generated classifier combinations: 260 interval-geometry cases plus 486 exact-match precedence cases across practitioner, type, location, duration, and temporal evidence. Six focused E3 Playwright outcomes cover no-slots, roster-unavailable, clarification, stale, failed, and confirmation-pending states with keyboard/focus and confirmation-authority absence checks. T2.1 corrected three UI defects; T2.2 required no product correction. Details are in `docs/bernie-t2-deterministic-behaviour-matrix.md`; ordering remains in `docs/bernie-consultant-triage-implementation-roadmap.md`. |
| **Next recommended work** | Continue without pause to T2.3: add a compact DB-backed route combination table across active/terminal status, same/other location conflicts, roster presence/absence, break warnings, and normalized bounds; retain stale confirmation as an authored stateful golden case and prove every search/proposal combination is non-mutating. Preserve E1/E3 evidence labels and all existing closed authority gates. Dependabot alert 5 remains open; do not force overrides. |
