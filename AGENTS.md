...
| Next implementation | Under Yuri's standing uninterrupted-gate authority, the next safe descendant is a separately bounded provider-free disposable local PostgreSQL-16 parse-and-catalogue rehearsal. Sol first freezes the narrowest fail-closed plan, then may execute only the accepted inert artifact against an owned disposable local cluster with repository-authored synthetic prerequisite stubs, exact PostgreSQL version/role/extension preflight, catalogue/privilege inspection, zero application migration or runtime wiring and complete owned cleanup. Trigger/RLS behavior, concurrency, applied Alembic migration, operational credentials or persistence, live outbox/feed/watcher/listener/source access, product/patient data, application/API/Diary changes, providers, commands/writes, deployment, production, release, Pages and protected-ref movement remain later or closed gates. Cross-Bureau/clinical implementation and Consultant, requests/referrals, prescribing/medicines and billing/claims authority remain separately closed. Preserve and exclude `docs/branding/` and all unrelated receipt/state/evidence/cost-ledger files. |
