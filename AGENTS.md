...
| Current result | At Continuity 268 / Compass 250, `raisa_provider_free_unmounted_status_confirm_route_convergence_composition_rehearsal_pass` is accepted at exact source `41f978ae9837cba50737cfb5f457ab62ac28dbdb`. One unmounted application-service callable now composes exact status-only admission, server-owned authority/session ingress, request/session binding, the accepted physical transaction factory, locked readmission, one injected atomic effect and closed outcome mapping. Before first v1 use, it reconciles the current complete `AppointmentConfirmStatusProposalOut` envelope with the earlier five-field status projection: exact canonical bytes store the complete existing envelope, while appointment/status/reason/waiting-area/warnings remain a validated projection. Initial and replay delivery are byte-identical; incomplete, corrupt, stale, conflicting, revoked and unavailable cases release no success body. Twelve scenarios, 65 hostile mutations, 13 focused, 163 current lineage and 191 canonical tests pass. No router imports the service and no route or database was executed. |
