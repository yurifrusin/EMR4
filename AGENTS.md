...
| **Current active track** | Sprint 207 API Spine raw compatibility frontend deprecation-header consumer integrated in implementation commit `0884bca9`: the shared Diary `apiFetch()` boundary now reads the `Deprecation` response header and emits a developer-facing console warning, while `appointment_raw_compat_mode` remains `audit` and no backend route/runtime behavior changes |
| **Next recommended work** | Continue bounded backend-readiness with a live/non-intercepted raw-compat header consumer verification and CORS exposed-header preflight before any `appointment_raw_compat_mode` change, or a small read-model gap inventory for the practitioner/reminder/message/directory gaps; use Ariadne plus three worker lanes for general sprint work (Claude through `scripts\drive_agent_headless.py` with `--mint-session`, Antigravity via `C:\Users\sarashera\AppData\Local\agy\bin\agy.exe`, and DeepSeek, with extra DeepSeek substitutions when a lane is unavailable), let DeepSeek workers finish before integration, keep readiness/provider gates blocked and false, and do not open provider, memory/RAG/GraphRAG, H15/H-series runtime, broad trove, GraphQL mutation, or model-to-database-write gates |
