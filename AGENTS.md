...
| **Current active track** | Bernie S25-S27 accessible booking authority is integrated and closed at `63bdf4a3`; final combined acceptance passed 249 tests. The next cross-phase strategy is now recorded in `docs/bernie-consultant-triage-implementation-roadmap.md`: first automate stateful multi-turn diary scenarios, then evaluate nondeterministic diary interpretation in shadow mode, establish a shared clinical-safety foundation, build a clinician-facing cited GP assistant, and only later expose a smaller protocol-bound intake/triage subset to reception-facing Bernie. GP-assistant success is not, by itself, triage validation. |
| **Next recommended work** | Run T1 Stateful Diary Scenario Laboratory as the next substantive tranche. Extend the existing YAML route replay with allowlisted diary-state setup, a first-class supervised-booking turn, per-turn DB/audit deltas, and redacted evidence records; then add Yuri's exact create-confirm-repeat duplicate sequence and selected controlled-backend Playwright coverage. Continue through ordinary sprints without pausing unless an explicit decision boundary in the roadmap is reached. Keep autonomous/model-to-database writes, patient-specific consultant runtime, reception triage, PHI-enabled new providers, live clinical pilots, deployment/release, external clients, H15/historical-trove runtime use, memory/RAG/GraphRAG authority, and GraphQL mutations closed. Dependabot alert 5 remains open; do not force overrides. |
