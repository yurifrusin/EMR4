...
| Next implementation | Under Yuri's standing uninterrupted-gate authority, the disposable PostgreSQL-16 plan mechanics passed at `009395ac28eb7ac05017fe5fbd1ae1439ecf948d`, but Sol withheld acceptance after exact manifest reconciliation found the plan's 4/17/11 type split must be 4 domains / 19 enums / 9 composites (total 32). Plan, design and tests now bind exact 4/19/9/32 and require a fresh exact-HEAD catalogue-delta veto before implementation or Docker preflight. Once accepted, Sol may implement only the closed contracts, fixed-path harness and tests, then use an already-local `postgres:16-bookworm` image in one owned `--pull=never`, `--network=none`, no-port/no-mount tmpfs container with rollback-first, cluster-role absence, exact `--file=-` admission, catalogue readback and exact-ID cleanup. Missing client/daemon/image fails without install, pull or fallback. Function/trigger/RLS behavior, concurrency, applied migration, operational credentials/persistence, live source access, product/patient data, application/API/Diary changes, providers, commands/writes, deployment, production, release, Pages and protected-ref movement remain closed. Preserve and exclude `docs/branding/` and unrelated untracked files. |
